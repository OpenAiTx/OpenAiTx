...
**OpenAiTx**は、あなたのGitHubプロジェクトのために20言語の翻訳READMEを自動生成します。プロジェクトのURLの`GitHub`を`OpenAiTx`に置き換えるだけで利用できます。例: https://github.com/OpenAiTx/OpenAiTx => https://openaitx.com/OpenAiTx/OpenAiTx 。
...
プロジェクトに貢献したい場合、必要なのは以下の手順だけです:
プロジェクトをフォーク => プロジェクトをクローン => あなたの言語のスクリプトを選択 => AIトークンを入力 => 実行 => コミット＆プッシュ＆PRを作成
...
- 1日あたり1コンソールで200～400プロジェクト対応。
- 既存プロジェクトは3～N日ごとに更新されます（今後の計算リソース次第😎）



### 感謝

- Microsoft MVPチームより、オープンソースコミュニティ向けazureプラットフォーム予算150USD/月を提供いただいています。
