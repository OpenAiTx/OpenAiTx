...
**OpenAiTx** — инструмент для автоматического перевода на 20 языков для файла readme вашего проекта на GitHub.

## Почему?

1. Одноразовый перевод с использованием LLM премиум-класса с бесплатным доступом к результату для open-source сообщества.
2. Поддержка SEO поиска в Google/Bing на разных языках — клиентские переводчики не могут это обеспечить.
3. Бесплатно и с открытым исходным кодом навсегда
4. Достаточно отправить один раз и получать автоматические обновления в будущем — экономит время для мейнтейнеров проектов.

## Начало работы

1. Замените URL `GitHub` на `OpenAiTx`, например https://github.com/OpenAiTx/OpenAiTx → https://openaitx.com/OpenAiTx/OpenAiTx  
2. Кликните и скопируйте понравившийся стиль бейджа или текст — система автоматически создаст PR для проекта после индексации.
...
Если вы можете помочь, отправив PR в ваш любимый проект, это значительно поможет продвижению нашего проекта.

Пример:

```
Заголовок:
docs: добавить 20 языковых ссылок в README

Содержание:
PR добавляет 20 языковых ссылок в README, и пользователи могут легко получить доступ к переведённой документации. Поддерживает многоязыковой SEO-поиск Google/Bing.

Демонстрация страницы https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ja

> OpenAiTx https://github.com/OpenAiTx/OpenAiTx 
> Почему?
> 1. Однократный перевод с использованием LLM премиум-класса и предоставление результата в открытый доступ для open-source сообщества.
> 2. Поддержка многоязыкового SEO-поиска Google/Bing, клиентские инструменты перевода не справляются с этим.
> 3. Бесплатно и с открытым исходным кодом навсегда.
> 4. Одна отправка и автоматическое обновление в будущем — это экономит ваше время, если вы мейнтейнер проекта.
```

## Вклад в вычисления

Если вы хотите внести вклад в проект, всё, что вам нужно сделать:
Fork проекта → Клонировать проект → Выбрать скрипт на вашем языке → Вставить свой AI токен → Запустить → Commit & push & создать PR
...
- Команда Microsoft MVP предоставляет бюджет 150USD/месяц на облачную платформу Azure для open source сообщества.

### Планы на будущее

- Поддержка автоматического перевода GitHub wiki
