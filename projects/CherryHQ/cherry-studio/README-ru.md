<div align="right" >
  <details>
    <summary >🌐 Язык</summary>
    <div>
      <div align="right">
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=en">English</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-CN">简体中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-TW">繁體中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ja">日本語</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ko">한국어</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=hi">हिन्दी</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=th">ไทย</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fr">Français</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=de">Deutsch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=es">Español</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=it">Italiano</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ru">Русский</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pt">Português</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=nl">Nederlands</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pl">Polski</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ar">العربية</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fa">فارسی</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=tr">Türkçe</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=vi">Tiếng Việt</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=id">Bahasa Indonesia</a></p>
      </div>
    </div>
  </details>
</div>

<h1 align="center">
  <a href="https://github.com/CherryHQ/cherry-studio/releases">
    <img src="https://github.com/CherryHQ/cherry-studio/blob/main/build/icon.png?raw=true" width="150" height="150" alt="banner" /><br>
  </a>
</h1>

<p align="center">English | <a href="./docs/README.zh.md">中文</a> | <a href="https://cherry-ai.com">Официальный сайт</a> | <a href="https://docs.cherry-ai.com/cherry-studio-wen-dang/en-us">Документация</a> | <a href="./docs/dev.md">Разработка</a> | <a href="https://github.com/CherryHQ/cherry-studio/issues">Обратная связь</a><br></p>

<div align="center">
  
[![][deepwiki-shield]][deepwiki-link]
[![][twitter-shield]][twitter-link]
[![][discord-shield]][discord-link]
[![][telegram-shield]][telegram-link]

</div>
<div align="center">
  
[![][github-release-shield]][github-release-link]
[![][github-nightly-shield]][github-nightly-link]
[![][github-contributors-shield]][github-contributors-link]
[![][license-shield]][license-link]
[![][commercial-shield]][commercial-link]
[![][sponsor-shield]][sponsor-link]

</div>

<div align="center">
 <a href="https://hellogithub.com/repository/1605492e1e2a4df3be07abfa4578dd37" target="_blank" style="text-decoration: none"><img src="https://api.hellogithub.com/v1/widgets/recommend.svg?rid=1605492e1e2a4df3be07abfa4578dd37" alt="Featured｜HelloGitHub"  width="220" height="55" /></a>
 <a href="https://trendshift.io/repositories/11772" target="_blank" style="text-decoration: none"><img src="https://trendshift.io/api/badge/repositories/11772" alt="kangfenmao%2Fcherry-studio | Trendshift" width="220" height="55" /></a>
 <a href="https://www.producthunt.com/posts/cherry-studio?embed=true&utm_source=badge-featured&utm_medium=badge&utm_souce=badge-cherry&#0045;studio" target="_blank"><img src="https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id=496640&theme=light" alt="Cherry&#0032;Studio - AI&#0032;Chatbots&#0044;&#0032;AI&#0032;Desktop&#0032;Client | Product Hunt" width="220" height="55" /></a>
</div>

# 🍒 Cherry Studio

Cherry Studio — это десктопный клиент с поддержкой множества LLM-провайдеров, доступный для Windows, Mac и Linux.

👏 Присоединяйтесь к [Telegram-группе](https://t.me/CherryStudioAI)｜[Discord](https://discord.gg/wez8HtpxqQ) | [QQ-группа(575014769)](https://qm.qq.com/q/lo0D4qVZKi)

❤️ Нравится Cherry Studio? Поставьте звезду 🌟 или [станьте спонсором](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/sponsor.md), чтобы поддержать разработку!

# 🌠 Скриншоты

![](https://github.com/user-attachments/assets/36dddb2c-e0fb-4a5f-9411-91447bab6e18)

![](https://github.com/user-attachments/assets/f549e8a0-2385-40b4-b52b-2039e39f2930)

![](https://github.com/user-attachments/assets/58e0237c-4d36-40de-b428-53051d982026)

# 🌟 Ключевые возможности

1. **Поддержка различных LLM-провайдеров**:

- ☁️ Крупные облачные LLM-сервисы: OpenAI, Gemini, Anthropic и другие
- 🔗 Интеграция с AI-веб-сервисами: Claude, Peplexity, Poe и др.
- 💻 Поддержка локальных моделей через Ollama, LM Studio

2. **AI-ассистенты и диалоги**:

- 📚 300+ преднастроенных AI-ассистентов
- 🤖 Создание собственных ассистентов
- 💬 Многомодельные одновременные диалоги

3. **Работа с документами и данными**:

- 📄 Поддержка текста, изображений, Office, PDF и др.
- ☁️ Управление файлами и резервное копирование через WebDAV
- 📊 Визуализация диаграмм Mermaid
- 💻 Подсветка синтаксиса кода

4. **Интеграция практических инструментов**:

- 🔍 Функция глобального поиска
- 📝 Система управления темами
- 🔤 Перевод на базе ИИ
- 🎯 Сортировка методом drag-and-drop
- 🔌 Поддержка мини-программ
- ⚙️ MCP(Model Context Protocol) сервер

5. **Улучшенный пользовательский опыт**:

- 🖥️ Кроссплатформенная поддержка Windows, Mac и Linux
- 📦 Готово к использованию — настройка окружения не требуется
- 🎨 Светлая/тёмная темы и прозрачное окно
- 📝 Полная отрисовка Markdown
- 🤲 Лёгкое совместное использование контента

# 📝 Дорожная карта

Мы активно работаем над следующими возможностями и улучшениями:

1. 🎯 **Основные функции**

- Ассистент выбора с интеллектуальным расширением выбора контента
- Глубокое исследование с расширенными возможностями анализа
- Система памяти с глобальной осведомлённостью о контексте
- Предобработка документов с улучшенной обработкой файлов
- MCP Marketplace для экосистемы Model Context Protocol

2. 🗂 **Управление знаниями**

- Заметки и коллекции
- Визуализация на динамическом холсте
- Возможности OCR
- Поддержка TTS (текст-в-речь)

3. 📱 **Поддержка платформ**

- HarmonyOS Edition (ПК)
- Android-приложение (Фаза 1)
- iOS-приложение (Фаза 1)
- Поддержка мультиоконности
- Функция закрепления окна

4. 🔌 **Продвинутые функции**

- Система плагинов
- ASR (автоматическое распознавание речи)
- Рефакторинг взаимодействия ассистента и тем

Следите за нашим прогрессом и вносите вклад на [доске проекта](https://github.com/orgs/CherryHQ/projects/7).

Хотите повлиять на нашу дорожную карту? Присоединяйтесь к [GitHub Discussions](https://github.com/CherryHQ/cherry-studio/discussions), чтобы поделиться идеями и отзывами!

# 🌈 Тема

- Галерея тем: <https://cherrycss.com>
- Тема Aero: <https://github.com/hakadao/CherryStudio-Aero>
- Тема PaperMaterial: <https://github.com/rainoffallingstar/CherryStudio-PaperMaterial>
- Claude dynamic-style: <https://github.com/bjl101501/CherryStudio-Claudestyle-dynamic>
- Тема Maple Neon: <https://github.com/BoningtonChen/CherryStudio_themes>

PR для новых тем приветствуются

# 🤝 Вклад в проект

Мы рады вкладу в развитие Cherry Studio! Вот как вы можете помочь:

1. **Вносите код**: Разрабатывайте новые функции или оптимизируйте существующий код.
2. **Исправляйте ошибки**: Сообщайте и исправляйте найденные баги.
3. **Ведение задач**: Помогайте управлять задачами на GitHub.
4. **Дизайн продукта**: Участвуйте в обсуждениях по дизайну.
5. **Пишите документацию**: Улучшайте руководства и инструкции для пользователей.
6. **Взаимодействие с сообществом**: Присоединяйтесь к обсуждениям и помогайте пользователям.
7. **Продвигайте использование**: Рассказывайте о Cherry Studio.

Ознакомьтесь с [Стратегией ветвления](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/branching-strategy-en.md) для руководства по вкладу

## Начало работы

1. **Сделайте форк репозитория**: Форкните и склонируйте его на свой компьютер.
2. **Создайте ветку**: Для ваших изменений.
3. **Внесите изменения**: Зафиксируйте и отправьте ваши изменения.
4. **Откройте Pull Request**: Опишите ваши изменения и причины.

Для более подробных рекомендаций ознакомьтесь с нашим [Руководством по вкладу](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/CONTRIBUTING.md).

Спасибо за вашу поддержку и участие!

# 🔧 Программа совместной разработки для разработчиков

Мы запускаем Программу совместной разработки Cherry Studio, чтобы способствовать здоровой и позитивной обратной связи в экосистеме open-source. Мы верим, что великое ПО создаётся вместе, и каждый принятый pull request вдыхает новую жизнь в проект.

Мы искренне приглашаем вас присоединиться к числу наших участников и вместе с нами формировать будущее Cherry Studio.

## Программа поощрения участников

Чтобы отблагодарить наших ключевых участников и создать благоприятный цикл, мы учредили следующий долгосрочный план мотивации.

**Первый отслеживаемый период этой программы — 3-й квартал 2025 года (июль, август, сентябрь). Награды за этот цикл будут выданы 1 октября.**
В течение любого отслеживаемого периода (например, с 1 июля по 30 сентября для первого цикла) любой разработчик, который внесет более **30 значимых коммитов** в любой из открытых проектов Cherry Studio на GitHub, получит право на следующие преимущества:

- **Спонсорство подписки Cursor**: Получите кредит или компенсацию в размере **$70 USD** за вашу подписку на [Cursor](https://cursor.sh/), чтобы сделать ИИ вашим самым эффективным помощником в кодировании.
- **Безлимитный доступ к моделям**: Получите **неограниченное** число API-запросов к моделям **DeepSeek** и **Qwen**.
- **Доступ к передовым технологиям**: Наслаждайтесь дополнительными привилегиями, включая доступ к API моделей **Claude**, **Gemini** и **OpenAI**, чтобы всегда оставаться на передовой технологий.

## Развиваемся вместе и планы на будущее

Яркое сообщество — движущая сила любого устойчивого open-source проекта. По мере роста Cherry Studio будет расширяться и наша программа поощрений. Мы стремимся постоянно адаптировать наши бонусы к лучшим инструментам и ресурсам отрасли. Это гарантирует значимую поддержку ключевым участникам, создавая позитивный цикл развития разработчиков, сообщества и самого проекта.

**В дальнейшем проект также будет придерживаться всё более открытой позиции, чтобы отдавать должное всему open-source сообществу.**

## Как начать?

Мы ждем ваш первый Pull Request!

Вы можете начать с изучения наших репозиториев, взяв `good first issue` или предложив свои улучшения. Каждый коммит — это проявление духа open source.

Спасибо за ваш интерес и вклад.

Давайте строить вместе.

# 🏢 Enterprise Edition

На базе Community Edition мы рады представить **Cherry Studio Enterprise Edition** — платформу для повышения продуктивности и управления ИИ, которую можно развернуть в частном порядке для современных команд и предприятий.

Enterprise Edition решает ключевые задачи командного взаимодействия за счет централизации управления ИИ-ресурсами, знаниями и данными. Она позволяет организациям повысить эффективность, стимулировать инновации и обеспечить соответствие требованиям, сохраняя 100% контроль над своими данными в безопасной среде.

## Основные преимущества

- **Единое управление моделями**: Централизованная интеграция и управление различными облачными LLM (например, OpenAI, Anthropic, Google Gemini) и локально развернутыми приватными моделями. Сотрудники могут использовать их "из коробки" без индивидуальной настройки.
- **Корпоративная база знаний**: Создавайте, управляйте и делитесь командными базами знаний. Это обеспечивает сохранение и согласованность информации, позволяя сотрудникам взаимодействовать с ИИ на основе единой и точной информации.
- **Тонкая настройка доступа**: Легко управляйте учетными записями сотрудников и назначайте ролевые права для разных моделей, баз знаний и функций через единый административный бэкенд.
- **Полностью приватное развертывание**: Развертывание всей серверной части на ваших собственных серверах или в частном облаке, что гарантирует 100% приватность и контроль над данными для соответствия самым строгим стандартам безопасности.
- **Надежные серверные сервисы**: Обеспечивает стабильные API-сервисы и механизмы резервного копирования и восстановления данных корпоративного уровня для обеспечения непрерывности бизнеса.

## ✨ Онлайн-демо

> 🚧 **Объявление о публичной бете**
>
> В данный момент Enterprise Edition находится на ранней стадии публичной беты, и мы активно работаем над доработкой и оптимизацией функционала. Мы понимаем, что стабильность может быть не идеальной. Если вы столкнетесь с проблемами или у вас будут ценные предложения во время тестирования, будем признательны за обратную связь по электронной почте.

**🔗 [Cherry Studio Enterprise](https://www.cherry-ai.com/enterprise)**

## Сравнение версий

| Функция           | Community Edition                          | Enterprise Edition                                                                                                                      |
| :---------------- | :----------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **Open Source**   | ✅ Да                                      | ⭕️ Частично доступен для клиентов                                                                                                              |
| **Стоимость**     | Бесплатно для личного пользования / Коммерческая лицензия | Покупка / Подписка                                                                                                               |
| **Админ-панель**  | —                                          | ● Централизованный доступ к **Моделям**<br>● Управление **Сотрудниками**<br>● Общая **База знаний**<br>● **Контроль доступа**<br>● **Бэкап данных** |
| **Сервер**        | —                                          | ✅ Выделенное приватное развертывание                                                                                                         |

## Как получить Enterprise Edition

Мы уверены, что Enterprise Edition станет движком продуктивности вашей команды на базе ИИ. Если вы заинтересованы в Cherry Studio Enterprise Edition и хотите узнать больше, получить коммерческое предложение или назначить демонстрацию, пожалуйста, свяжитесь с нами.

- **Для бизнес-запросов и покупки**:
  **📧 [bd@cherry-ai.com](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/mailto:bd@cherry-ai.com)**

# 🔗 Связанные проекты

- [one-api](https://github.com/songquanpeng/one-api): Система управления и распределения LLM API с поддержкой основных моделей, таких как OpenAI, Azure и Anthropic. Имеет единый API-интерфейс, подходит для управления ключами и вторичного распределения.

- [ublacklist](https://github.com/iorate/ublacklist): Блокирует отображение определённых сайтов в результатах поиска Google

# 🚀 Участники

<a href="https://github.com/CherryHQ/cherry-studio/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=CherryHQ/cherry-studio" />
</a>
<br /><br />

# 📊 GitHub-статистика

![Stats](https://repobeats.axiom.co/api/embed/a693f2e5f773eed620f70031e974552156c7f397.svg 'Repobeats analytics image')

# ⭐️ История звёзд

<a href="https://www.star-history.com/#CherryHQ/cherry-studio&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
 </picture>
</a>

<!-- Ссылки и изображения -->

[deepwiki-shield]: https://img.shields.io/badge/Deepwiki-CherryHQ-0088CC?logo=data:image/svg+xml;base64,
[deepwiki-link]: https://deepwiki.com/CherryHQ/cherry-studio
[twitter-shield]: https://img.shields.io/badge/Twitter-CherryStudioApp-0088CC?logo=x
[twitter-link]: https://twitter.com/CherryStudioHQ
[discord-shield]: https://img.shields.io/badge/Discord-@CherryStudio-0088CC?logo=discord
[discord-link]: https://discord.gg/wez8HtpxqQ
[telegram-shield]: https://img.shields.io/badge/Telegram-@CherryStudioAI-0088CC?logo=telegram
[telegram-link]: https://t.me/CherryStudioAI

<!-- Ссылки и изображения -->

[github-release-shield]: https://img.shields.io/github/v/release/CherryHQ/cherry-studio?logo=github
[github-release-link]: https://github.com/CherryHQ/cherry-studio/releases
[github-nightly-shield]: https://img.shields.io/github/actions/workflow/status/CherryHQ/cherry-studio/nightly-build.yml?label=nightly%20build&logo=github
[github-nightly-link]: https://github.com/CherryHQ/cherry-studio/actions/workflows/nightly-build.yml
[github-contributors-shield]: https://img.shields.io/github/contributors/CherryHQ/cherry-studio?logo=github
[github-contributors-link]: https://github.com/CherryHQ/cherry-studio/graphs/contributors

<!-- Ссылки и изображения -->

[license-shield]: https://img.shields.io/badge/License-AGPLv3-important.svg?logo=gnu
[license-link]: https://www.gnu.org/licenses/agpl-3.0
[commercial-shield]: https://img.shields.io/badge/License-Contact-white.svg?logoColor=white&logo=telegram&color=blue
[commercial-link]: mailto:license@cherry-ai.com?subject=Commercial%20License%20Inquiry
[sponsor-shield]: https://img.shields.io/badge/Sponsor-FF6699.svg?logo=githubsponsors&logoColor=white
[sponsor-link]: https://github.com/CherryHQ/cherry-studio/blob/main/docs/sponsor.md



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2025-07-22

---