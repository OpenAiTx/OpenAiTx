<div align="right" >
  <details>
    <summary >🌐 ภาษา</summary>
    <div>
      <div align="right">
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=en">English</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-CN">简体中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-TW">繁體中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ja">日本語</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ko">한국어</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=hi">हिन्दी</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=th">ไทย</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fr">Français</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=de">Deutsch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=es">Español</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=it">Italiano</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ru">Русский</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pt">Português</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=nl">Nederlands</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pl">Polski</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ar">العربية</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fa">فارسی</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=tr">Türkçe</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=vi">Tiếng Việt</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=id">Bahasa Indonesia</a></p>
      </div>
    </div>
  </details>
</div>

<h1 align="center">
  <a href="https://github.com/CherryHQ/cherry-studio/releases">
    <img src="https://github.com/CherryHQ/cherry-studio/blob/main/build/icon.png?raw=true" width="150" height="150" alt="banner" /><br>
  </a>
</h1>

<p align="center">English | <a href="./docs/README.zh.md">中文</a> | <a href="https://cherry-ai.com">เว็บไซต์หลัก</a> | <a href="https://docs.cherry-ai.com/cherry-studio-wen-dang/en-us">เอกสารประกอบ</a> | <a href="./docs/dev.md">การพัฒนา</a> | <a href="https://github.com/CherryHQ/cherry-studio/issues">ข้อเสนอแนะ</a><br></p>

<div align="center">
  
[![][deepwiki-shield]][deepwiki-link]
[![][twitter-shield]][twitter-link]
[![][discord-shield]][discord-link]
[![][telegram-shield]][telegram-link]

</div>
<div align="center">
  
[![][github-release-shield]][github-release-link]
[![][github-nightly-shield]][github-nightly-link]
[![][github-contributors-shield]][github-contributors-link]
[![][license-shield]][license-link]
[![][commercial-shield]][commercial-link]
[![][sponsor-shield]][sponsor-link]

</div>

<div align="center">
 <a href="https://hellogithub.com/repository/1605492e1e2a4df3be07abfa4578dd37" target="_blank" style="text-decoration: none"><img src="https://api.hellogithub.com/v1/widgets/recommend.svg?rid=1605492e1e2a4df3be07abfa4578dd37" alt="Featured｜HelloGitHub"  width="220" height="55" /></a>
 <a href="https://trendshift.io/repositories/14318" target="_blank" style="text-decoration: none"><img src="https://trendshift.io/api/badge/repositories/14318" alt="CherryHQ%2Fcherry-studio | Trendshift" width="220" height="55" /></a>
 <a href="https://www.producthunt.com/posts/cherry-studio?embed=true&utm_source=badge-featured&utm_medium=badge&utm_souce=badge-cherry&#0045;studio" target="_blank"><img src="https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id=496640&theme=light" alt="Cherry&#0032;Studio - AI&#0032;Chatbots&#0044;&#0032;AI&#0032;Desktop&#0032;Client | Product Hunt" width="220" height="55" /></a>
</div>

# 🍒 Cherry Studio

Cherry Studio เป็นโปรแกรมเดสก์ท็อปที่รองรับผู้ให้บริการ LLM หลายราย ใช้งานได้บน Windows, Mac และ Linux

👏 เข้าร่วม [กลุ่ม Telegram](https://t.me/CherryStudioAI)｜[Discord](https://discord.gg/wez8HtpxqQ) | [กลุ่ม QQ (575014769)](https://qm.qq.com/q/lo0D4qVZKi)

❤️ ชอบ Cherry Studio? ให้ดาว 🌟 หรือ [สนับสนุน](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/sponsor.md) เพื่อช่วยพัฒนา!

# 🌠 ภาพหน้าจอ

![](https://github.com/user-attachments/assets/36dddb2c-e0fb-4a5f-9411-91447bab6e18)

![](https://github.com/user-attachments/assets/f549e8a0-2385-40b4-b52b-2039e39f2930)

![](https://github.com/user-attachments/assets/58e0237c-4d36-40de-b428-53051d982026)

# 🌟 คุณสมบัติเด่น
1. **รองรับผู้ให้บริการ LLM ที่หลากหลาย**:

- ☁️ บริการคลาวด์ LLM รายใหญ่: OpenAI, Gemini, Anthropic และอื่น ๆ
- 🔗 การเชื่อมต่อบริการ AI ทางเว็บ: Claude, Perplexity, Poe และอื่น ๆ
- 💻 รองรับโมเดลในเครื่องด้วย Ollama, LM Studio

2. **ผู้ช่วย AI & การสนทนา**:

- 📚 ผู้ช่วย AI ที่ตั้งค่าล่วงหน้ามากกว่า 300 รายการ
- 🤖 สร้างผู้ช่วยแบบกำหนดเอง
- 💬 สนทนาแบบหลายโมเดลพร้อมกัน

3. **การประมวลผลเอกสาร & ข้อมูล**:

- 📄 รองรับข้อความ, รูปภาพ, Office, PDF และอื่น ๆ
- ☁️ จัดการและสำรองไฟล์ด้วย WebDAV
- 📊 แสดงกราฟด้วย Mermaid Chart
- 💻 ไฮไลท์ไวยากรณ์โค้ด

4. **เครื่องมือใช้งานจริงแบบบูรณาการ**:

- 🔍 ฟังก์ชันค้นหาทั่วระบบ
- 📝 ระบบจัดการหัวข้อ
- 🔤 แปลภาษาโดย AI
- 🎯 ลากและวางเพื่อจัดเรียง
- 🔌 รองรับมินิโปรแกรม
- ⚙️ เซิร์ฟเวอร์ MCP(Model Context Protocol)

5. **ประสบการณ์ผู้ใช้ที่ดียิ่งขึ้น**:

- 🖥️ รองรับข้ามแพลตฟอร์มทั้ง Windows, Mac และ Linux
- 📦 พร้อมใช้งานทันที ไม่ต้องตั้งค่าสภาพแวดล้อม
- 🎨 ธีมสว่าง/มืด และหน้าต่างโปร่งใส
- 📝 แสดงผล Markdown ได้สมบูรณ์
- 🤲 แชร์เนื้อหาได้ง่าย

# 📝 แผนงาน

เรากำลังดำเนินการพัฒนาฟีเจอร์และปรับปรุงดังต่อไปนี้:


1. 🎯 **ฟีเจอร์หลัก**

- ผู้ช่วยเลือกด้วยการเสริมการเลือกเนื้อหาอัจฉริยะ
- การวิจัยเชิงลึกด้วยความสามารถการวิจัยขั้นสูง
- ระบบหน่วยความจำที่มีการรับรู้บริบททั่วโลก
- การเตรียมเอกสารล่วงหน้าด้วยการจัดการเอกสารที่ดีขึ้น
- MCP Marketplace สำหรับระบบนิเวศ Model Context Protocol

2. 🗂 **การจัดการความรู้**

- โน้ตและคอลเลกชัน
- การแสดงภาพ Dynamic Canvas
- ความสามารถ OCR
- รองรับ TTS (ข้อความเป็นเสียง)

3. 📱 **รองรับแพลตฟอร์ม**

- HarmonyOS Edition (PC)
- แอป Android (เฟส 1)
- แอป iOS (เฟส 1)
- รองรับหลายหน้าต่าง
- ฟังก์ชั่นตรึงหน้าต่าง

4. 🔌 **ฟีเจอร์ขั้นสูง**

- ระบบปลั๊กอิน
- ASR (การรู้จำเสียงอัตโนมัติ)
- ปรับปรุงปฏิสัมพันธ์ผู้ช่วยและหัวข้อ

ติดตามความคืบหน้าและมีส่วนร่วมใน [บอร์ดโครงการของเรา](https://github.com/orgs/CherryHQ/projects/7)

อยากมีส่วนร่วมในแผนงาน? เข้าร่วม [GitHub Discussions](https://github.com/CherryHQ/cherry-studio/discussions) เพื่อแบ่งปันแนวคิดและข้อเสนอแนะ!

# 🌈 ธีม

- แกลเลอรีธีม: <https://cherrycss.com>
- ธีม Aero: <https://github.com/hakadao/CherryStudio-Aero>
- ธีม PaperMaterial: <https://github.com/rainoffallingstar/CherryStudio-PaperMaterial>
- Claude dynamic-style: <https://github.com/bjl101501/CherryStudio-Claudestyle-dynamic>
- ธีม Maple Neon: <https://github.com/BoningtonChen/CherryStudio_themes>

ยินดีรับ PR สำหรับธีมเพิ่มเติม

# 🤝 การมีส่วนร่วม

เรายินดีต้อนรับทุกการมีส่วนร่วมกับ Cherry Studio! คุณสามารถช่วยเหลือเราได้หลายวิธีดังนี้:

1. **ร่วมพัฒนาโค้ด**: พัฒนาฟีเจอร์ใหม่หรือปรับปรุงโค้ดเดิม
2. **แก้ไขข้อบกพร่อง**: ส่งข้อเสนอการแก้ไขบั๊กที่พบ
3. **ดูแลประเด็นต่างๆ**: ช่วยจัดการประเด็นใน GitHub
4. **ออกแบบผลิตภัณฑ์**: ร่วมอภิปรายด้านการออกแบบ
5. **เขียนเอกสาร**: ปรับปรุงคู่มือผู้ใช้และแนวทางการใช้งาน
6. **สร้างชุมชน**: เข้าร่วมสนทนาและช่วยเหลือผู้ใช้
7. **ประชาสัมพันธ์การใช้งาน**: เผยแพร่ Cherry Studio ให้เป็นที่รู้จัก

ศึกษาคู่มือการมีส่วนร่วมได้ที่ [กลยุทธ์การแบ่งสาขา](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/branching-strategy-en.md)

## เริ่มต้นใช้งาน

1. **Fork Repository**: Fork และโคลนไปยังเครื่องของคุณ
2. **สร้างสาขาใหม่**: สำหรับการเปลี่ยนแปลงของคุณ
3. **ส่งการเปลี่ยนแปลง**: Commit และ push การเปลี่ยนแปลงของคุณ
4. **เปิด Pull Request**: อธิบายสิ่งที่เปลี่ยนแปลงและเหตุผล

ศึกษาคู่มือแบบละเอียดได้ที่ [Contributing Guide](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/CONTRIBUTING.md)

ขอขอบคุณสำหรับการสนับสนุนและการมีส่วนร่วมของคุณ!

# 🔧 โปรแกรมร่วมสร้างสรรค์สำหรับนักพัฒนา

เรากำลังเปิดตัวโปรแกรมร่วมสร้างสรรค์ Cherry Studio เพื่อส่งเสริมระบบนิเวศโอเพ่นซอร์สให้แข็งแกร่งและเกิดผลตอบรับเชิงบวก เราเชื่อว่าซอฟต์แวร์ที่ยอดเยี่ยมเกิดจากความร่วมมือ และทุก Pull Request ที่ถูก Merge จะช่วยขับเคลื่อนโครงการให้มีชีวิตชีวา

ขอเชิญคุณเข้าร่วมเป็นผู้สนับสนุนและร่วมกำหนดอนาคตของ Cherry Studio กับเรา

## โปรแกรมรางวัลสำหรับผู้ร่วมพัฒนา

เพื่อขอบคุณผู้มีส่วนร่วมหลักและสร้างวงจรที่ดี เราได้จัดตั้งแผนจูงใจระยะยาวดังนี้

**ช่วงติดตามครั้งแรกของโปรแกรมนี้คือ Q3 2025 (กรกฎาคม, สิงหาคม, กันยายน) โดยจะมีการแจกจ่ายรางวัลในวันที่ 1 ตุลาคมนี้**

ในช่วงระยะเวลาการติดตามใดๆ (เช่น 1 กรกฎาคม ถึง 30 กันยายน สำหรับรอบแรก) นักพัฒนาที่มีส่วนร่วมมากกว่า **30 คอมมิตที่มีความหมาย** ในโปรเจกต์โอเพนซอร์สใดๆ ของ Cherry Studio บน GitHub จะมีสิทธิ์ได้รับสิทธิประโยชน์ต่อไปนี้:

- **สนับสนุนการสมัครสมาชิก Cursor**: รับเครดิตหรือคืนเงิน **$70 USD** สำหรับการสมัครสมาชิก [Cursor](https://cursor.sh/) เพื่อให้ AI เป็นคู่หูในการเขียนโค้ดที่มีประสิทธิภาพสูงสุดของคุณ
- **เข้าถึงโมเดลแบบไม่จำกัด**: รับสิทธิ์การเรียก API แบบ **ไม่จำกัด** สำหรับโมเดล **DeepSeek** และ **Qwen**
- **เข้าถึงเทคโนโลยีล้ำสมัย**: รับสิทธิพิเศษในบางโอกาส เช่น การเข้าถึง API ของโมเดล **Claude**, **Gemini** และ **OpenAI** เพื่อให้คุณอยู่ในแนวหน้าของเทคโนโลยี

## เติบโตไปด้วยกัน & แผนในอนาคต

ชุมชนที่มีชีวิตชีวาคือแรงขับเคลื่อนหลักของโปรเจกต์โอเพนซอร์สที่ยั่งยืน เมื่อ Cherry Studio เติบโตขึ้น โปรแกรมรางวัลของเราก็จะเติบโตขึ้นด้วย เรามุ่งมั่นที่จะปรับสิทธิประโยชน์ให้สอดคล้องกับเครื่องมือและทรัพยากรระดับแนวหน้าของอุตสาหกรรมอย่างต่อเนื่อง เพื่อให้ผู้มีส่วนร่วมหลักได้รับการสนับสนุนที่มีความหมาย สร้างวงจรบวกที่นักพัฒนา ชุมชน และโปรเจกต์เติบโตไปด้วยกัน

**ต่อจากนี้ โปรเจกต์จะเปิดกว้างมากขึ้นเพื่อคืนสู่ชุมชนโอเพนซอร์สทั้งหมด**

## จะเริ่มต้นอย่างไร?

เรารอคอย Pull Request แรกของคุณ!

คุณสามารถเริ่มต้นได้โดยสำรวจรีโพสิตอรีของเรา เลือก `good first issue` หรือเสนอแนวคิดปรับปรุงของคุณเอง ทุกคอมมิตคือหลักฐานของจิตวิญญาณโอเพนซอร์ส

ขอขอบคุณสำหรับความสนใจและการมีส่วนร่วมของคุณ

มาสร้างไปด้วยกัน

# 🏢 Enterprise Edition

ต่อยอดจาก Community Edition เราภูมิใจนำเสนอ **Cherry Studio Enterprise Edition**—แพลตฟอร์ม AI เพื่อเพิ่มประสิทธิผลและบริหารจัดการสำหรับทีมและองค์กรยุคใหม่ที่ติดตั้งใช้งานแบบส่วนตัวได้

Enterprise Edition มุ่งแก้ไขปัญหาหลักในการทำงานร่วมกันของทีม โดยรวมศูนย์การจัดการทรัพยากร AI ความรู้ และข้อมูล ช่วยให้องค์กรเพิ่มประสิทธิภาพ ส่งเสริมนวัตกรรม และรักษามาตรฐานความปลอดภัยและการปฏิบัติตามข้อกำหนด พร้อมควบคุมข้อมูลได้ 100% ในสภาพแวดล้อมที่ปลอดภัย

## ข้อดีหลัก

- **การจัดการโมเดลแบบรวมศูนย์**: บริหารจัดการ LLM จากคลาวด์ (เช่น OpenAI, Anthropic, Google Gemini) และโมเดลส่วนตัวที่ติดตั้งในองค์กรได้จากส่วนกลาง พนักงานใช้งานได้ทันทีโดยไม่ต้องตั้งค่าเอง
- **ฐานความรู้ระดับองค์กร**: สร้าง จัดการ และแบ่งปันฐานความรู้สำหรับทั้งทีม เพื่อรักษาและสร้างความต่อเนื่องของความรู้ ให้สมาชิกทีมโต้ตอบกับ AI โดยอิงข้อมูลที่ถูกต้องและเป็นหนึ่งเดียว
- **การควบคุมสิทธิ์การเข้าถึงละเอียด**: จัดการบัญชีพนักงานและกำหนดสิทธิ์ตามบทบาทสำหรับโมเดล ฐานความรู้ และฟีเจอร์ต่างๆ ผ่านระบบแอดมินส่วนกลาง
- **ติดตั้งใช้งานแบบส่วนตัวเต็มรูปแบบ**: สามารถติดตั้งบริการ backend ทั้งหมดบนเซิร์ฟเวอร์ในองค์กรหรือคลาวด์ส่วนตัว เพื่อให้ข้อมูลของคุณเป็นส่วนตัว 100% และควบคุมได้เอง รองรับมาตรฐานความปลอดภัยและการปฏิบัติตามข้อกำหนดที่เข้มงวดที่สุด
- **บริการ backend ที่เชื่อถือได้**: ให้บริการ API ที่เสถียร พร้อมระบบสำรองข้อมูลและกู้คืนสำหรับองค์กร เพื่อรับประกันความต่อเนื่องทางธุรกิจ

## ✨ สาธิตออนไลน์

> 🚧 **แจ้งเตือนรุ่น Public Beta**
>
> ขณะนี้ Cherry Studio Enterprise Edition อยู่ในช่วงเบต้าสาธารณะระยะแรก และเรากำลังพัฒนาและปรับปรุงฟีเจอร์ต่าง ๆ อย่างต่อเนื่อง เราตระหนักดีว่าระบบอาจยังไม่เสถียรสมบูรณ์ หากคุณพบปัญหาใด ๆ หรือมีข้อเสนอแนะที่มีคุณค่าในระหว่างการทดลองใช้งาน จะขอขอบคุณอย่างยิ่งหากคุณติดต่อเราทางอีเมลเพื่อแจ้งข้อคิดเห็น

**🔗 [Cherry Studio Enterprise](https://www.cherry-ai.com/enterprise)**

## เปรียบเทียบเวอร์ชัน

| ฟีเจอร์              | Community Edition                             | Enterprise Edition                                                                                                                        |
| :-------------------- | :-------------------------------------------- | :--------------------------------------------------------------------------------------------------------------------------------------- |
| **โอเพ่นซอร์ส**      | ✅ ใช่                                        | ⭕️ เปิดให้ลูกค้าบางส่วนเท่านั้น                                                                                                         |
| **ค่าใช้จ่าย**        | ฟรีสำหรับใช้ส่วนตัว / ใบอนุญาตเชิงพาณิชย์   | ซื้อขาด / ค่าบริการรายเดือน                                                                                                            |
| **ระบบแอดมิน**        | —                                            | ● การเข้าถึง **โมเดล** แบบรวมศูนย์<br>● การจัดการ **พนักงาน**<br>● ฐานความรู้ **ร่วมกัน**<br>● การควบคุม **สิทธิ์เข้าถึง**<br>● สำรองข้อมูล **Data** |
| **เซิร์ฟเวอร์**        | —                                            | ✅ ติดตั้งส่วนตัวเฉพาะองค์กร                                                                                                            |

## วิธีรับ Enterprise Edition

เราเชื่อว่า Enterprise Edition จะกลายเป็นเครื่องมือเพิ่มประสิทธิภาพ AI สำหรับทีมของคุณ หากคุณสนใจ Cherry Studio Enterprise Edition และต้องการข้อมูลเพิ่มเติม ขอใบเสนอราคา หรือจองเดโม กรุณาติดต่อเราได้ทุกเมื่อ

- **สำหรับติดต่อธุรกิจ & สั่งซื้อ**:
  **📧 [bd@cherry-ai.com](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/mailto:bd@cherry-ai.com)**

# 🔗 โปรเจกต์ที่เกี่ยวข้อง

- [one-api](https://github.com/songquanpeng/one-api): ระบบจัดการและกระจาย API สำหรับ LLM รองรับโมเดลหลัก เช่น OpenAI, Azure, Anthropic อินเทอร์เฟซ API เดียว เหมาะสำหรับจัดการคีย์และกระจาย API ต่อ

- [ublacklist](https://github.com/iorate/ublacklist): บล็อกเว็บไซต์ที่ไม่ต้องการไม่ให้แสดงในผลการค้นหา Google

# 🚀 ผู้ร่วมพัฒนา

<a href="https://github.com/CherryHQ/cherry-studio/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=CherryHQ/cherry-studio" />
</a>
<br /><br />

# 📊 สถิติ GitHub

![Stats](https://repobeats.axiom.co/api/embed/a693f2e5f773eed620f70031e974552156c7f397.svg 'Repobeats analytics image')

# ⭐️ ประวัติการให้ดาว

<a href="https://www.star-history.com/#CherryHQ/cherry-studio&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
   <img alt="กราฟประวัติดาว" src="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
 </picture>
</a>

<!-- ลิงก์ & รูปภาพ -->

[deepwiki-shield]: https://img.shields.io/badge/Deepwiki-CherryHQ-0088CC?logo=data:image/svg+xml;base64,
[deepwiki-link]: https://deepwiki.com/CherryHQ/cherry-studio
[twitter-shield]: https://img.shields.io/badge/Twitter-CherryStudioApp-0088CC?logo=x
[twitter-link]: https://twitter.com/CherryStudioHQ
[discord-shield]: https://img.shields.io/badge/Discord-@CherryStudio-0088CC?logo=discord
[discord-link]: https://discord.gg/wez8HtpxqQ
[telegram-shield]: https://img.shields.io/badge/Telegram-@CherryStudioAI-0088CC?logo=telegram
[telegram-link]: https://t.me/CherryStudioAI

<!-- ลิงก์ & รูปภาพ -->

[github-release-shield]: https://img.shields.io/github/v/release/CherryHQ/cherry-studio?logo=github
[github-release-link]: https://github.com/CherryHQ/cherry-studio/releases
[github-nightly-shield]: https://img.shields.io/github/actions/workflow/status/CherryHQ/cherry-studio/nightly-build.yml?label=nightly%20build&logo=github
[github-nightly-link]: https://github.com/CherryHQ/cherry-studio/actions/workflows/nightly-build.yml
[github-contributors-shield]: https://img.shields.io/github/contributors/CherryHQ/cherry-studio?logo=github
[github-contributors-link]: https://github.com/CherryHQ/cherry-studio/graphs/contributors

<!-- ลิงก์ & รูปภาพ -->

[license-shield]: https://img.shields.io/badge/License-AGPLv3-important.svg?logo=gnu
[license-link]: https://www.gnu.org/licenses/agpl-3.0
[commercial-shield]: https://img.shields.io/badge/License-Contact-white.svg?logoColor=white&logo=telegram&color=blue
[commercial-link]: mailto:license@cherry-ai.com?subject=Commercial%20License%20Inquiry
[sponsor-shield]: https://img.shields.io/badge/Sponsor-FF6699.svg?logo=githubsponsors&logoColor=white
[sponsor-link]: https://github.com/CherryHQ/cherry-studio/blob/main/docs/sponsor.md


---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2025-10-05

---