<div align="right" >
  <details>
    <summary >🌐 ภาษา</summary>
    <div>
      <div align="right">
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=en">English</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-CN">简体中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-TW">繁體中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ja">日本語</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ko">한국어</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=hi">हिन्दी</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=th">ไทย</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fr">Français</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=de">Deutsch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=es">Español</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=it">Italiano</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ru">Русский</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pt">Português</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=nl">Nederlands</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pl">Polski</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ar">العربية</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fa">فارسی</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=tr">Türkçe</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=vi">Tiếng Việt</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=id">Bahasa Indonesia</a></p>
      </div>
    </div>
  </details>
</div>

<h1 align="center">
  <a href="https://github.com/CherryHQ/cherry-studio/releases">
    <img src="https://github.com/CherryHQ/cherry-studio/blob/main/build/icon.png?raw=true" width="150" height="150" alt="banner" /><br>
  </a>
</h1>

<p align="center">English | <a href="./docs/README.zh.md">中文</a> | <a href="https://cherry-ai.com">เว็บไซต์ทางการ</a> | <a href="https://docs.cherry-ai.com/cherry-studio-wen-dang/en-us">เอกสาร</a> | <a href="./docs/dev.md">การพัฒนา</a> | <a href="https://github.com/CherryHQ/cherry-studio/issues">ข้อเสนอแนะ</a><br></p>

<div align="center">
  
[![][deepwiki-shield]][deepwiki-link]
[![][twitter-shield]][twitter-link]
[![][discord-shield]][discord-link]
[![][telegram-shield]][telegram-link]

</div>
<div align="center">
  
[![][github-release-shield]][github-release-link]
[![][github-nightly-shield]][github-nightly-link]
[![][github-contributors-shield]][github-contributors-link]
[![][license-shield]][license-link]
[![][commercial-shield]][commercial-link]
[![][sponsor-shield]][sponsor-link]

</div>

<div align="center">
 <a href="https://hellogithub.com/repository/1605492e1e2a4df3be07abfa4578dd37" target="_blank" style="text-decoration: none"><img src="https://api.hellogithub.com/v1/widgets/recommend.svg?rid=1605492e1e2a4df3be07abfa4578dd37" alt="Featured｜HelloGitHub"  width="220" height="55" /></a>
 <a href="https://trendshift.io/repositories/11772" target="_blank" style="text-decoration: none"><img src="https://trendshift.io/api/badge/repositories/11772" alt="kangfenmao%2Fcherry-studio | Trendshift" width="220" height="55" /></a>
 <a href="https://www.producthunt.com/posts/cherry-studio?embed=true&utm_source=badge-featured&utm_medium=badge&utm_souce=badge-cherry&#0045;studio" target="_blank"><img src="https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id=496640&theme=light" alt="Cherry&#0032;Studio - AI&#0032;Chatbots&#0044;&#0032;AI&#0032;Desktop&#0032;Client | Product Hunt" width="220" height="55" /></a>
</div>

# 🍒 Cherry Studio

Cherry Studio เป็นโปรแกรมเดสก์ท็อปที่รองรับผู้ให้บริการ LLM หลายราย ใช้งานได้บน Windows, Mac และ Linux

👏 เข้าร่วม [กลุ่ม Telegram](https://t.me/CherryStudioAI)｜[Discord](https://discord.gg/wez8HtpxqQ) | [กลุ่ม QQ(575014769)](https://qm.qq.com/q/lo0D4qVZKi)

❤️ ชอบ Cherry Studio ใช่ไหม? ให้ดาว 🌟 หรือ [สนับสนุน](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/sponsor.md) เพื่อสนับสนุนการพัฒนา!

# 🌠 ภาพหน้าจอ

![](https://github.com/user-attachments/assets/36dddb2c-e0fb-4a5f-9411-91447bab6e18)

![](https://github.com/user-attachments/assets/f549e8a0-2385-40b4-b52b-2039e39f2930)

![](https://github.com/user-attachments/assets/58e0237c-4d36-40de-b428-53051d982026)

# 🌟 คุณสมบัติเด่น

1. **รองรับผู้ให้บริการ LLM หลากหลาย**:

- ☁️ บริการ LLM คลาวด์หลัก: OpenAI, Gemini, Anthropic และอื่นๆ
- 🔗 การเชื่อมต่อบริการ AI บนเว็บ: Claude, Peplexity, Poe และอื่นๆ
- 💻 รองรับโมเดลโลคัลด้วย Ollama, LM Studio

2. **ผู้ช่วย AI & การสนทนา**:

- 📚 มีผู้ช่วย AI ที่ตั้งค่าล่วงหน้ามากกว่า 300 รายการ
- 🤖 สร้างผู้ช่วยแบบกำหนดเองได้
- 💬 สนทนาแบบหลายโมเดลพร้อมกัน

3. **ประมวลผลเอกสารและข้อมูล**:

- 📄 รองรับข้อความ รูปภาพ เอกสาร Office, PDF และอื่นๆ
- ☁️ การจัดการไฟล์และสำรองข้อมูลด้วย WebDAV
- 📊 สร้างภาพแผนภูมิ Mermaid
- 💻 ไฮไลต์ไวยากรณ์โค้ด

4. **การผสานเครื่องมือที่ใช้งานจริง**:

- 🔍 ฟังก์ชันค้นหาทั่วโลก
- 📝 ระบบจัดการหัวข้อ
- 🔤 การแปลภาษาโดยใช้ AI
- 🎯 การจัดลำดับด้วยการลากและวาง
- 🔌 รองรับมินิโปรแกรม
- ⚙️ เซิร์ฟเวอร์ MCP (Model Context Protocol)

5. **ประสบการณ์ผู้ใช้ที่ดียิ่งขึ้น**:

- 🖥️ รองรับข้ามแพลตฟอร์มสำหรับ Windows, Mac, และ Linux
- 📦 พร้อมใช้งานทันที - ไม่ต้องตั้งค่าสภาพแวดล้อม
- 🎨 ธีมสว่าง/มืดและหน้าต่างโปร่งใส
- 📝 การแสดงผล Markdown อย่างสมบูรณ์
- 🤲 แชร์เนื้อหาได้ง่าย

# 📝 Roadmap

เรากำลังพัฒนาและปรับปรุงฟีเจอร์ต่อไปนี้อย่างต่อเนื่อง:

1. 🎯 **ฟีเจอร์หลัก**

- ตัวช่วยเลือกเนื้อหาด้วยการเพิ่มประสิทธิภาพการเลือกอย่างชาญฉลาด
- การวิจัยเชิงลึกด้วยศักยภาพการค้นคว้าขั้นสูง
- ระบบหน่วยความจำพร้อมความตระหนักรู้บริบททั่วโลก
- การเตรียมเอกสารล่วงหน้าด้วยการจัดการเอกสารที่ดีขึ้น
- ตลาด MCP สำหรับระบบนิเวศ Model Context Protocol

2. 🗂 **การจัดการความรู้**

- โน้ตและชุดสะสม
- การแสดงผลแบบ Canvas แบบไดนามิก
- ความสามารถ OCR
- รองรับ TTS (Text-to-Speech)

3. 📱 **รองรับแพลตฟอร์ม**

- HarmonyOS Edition (PC)
- แอป Android (เฟส 1)
- แอป iOS (เฟส 1)
- รองรับ Multi-Window
- ฟังก์ชันปักหมุดหน้าต่าง

4. 🔌 **ฟีเจอร์ขั้นสูง**

- ระบบปลั๊กอิน
- ASR (Automatic Speech Recognition)
- ปรับโครงสร้างผู้ช่วยและการโต้ตอบหัวข้อ

ติดตามความคืบหน้าและมีส่วนร่วมได้ที่ [project board](https://github.com/orgs/CherryHQ/projects/7)

ต้องการมีอิทธิพลต่อแผนงานของเรา? เข้าร่วม [GitHub Discussions](https://github.com/CherryHQ/cherry-studio/discussions) เพื่อแบ่งปันแนวคิดและข้อเสนอแนะ!

# 🌈 Theme

- แกลเลอรีธีม: <https://cherrycss.com>
- ธีม Aero: <https://github.com/hakadao/CherryStudio-Aero>
- ธีม PaperMaterial: <https://github.com/rainoffallingstar/CherryStudio-PaperMaterial>
- Claude dynamic-style: <https://github.com/bjl101501/CherryStudio-Claudestyle-dynamic>
- ธีม Maple Neon: <https://github.com/BoningtonChen/CherryStudio_themes>

ยินดีต้อนรับ PR สำหรับธีมเพิ่มเติม

# 🤝 การมีส่วนร่วม

เรายินดีต้อนรับทุกการมีส่วนร่วมกับ Cherry Studio! นี่คือวิธีที่คุณสามารถร่วมพัฒนา:

1. **ร่วมพัฒนาซอร์สโค้ด**: สร้างฟีเจอร์ใหม่หรือปรับปรุงโค้ดที่มีอยู่
2. **แก้ไขบั๊ก**: ส่งการแก้ไขสำหรับข้อผิดพลาดที่คุณพบ
3. **ดูแล Issue**: ช่วยจัดการกับ Issue บน GitHub
4. **ออกแบบผลิตภัณฑ์**: เข้าร่วมในการอภิปรายการออกแบบ
5. **เขียนเอกสาร**: ปรับปรุงคู่มือผู้ใช้และคำแนะนำ
6. **มีส่วนร่วมกับชุมชน**: เข้าร่วมพูดคุยและช่วยเหลือผู้ใช้
7. **ส่งเสริมการใช้งาน**: เผยแพร่ Cherry Studio ให้เป็นที่รู้จัก

ดู [Branching Strategy](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/branching-strategy-en.md) สำหรับแนวทางการร่วมพัฒนา

## เริ่มต้นใช้งาน

1. **Fork Repository**: Fork และ clone ไปยังเครื่องของคุณ
2. **สร้าง Branch**: สำหรับการเปลี่ยนแปลงของคุณ
3. **ส่งการเปลี่ยนแปลง**: Commit และ push การเปลี่ยนแปลงของคุณ
4. **เปิด Pull Request**: อธิบายการเปลี่ยนแปลงและเหตุผล

สำหรับแนวทางละเอียดเพิ่มเติม โปรดดู [Contributing Guide](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/CONTRIBUTING.md) ของเรา

ขอขอบคุณสำหรับการสนับสนุนและการมีส่วนร่วม!

# 🔧 โปรแกรมร่วมสร้างสรรค์สำหรับนักพัฒนา

เรากำลังเปิดตัวโปรแกรมร่วมสร้างสรรค์ Cherry Studio เพื่อเสริมสร้างระบบนิเวศโอเพ่นซอร์สที่แข็งแกร่งและมีฟีดแบ็กเชิงบวก เราเชื่อว่าซอฟต์แวร์ที่ยอดเยี่ยมเกิดจากการร่วมมือกัน และทุก Pull Request ที่ถูกรวมคือชีวิตใหม่ของโปรเจกต์

เราขอเชิญคุณเข้าร่วมเป็นหนึ่งในผู้ร่วมพัฒนากับเราและร่วมกำหนดอนาคตของ Cherry Studio

## โปรแกรมรางวัลสำหรับผู้ร่วมพัฒนา

เพื่อขอบคุณผู้ร่วมพัฒนาหลักและสร้างวัฏจักรที่ดี เราจึงจัดตั้งแผนจูงใจระยะยาวดังนี้

**ช่วงติดตามรอบแรกของโปรแกรมนี้จะเป็นไตรมาส 3 ปี 2025 (กรกฎาคม, สิงหาคม, กันยายน) รางวัลสำหรับรอบนี้จะมอบให้ในวันที่ 1 ตุลาคม**

ภายในแต่ละรอบระยะเวลาการนับ (เช่น 1 กรกฎาคม ถึง 30 กันยายน สำหรับรอบแรก) นักพัฒนาคนใดที่มีส่วนร่วมมากกว่า **30 คอมมิทที่มีความหมาย** ในโปรเจ็กต์โอเพนซอร์สใด ๆ ของ Cherry Studio บน GitHub จะมีสิทธิได้รับสิทธิประโยชน์ดังต่อไปนี้:

- **สนับสนุนการสมัคร Cursor**: รับเครดิตหรือเงินคืนเป็นจำนวน **$70 USD** สำหรับการสมัคร [Cursor](https://cursor.sh/) ของคุณ เพื่อให้ AI เป็นคู่หูเขียนโค้ดที่มีประสิทธิภาพสูงสุดของคุณ
- **เข้าถึงโมเดลไม่จำกัด**: รับสิทธิ์ **เรียกใช้งาน API ไม่จำกัด** สำหรับโมเดล **DeepSeek** และ **Qwen**
- **เข้าถึงเทคโนโลยีล้ำสมัย**: รับสิทธิประโยชน์เป็นครั้งคราว เช่น การเข้าถึง API ของโมเดล **Claude**, **Gemini** และ **OpenAI** เพื่อให้คุณก้าวทันเทคโนโลยี

## เติบโตไปด้วยกัน & แผนในอนาคต

ชุมชนที่มีชีวิตชีวาคือแรงขับเคลื่อนสำคัญของโปรเจกต์โอเพนซอร์สที่ยั่งยืน เมื่อ Cherry Studio เติบโตขึ้น โปรแกรมรางวัลของเราก็จะขยายตามไปด้วย เรามุ่งมั่นในการปรับปรุงสิทธิประโยชน์อย่างต่อเนื่องให้สอดคล้องกับเครื่องมือและทรัพยากรที่ดีที่สุดในอุตสาหกรรม เพื่อให้ผู้ร่วมพัฒนาหลักได้รับการสนับสนุนที่มีความหมาย สร้างวงจรบวกที่นักพัฒนา ชุมชน และโปรเจกต์เติบโตไปด้วยกัน

**ต่อจากนี้ โปรเจกต์จะเปิดกว้างมากขึ้น เพื่อคืนคุณค่าให้กับชุมชนโอเพนซอร์สทั้งหมด**

## จะเริ่มต้นอย่างไร?

เราตั้งตารอ Pull Request แรกของคุณ!

คุณสามารถเริ่มต้นด้วยการสำรวจรีโพซิทอรีของเรา รับ `good first issue` หรือเสนอการปรับปรุงของคุณเอง ทุกคอมมิทคือข้อพิสูจน์ถึงจิตวิญญาณโอเพนซอร์ส

ขอขอบคุณสำหรับความสนใจและการมีส่วนร่วมของคุณ

มาสร้างสรรค์ไปด้วยกัน

# 🏢 Enterprise Edition

สืบเนื่องจาก Community Edition เราภูมิใจนำเสนอ **Cherry Studio Enterprise Edition** — แพลตฟอร์ม AI เพื่อการเพิ่มประสิทธิภาพและการบริหารจัดการสำหรับทีมและองค์กรยุคใหม่ ที่สามารถนำไปติดตั้งใช้งานแบบ private ได้

Enterprise Edition นี้ตอบโจทย์ปัญหาหลักด้านการทำงานร่วมกันของทีม ด้วยการรวมศูนย์การจัดการทรัพยากร AI, ความรู้ และข้อมูลต่าง ๆ ช่วยให้องค์กรเพิ่มประสิทธิภาพ ส่งเสริมนวัตกรรม และคงมาตรฐานความปลอดภัย โดยที่ยังคงควบคุมข้อมูลได้ 100% ในสภาพแวดล้อมที่ปลอดภัย

## ข้อดีหลัก

- **จัดการโมเดลแบบรวมศูนย์**: ผนวกและบริหาร LLM คลาวด์ (OpenAI, Anthropic, Google Gemini ฯลฯ) และโมเดลส่วนตัวในองค์กรได้จากศูนย์กลาง พนักงานใช้งานได้ทันทีโดยไม่ต้องตั้งค่าเอง
- **คลังความรู้ระดับองค์กร**: สร้าง จัดการ และแบ่งปันคลังความรู้ให้ทั้งทีม รักษาความรู้และความสอดคล้อง ให้สมาชิกทีมโต้ตอบกับ AI บนข้อมูลที่แม่นยำและเป็นหนึ่งเดียว
- **ควบคุมสิทธิ์ละเอียด**: จัดการบัญชีพนักงานและกำหนดสิทธิ์ตามบทบาท (role-based) สำหรับโมเดล คลังความรู้ และฟีเจอร์ต่าง ๆ ผ่านแอดมินหลังบ้านแบบศูนย์กลาง
- **ติดตั้งแบบ private 100%**: ติดตั้งระบบ backend ทั้งหมดบนเซิร์ฟเวอร์ในองค์กรหรือ private cloud เพื่อให้ข้อมูลคุณเป็นส่วนตัวและควบคุมได้ 100% ตอบโจทย์มาตรฐานความปลอดภัยสูงสุด
- **บริการ backend ที่เชื่อถือได้**: ให้บริการ API ที่เสถียร พร้อมระบบสำรองและกู้คืนข้อมูลระดับองค์กร เพื่อความต่อเนื่องทางธุรกิจ

## ✨ เดโมออนไลน์

> 🚧 **แจ้งเตือน Public Beta**
>
> Enterprise Edition ขณะนี้อยู่ในช่วง public beta ระยะแรก เรากำลังพัฒนาและปรับปรุงฟีเจอร์อย่างต่อเนื่อง อาจยังไม่สมบูรณ์หรือเสถียร หากคุณพบปัญหาหรือมีข้อเสนอแนะระหว่างทดลองใช้ เราจะขอบคุณเป็นอย่างยิ่งหากคุณติดต่อเราเพื่อให้ feedback ทางอีเมล

**🔗 [Cherry Studio Enterprise](https://www.cherry-ai.com/enterprise)**

## เปรียบเทียบเวอร์ชัน

| ฟีเจอร์           | Community Edition                          | Enterprise Edition                                                                                                                      |
| :---------------- | :----------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **โอเพนซอร์ส**   | ✅ ใช่                                     | ⭕️ เปิดซอร์สบางส่วนให้ลูกค้า                                                                                                      |
| **ค่าใช้จ่าย**    | ฟรีสำหรับใช้ส่วนตัว / ใบอนุญาตเชิงพาณิชย์ | ซื้อขาด / ค่าสมัครสมาชิก                                                                                                               |
| **แอดมิน backend**| —                                          | ● ศูนย์กลางการเข้าถึง **Model**<br>● บริหาร **พนักงาน**<br>● **คลังความรู้** ร่วม<br>● **ควบคุมสิทธิ์**<br>● **สำรองข้อมูล** |
| **เซิร์ฟเวอร์**   | —                                          | ✅ ติดตั้งแบบส่วนตัวโดยเฉพาะ                                                                                                         |

## ขอรับ Enterprise Edition

เราเชื่อว่า Enterprise Edition จะเป็นเครื่องยนต์เพิ่มประสิทธิภาพ AI ให้ทีมของคุณ หากคุณสนใจ Cherry Studio Enterprise Edition และต้องการข้อมูลเพิ่มเติม ขอใบเสนอราคา หรือขอเดโม กรุณาติดต่อเราได้ทุกเมื่อ

- **สำหรับธุรกิจและการสั่งซื้อ**:
  **📧 [bd@cherry-ai.com](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/mailto:bd@cherry-ai.com)**

# 🔗 โปรเจกต์ที่เกี่ยวข้อง

- [one-api](https://github.com/songquanpeng/one-api): ระบบบริหารและกระจาย API ของ LLM รองรับโมเดลหลักอย่าง OpenAI, Azure, Anthropic ฯลฯ มี API เดียว เหมาะกับการบริหารกุญแจและกระจายต่อ

- [ublacklist](https://github.com/iorate/ublacklist): บล็อกเว็บไซต์เฉพาะไม่ให้แสดงในผลการค้นหาของ Google

# 🚀 ผู้ร่วมพัฒนา

<a href="https://github.com/CherryHQ/cherry-studio/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=CherryHQ/cherry-studio" />
</a>
<br /><br />

# 📊 สถิติ GitHub

![Stats](https://repobeats.axiom.co/api/embed/a693f2e5f773eed620f70031e974552156c7f397.svg 'Repobeats analytics image')

# ⭐️ ประวัติดาว

<a href="https://www.star-history.com/#CherryHQ/cherry-studio&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
 </picture>
</a>

<!-- ลิงก์ & รูปภาพ -->

[deepwiki-shield]: https://img.shields.io/badge/Deepwiki-CherryHQ-0088CC?logo=data:image/svg+xml;base64,
[deepwiki-link]: https://deepwiki.com/CherryHQ/cherry-studio
[twitter-shield]: https://img.shields.io/badge/Twitter-CherryStudioApp-0088CC?logo=x
[twitter-link]: https://twitter.com/CherryStudioHQ
[discord-shield]: https://img.shields.io/badge/Discord-@CherryStudio-0088CC?logo=discord
[discord-link]: https://discord.gg/wez8HtpxqQ
[telegram-shield]: https://img.shields.io/badge/Telegram-@CherryStudioAI-0088CC?logo=telegram
[telegram-link]: https://t.me/CherryStudioAI

<!-- ลิงก์ & รูปภาพ -->
[github-release-shield]: https://img.shields.io/github/v/release/CherryHQ/cherry-studio?logo=github
[github-release-link]: https://github.com/CherryHQ/cherry-studio/releases
[github-nightly-shield]: https://img.shields.io/github/actions/workflow/status/CherryHQ/cherry-studio/nightly-build.yml?label=nightly%20build&logo=github
[github-nightly-link]: https://github.com/CherryHQ/cherry-studio/actions/workflows/nightly-build.yml
[github-contributors-shield]: https://img.shields.io/github/contributors/CherryHQ/cherry-studio?logo=github
[github-contributors-link]: https://github.com/CherryHQ/cherry-studio/graphs/contributors

<!-- ลิงก์และรูปภาพ -->

[license-shield]: https://img.shields.io/badge/License-AGPLv3-important.svg?logo=gnu
[license-link]: https://www.gnu.org/licenses/agpl-3.0
[commercial-shield]: https://img.shields.io/badge/License-Contact-white.svg?logoColor=white&logo=telegram&color=blue
[commercial-link]: mailto:license@cherry-ai.com?subject=Commercial%20License%20Inquiry
[sponsor-shield]: https://img.shields.io/badge/Sponsor-FF6699.svg?logo=githubsponsors&logoColor=white
[sponsor-link]: https://github.com/CherryHQ/cherry-studio/blob/main/docs/sponsor.md



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2025-07-25

---