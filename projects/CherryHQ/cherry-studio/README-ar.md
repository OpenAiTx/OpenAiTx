<div align="right" >
  <details>
    <summary >🌐 اللغة</summary>
    <div>
      <div align="right">
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=en">English</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-CN">简体中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-TW">繁體中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ja">日本語</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ko">한국어</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=hi">हिन्दी</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=th">ไทย</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fr">Français</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=de">Deutsch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=es">Español</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=it">Italiano</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ru">Русский</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pt">Português</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=nl">Nederlands</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pl">Polski</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ar">العربية</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fa">فارسی</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=tr">Türkçe</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=vi">Tiếng Việt</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=id">Bahasa Indonesia</a></p>
      </div>
    </div>
  </details>
</div>

<h1 align="center">
  <a href="https://github.com/CherryHQ/cherry-studio/releases">
    <img src="https://github.com/CherryHQ/cherry-studio/blob/main/build/icon.png?raw=true" width="150" height="150" alt="banner" /><br>
  </a>
</h1>

<p align="center">English | <a href="./docs/README.zh.md">中文</a> | <a href="https://cherry-ai.com">الموقع الرسمي</a> | <a href="https://docs.cherry-ai.com/cherry-studio-wen-dang/en-us">الوثائق</a> | <a href="./docs/dev.md">التطوير</a> | <a href="https://github.com/CherryHQ/cherry-studio/issues">ملاحظات</a><br></p>

<div align="center">
  
[![][deepwiki-shield]][deepwiki-link]
[![][twitter-shield]][twitter-link]
[![][discord-shield]][discord-link]
[![][telegram-shield]][telegram-link]

</div>
<div align="center">
  
[![][github-release-shield]][github-release-link]
[![][github-nightly-shield]][github-nightly-link]
[![][github-contributors-shield]][github-contributors-link]
[![][license-shield]][license-link]
[![][commercial-shield]][commercial-link]
[![][sponsor-shield]][sponsor-link]

</div>

<div align="center">
 <a href="https://hellogithub.com/repository/1605492e1e2a4df3be07abfa4578dd37" target="_blank" style="text-decoration: none"><img src="https://api.hellogithub.com/v1/widgets/recommend.svg?rid=1605492e1e2a4df3be07abfa4578dd37" alt="Featured｜HelloGitHub"  width="220" height="55" /></a>
 <a href="https://trendshift.io/repositories/11772" target="_blank" style="text-decoration: none"><img src="https://trendshift.io/api/badge/repositories/11772" alt="kangfenmao%2Fcherry-studio | Trendshift" width="220" height="55" /></a>
 <a href="https://www.producthunt.com/posts/cherry-studio?embed=true&utm_source=badge-featured&utm_medium=badge&utm_souce=badge-cherry&#0045;studio" target="_blank"><img src="https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id=496640&theme=light" alt="Cherry&#0032;Studio - AI&#0032;Chatbots&#0044;&#0032;AI&#0032;Desktop&#0032;Client | Product Hunt" width="220" height="55" /></a>
</div>

# 🍒 Cherry Studio

Cherry Studio هو عميل سطح مكتب يدعم عدة مزودي LLM، متوفر على ويندوز وماك ولينكس.

👏 انضم إلى [مجموعة تيليجرام](https://t.me/CherryStudioAI)｜[ديسكورد](https://discord.gg/wez8HtpxqQ) | [مجموعة QQ(575014769)](https://qm.qq.com/q/lo0D4qVZKi)

❤️ أعجبتك Cherry Studio؟ ضع نجمة 🌟 أو [ادعمنا](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/sponsor.md) لدعم التطوير!

# 🌠 لقطة شاشة

![](https://github.com/user-attachments/assets/36dddb2c-e0fb-4a5f-9411-91447bab6e18)

![](https://github.com/user-attachments/assets/f549e8a0-2385-40b4-b52b-2039e39f2930)

![](https://github.com/user-attachments/assets/58e0237c-4d36-40de-b428-53051d982026)

# 🌟 الميزات الرئيسية

1. **دعم متنوع لمزودي LLM**:

- ☁️ خدمات سحابية رئيسية: OpenAI, Gemini, Anthropic، وأكثر
- 🔗 تكامل مع خدمات الويب للذكاء الاصطناعي: Claude, Peplexity, Poe وغيرها
- 💻 دعم النماذج المحلية مع Ollama, LM Studio

2. **المساعدات الذكية والمحادثات**:

- 📚 أكثر من 300 مساعد ذكي جاهز
- 🤖 إنشاء مساعدين مخصصين
- 💬 محادثات متعددة النماذج في نفس الوقت

3. **معالجة المستندات والبيانات**:

- 📄 يدعم النصوص، الصور، أوفيس، PDF والمزيد
- ☁️ إدارة ونسخ احتياطي للملفات عبر WebDAV
- 📊 تصور المخططات باستخدام Mermaid
- 💻 تمييز بناء الجمل البرمجية

4. **تكامل الأدوات العملية**:

- 🔍 وظيفة البحث الشامل
- 📝 نظام إدارة المواضيع
- 🔤 ترجمة مدعومة بالذكاء الاصطناعي
- 🎯 ترتيب بالسحب والإفلات
- 🔌 دعم البرامج المصغرة
- ⚙️ خادم MCP (بروتوكول سياق النماذج)

5. **تجربة مستخدم محسّنة**:

- 🖥️ دعم متعدد المنصات لنظام Windows، Mac و Linux
- 📦 جاهز للاستخدام - لا حاجة لإعداد بيئة العمل
- 🎨 سمات فاتحة/داكنة ونافذة شفافة
- 📝 عرض كامل لـ Markdown
- 🤲 مشاركة المحتوى بسهولة

# 📝 خارطة الطريق

نعمل بنشاط على الميزات والتحسينات التالية:

1. 🎯 **الميزات الأساسية**

- مساعد التحديد مع تعزيز ذكي لاختيار المحتوى
- بحث معمق بإمكانات بحث متقدمة
- نظام ذاكرة مع وعي بالسياق العالمي
- معالجة مسبقة للمستندات مع تحسين التعامل مع الملفات
- سوق MCP لنظام بروتوكول سياق النماذج

2. 🗂 **إدارة المعرفة**

- ملاحظات وتجميعات
- تصور ديناميكي للرسم البياني
- قدرات التعرف البصري على الحروف (OCR)
- دعم TTS (النص إلى كلام)

3. 📱 **دعم المنصات**

- إصدار HarmonyOS (للكمبيوتر)
- تطبيق Android (المرحلة 1)
- تطبيق iOS (المرحلة 1)
- دعم النوافذ المتعددة
- وظيفة تثبيت النافذة

4. 🔌 **ميزات متقدمة**

- نظام الإضافات
- ASR (التعرف التلقائي على الكلام)
- إعادة هيكلة التفاعل بين المساعد والمواضيع

تابع تقدمنا وساهم عبر [لوحة المشروع](https://github.com/orgs/CherryHQ/projects/7).

ترغب في التأثير على خارطة الطريق؟ انضم إلى [مناقشات GitHub](https://github.com/CherryHQ/cherry-studio/discussions) وشارك أفكارك وملاحظاتك!

# 🌈 السِمة

- معرض السمات: <https://cherrycss.com>
- سِمة Aero: <https://github.com/hakadao/CherryStudio-Aero>
- سِمة PaperMaterial: <https://github.com/rainoffallingstar/CherryStudio-PaperMaterial>
- سِمة Claude الديناميكية: <https://github.com/bjl101501/CherryStudio-Claudestyle-dynamic>
- سِمة Maple Neon: <https://github.com/BoningtonChen/CherryStudio_themes>

نرحب بطلبات السحب لمزيد من السمات

# 🤝 المساهمة

نرحب بمساهماتكم في Cherry Studio! إليك بعض الطرق التي يمكنك المساهمة بها:

1. **المساهمة بالكود**: تطوير ميزات جديدة أو تحسين الكود الحالي.
2. **إصلاح الأخطاء**: تقديم إصلاحات لأي أخطاء تجدها.
3. **إدارة القضايا**: المساعدة في إدارة قضايا GitHub.
4. **تصميم المنتج**: المشاركة في مناقشات التصميم.
5. **كتابة التوثيقات**: تحسين أدلة المستخدم والكتيبات.
6. **مشاركة المجتمع**: الانضمام للنقاشات ومساعدة المستخدمين.
7. **الترويج للاستخدام**: نشر الوعي حول Cherry Studio.

راجع [استراتيجية التفرع](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/branching-strategy-en.md) لإرشادات المساهمة

## الشروع في العمل

1. **تفرع المستودع**: أنشئ تفرعًا واستنسخه على جهازك المحلي.
2. **أنشئ فرعًا**: لتغييراتك.
3. **قدّم التغييرات**: قم بحفظ ودفع التغييرات الخاصة بك.
4. **افتح طلب سحب**: وصف تغييراتك وأسبابها.

لمزيد من الإرشادات التفصيلية، يرجى الرجوع إلى [دليل المساهمة](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/CONTRIBUTING.md).

شكرًا لدعمكم ومساهماتكم!

# 🔧 برنامج التعاون المشترك للمطورين

نطلق برنامج Cherry Studio للتعاون المشترك بين المطورين لتعزيز حلقة تغذية راجعة صحية وإيجابية ضمن نظام المصادر المفتوحة. نحن نؤمن أن البرمجيات الرائعة تُبنى بشكل جماعي، وكل طلب سحب مدمج يمنح المشروع حياة جديدة.

ندعوكم بصدق للانضمام إلى صفوف المساهمين معنا وتشكيل مستقبل Cherry Studio سويًا.

## برنامج مكافآت المساهمين

لرد الجميل لمساهمينا الأساسيين وخلق دورة فضيلة، أنشأنا خطة حوافز طويلة الأمد على النحو التالي.

**ستكون فترة المتابعة الأولى لهذا البرنامج هي الربع الثالث من عام 2025 (يوليو، أغسطس، سبتمبر). سيتم توزيع المكافآت لهذه الدورة في 1 أكتوبر.**
خلال أي فترة تتبع (على سبيل المثال، من 1 يوليو إلى 30 سبتمبر للدورة الأولى)، أي مطور يساهم بأكثر من **30 التزامًا ذا مغزى** في أي من مشاريع Cherry Studio مفتوحة المصدر على GitHub سيكون مؤهلاً للحصول على المزايا التالية:

- **رعاية اشتراك Cursor**: احصل على رصيد أو تعويض بقيمة **70 دولار أمريكي** لاشتراكك في [Cursor](https://cursor.sh/)، ليصبح الذكاء الاصطناعي شريكك الأكثر كفاءة في البرمجة.
- **وصول غير محدود للنماذج**: احصل على مكالمات API **غير محدودة** لنماذج **DeepSeek** و **Qwen**.
- **وصول إلى أحدث التقنيات**: استمتع بمزايا عرضية مثل وصول API إلى نماذج مثل **Claude** و **Gemini** و **OpenAI**، لتبقى في طليعة التكنولوجيا.

## النمو معًا والخطط المستقبلية

المجتمع الحيوي هو القوة الدافعة وراء أي مشروع مفتوح المصدر مستدام. مع نمو Cherry Studio، سينمو أيضًا برنامج المكافآت لدينا. نحن ملتزمون بمواصلة مواءمة مزايانا مع أفضل الأدوات والموارد في الصناعة. هذا يضمن حصول المساهمين الأساسيين لدينا على دعم فعلي، مما يخلق دورة إيجابية ينمو فيها المطورون والمجتمع والمشروع معًا.

**مستقبلاً، سيتبنى المشروع موقفًا أكثر انفتاحًا لإفادة مجتمع المصادر المفتوحة بأكمله.**

## كيف تبدأ؟

نحن نتطلع إلى أول طلب سحب لك!

يمكنك البدء باستكشاف مستودعاتنا، أو اختيار `good first issue`، أو اقتراح تحسيناتك الخاصة. كل التزام هو شهادة على روح المصدر المفتوح.

شكرًا لاهتمامك ومساهماتك.

لنُنشئ معًا.

# 🏢 إصدار المؤسسات

استنادًا إلى إصدار المجتمع، نحن فخورون بتقديم **إصدار المؤسسات من Cherry Studio**—منصة إنتاجية وإدارة ذكاء اصطناعي قابلة للنشر الخاص، مصممة للفرق والمؤسسات الحديثة.

يعالج إصدار المؤسسات التحديات الأساسية في التعاون بين الفرق من خلال مركزية إدارة موارد الذكاء الاصطناعي والمعرفة والبيانات. فهو يمكّن المؤسسات من تعزيز الكفاءة، ودعم الابتكار، وضمان الالتزام، مع الحفاظ على سيطرة كاملة على بياناتهم في بيئة آمنة.

## المزايا الأساسية

- **إدارة النماذج الموحدة**: دمج وإدارة نماذج LLM السحابية (مثل OpenAI، Anthropic، Google Gemini) والنماذج الخاصة المحلية مركزيًا. يمكن للموظفين استخدامها مباشرة دون إعدادات فردية.
- **قاعدة معرفة بمستوى المؤسسات**: أنشئ وأدر وشارك قواعد المعرفة على مستوى الفريق. يضمن ذلك الاحتفاظ بالمعرفة والاتساق، مما يمكّن أعضاء الفريق من التفاعل مع الذكاء الاصطناعي بناءً على معلومات موحدة ودقيقة.
- **تحكم دقيق في الوصول**: إدارة حسابات الموظفين بسهولة وتعيين أذونات قائمة على الدور لنماذج وقواعد المعرفة والميزات المختلفة من خلال واجهة إدارة موحدة.
- **نشر خاص بالكامل**: نشر الخدمة الخلفية كاملة على خوادمك المحلية أو السحابة الخاصة بك، لضمان بقاء بياناتك خاصة وتحت سيطرتك الكاملة لتلبية أعلى معايير الأمان والامتثال.
- **خدمات خلفية موثوقة**: توفر خدمات API مستقرة وآليات نسخ احتياطي واستعادة بيانات بمستوى المؤسسات لضمان استمرارية الأعمال.

## ✨ عرض توضيحي عبر الإنترنت

> 🚧 **إشعار بيتا العامة**
>
> إصدار المؤسسات لا يزال في مرحلة البيتا العامة المبكرة، ونحن نعمل بنشاط على تكرار وتحسين ميزاته. نحن ندرك أنه قد لا يكون مستقرًا تمامًا بعد. إذا واجهت أي مشاكل أو لديك اقتراحات قيمة أثناء تجربتك، سنكون ممتنين جدًا إذا تواصلت معنا عبر البريد الإلكتروني لإبداء ملاحظاتك.

**🔗 [Cherry Studio Enterprise](https://www.cherry-ai.com/enterprise)**

## مقارنة الإصدارات

| الميزة            | إصدار المجتمع                            | إصدار المؤسسات                                                                                                                    |
| :---------------- | :--------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------- |
| **مفتوح المصدر**  | ✅ نعم                                    | ⭕️ متوفر جزئيًا للعملاء                                                                                                                  |
| **التكلفة**       | مجاني للاستخدام الشخصي / ترخيص تجاري     | شراء كامل / رسوم اشتراك                                                                                                          |
| **واجهة الإدارة** | —                                        | ● وصول **النماذج** مركزي<br>● إدارة **الموظفين**<br>● **قاعدة معرفة** مشتركة<br>● **تحكم** في الوصول<br>● **نسخ** بيانات احتياطي   |
| **الخادم**        | —                                        | ✅ نشر خاص مخصص                                                                                                                  |

## الحصول على إصدار المؤسسات

نحن نؤمن بأن إصدار المؤسسات سيصبح محرك إنتاجية الذكاء الاصطناعي لفريقك. إذا كنت مهتمًا بإصدار المؤسسات من Cherry Studio وترغب في معرفة المزيد أو طلب عرض أسعار أو جدولة عرض توضيحي، لا تتردد في التواصل معنا.

- **للاستفسارات التجارية والشراء**:
  **📧 [bd@cherry-ai.com](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/mailto:bd@cherry-ai.com)**

# 🔗 مشاريع ذات صلة

- [one-api](https://github.com/songquanpeng/one-api): نظام إدارة وتوزيع LLM API يدعم النماذج الرئيسية مثل OpenAI وAzure وAnthropic. يوفر واجهة API موحدة، مناسب لإدارة المفاتيح والتوزيع الثانوي.

- [ublacklist](https://github.com/iorate/ublacklist): يحجب مواقع محددة من الظهور في نتائج بحث Google

# 🚀 المساهمون

<a href="https://github.com/CherryHQ/cherry-studio/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=CherryHQ/cherry-studio" />
</a>
<br /><br />

# 📊 إحصائيات GitHub

![Stats](https://repobeats.axiom.co/api/embed/a693f2e5f773eed620f70031e974552156c7f397.svg 'Repobeats analytics image')

# ⭐️ سجل النجوم

<a href="https://www.star-history.com/#CherryHQ/cherry-studio&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
 </picture>
</a>

<!-- روابط وصور -->

[deepwiki-shield]: https://img.shields.io/badge/Deepwiki-CherryHQ-0088CC?logo=data:image/svg+xml;base64,
[deepwiki-link]: https://deepwiki.com/CherryHQ/cherry-studio
[twitter-shield]: https://img.shields.io/badge/Twitter-CherryStudioApp-0088CC?logo=x
[twitter-link]: https://twitter.com/CherryStudioHQ
[discord-shield]: https://img.shields.io/badge/Discord-@CherryStudio-0088CC?logo=discord
[discord-link]: https://discord.gg/wez8HtpxqQ
[telegram-shield]: https://img.shields.io/badge/Telegram-@CherryStudioAI-0088CC?logo=telegram
[telegram-link]: https://t.me/CherryStudioAI

<!-- روابط وصور -->

[github-release-shield]: https://img.shields.io/github/v/release/CherryHQ/cherry-studio?logo=github
[github-release-link]: https://github.com/CherryHQ/cherry-studio/releases
[github-nightly-shield]: https://img.shields.io/github/actions/workflow/status/CherryHQ/cherry-studio/nightly-build.yml?label=nightly%20build&logo=github
[github-nightly-link]: https://github.com/CherryHQ/cherry-studio/actions/workflows/nightly-build.yml
[github-contributors-shield]: https://img.shields.io/github/contributors/CherryHQ/cherry-studio?logo=github
[github-contributors-link]: https://github.com/CherryHQ/cherry-studio/graphs/contributors

<!-- الروابط والصور -->

[license-shield]: https://img.shields.io/badge/License-AGPLv3-important.svg?logo=gnu
[license-link]: https://www.gnu.org/licenses/agpl-3.0
[commercial-shield]: https://img.shields.io/badge/License-Contact-white.svg?logoColor=white&logo=telegram&color=blue
[commercial-link]: mailto:license@cherry-ai.com?subject=Commercial%20License%20Inquiry
[sponsor-shield]: https://img.shields.io/badge/Sponsor-FF6699.svg?logo=githubsponsors&logoColor=white
[sponsor-link]: https://github.com/CherryHQ/cherry-studio/blob/main/docs/sponsor.md



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2025-07-22

---