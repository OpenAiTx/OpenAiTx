<div align="right" >
  <details>
    <summary >🌐 Sprache</summary>
    <div>
      <div align="right">
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=en">Englisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-CN">简体中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-TW">繁體中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ja">日本語</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ko">한국어</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=hi">हिन्दी</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=th">ไทย</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fr">Französisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=de">Deutsch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=es">Spanisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=it">Italienisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ru">Russisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pt">Portugiesisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=nl">Niederländisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pl">Polnisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ar">العربية</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fa">فارسی</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=tr">Türkisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=vi">Vietnamesisch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=id">Bahasa Indonesia</a></p>
      </div>
    </div>
  </details>
</div>

<h1 align="center">
  <a href="https://github.com/CherryHQ/cherry-studio/releases">
    <img src="https://github.com/CherryHQ/cherry-studio/blob/main/build/icon.png?raw=true" width="150" height="150" alt="banner" /><br>
  </a>
</h1>

<p align="center">Englisch | <a href="./docs/README.zh.md">中文</a> | <a href="https://cherry-ai.com">Offizielle Seite</a> | <a href="https://docs.cherry-ai.com/cherry-studio-wen-dang/en-us">Dokumente</a> | <a href="./docs/dev.md">Entwicklung</a> | <a href="https://github.com/CherryHQ/cherry-studio/issues">Feedback</a><br></p>

<div align="center">
  
[![][deepwiki-shield]][deepwiki-link]
[![][twitter-shield]][twitter-link]
[![][discord-shield]][discord-link]
[![][telegram-shield]][telegram-link]

</div>
<div align="center">
  
[![][github-release-shield]][github-release-link]
[![][github-nightly-shield]][github-nightly-link]
[![][github-contributors-shield]][github-contributors-link]
[![][license-shield]][license-link]
[![][commercial-shield]][commercial-link]
[![][sponsor-shield]][sponsor-link]

</div>

<div align="center">
 <a href="https://hellogithub.com/repository/1605492e1e2a4df3be07abfa4578dd37" target="_blank" style="text-decoration: none"><img src="https://api.hellogithub.com/v1/widgets/recommend.svg?rid=1605492e1e2a4df3be07abfa4578dd37" alt="Vorgestellt｜HelloGitHub"  width="220" height="55" /></a>
 <a href="https://trendshift.io/repositories/11772" target="_blank" style="text-decoration: none"><img src="https://trendshift.io/api/badge/repositories/11772" alt="kangfenmao%2Fcherry-studio | Trendshift" width="220" height="55" /></a>
 <a href="https://www.producthunt.com/posts/cherry-studio?embed=true&utm_source=badge-featured&utm_medium=badge&utm_souce=badge-cherry&#0045;studio" target="_blank"><img src="https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id=496640&theme=light" alt="Cherry&#0032;Studio - AI&#0032;Chatbots&#0044;&#0032;AI&#0032;Desktop&#0032;Client | Product Hunt" width="220" height="55" /></a>
</div>

# 🍒 Cherry Studio

Cherry Studio ist ein Desktop-Client, der mehrere LLM-Anbieter unterstützt und für Windows, Mac und Linux verfügbar ist.

👏 Tritt der [Telegram-Gruppe](https://t.me/CherryStudioAI)｜[Discord](https://discord.gg/wez8HtpxqQ) | [QQ-Gruppe(575014769)](https://qm.qq.com/q/lo0D4qVZKi) bei

❤️ Gefällt dir Cherry Studio? Gib uns einen Stern 🌟 oder [unterstütze](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/sponsor.md) die Entwicklung als Sponsor!

# 🌠 Screenshot

![](https://github.com/user-attachments/assets/36dddb2c-e0fb-4a5f-9411-91447bab6e18)

![](https://github.com/user-attachments/assets/f549e8a0-2385-40b4-b52b-2039e39f2930)

![](https://github.com/user-attachments/assets/58e0237c-4d36-40de-b428-53051d982026)

# 🌟 Hauptfunktionen

1. **Vielfältige LLM-Anbieter-Unterstützung**:

- ☁️ Wichtige LLM-Cloud-Dienste: OpenAI, Gemini, Anthropic und mehr
- 🔗 Integration von KI-Webdiensten: Claude, Peplexity, Poe und weitere
- 💻 Lokale Modellunterstützung mit Ollama, LM Studio

2. **KI-Assistenten & Konversationen**:

- 📚 Über 300 vorgefertigte KI-Assistenten
- 🤖 Eigene Assistenten erstellen
- 💬 Gleichzeitige Konversationen mit mehreren Modellen

3. **Dokumenten- & Datenverarbeitung**:

- 📄 Unterstützt Text, Bilder, Office, PDF und mehr
- ☁️ WebDAV-Dateimanagement und Backup
- 📊 Mermaid-Diagrammvisualisierung
- 💻 Code-Syntax-Hervorhebung

4. **Integration praktischer Werkzeuge**:

- 🔍 Globale Suchfunktion
- 📝 Themenverwaltungssystem
- 🔤 KI-gestützte Übersetzung
- 🎯 Drag-and-drop Sortierung
- 🔌 Mini-Programm-Unterstützung
- ⚙️ MCP(Model Context Protocol) Server

5. **Verbesserte Benutzererfahrung**:

- 🖥️ Plattformübergreifende Unterstützung für Windows, Mac und Linux
- 📦 Sofort einsatzbereit – keine Umgebungseinrichtung erforderlich
- 🎨 Helle/dunkle Designs und transparentes Fenster
- 📝 Vollständige Markdown-Darstellung
- 🤲 Einfaches Teilen von Inhalten

# 📝 Roadmap

Wir arbeiten aktiv an den folgenden Funktionen und Verbesserungen:

1. 🎯 **Kernfunktionen**

- Auswahlassistent mit intelligenter Inhaltsauswahlverbesserung
- Tiefenrecherche mit fortschrittlichen Forschungsmöglichkeiten
- Speichersystem mit globalem Kontextbewusstsein
- Dokumentvorverarbeitung mit verbesserter Dokumentenhandhabung
- MCP-Marktplatz für das Model Context Protocol-Ökosystem

2. 🗂 **Wissensmanagement**

- Notizen und Sammlungen
- Dynamische Canvas-Visualisierung
- OCR-Fähigkeiten
- TTS (Text-zu-Sprache) Unterstützung

3. 📱 **Plattformunterstützung**

- HarmonyOS Edition (PC)
- Android-App (Phase 1)
- iOS-App (Phase 1)
- Multi-Fenster-Unterstützung
- Fenster-Anheften-Funktion

4. 🔌 **Erweiterte Funktionen**

- Plugin-System
- ASR (Automatische Spracherkennung)
- Assistenten- und Themeninteraktions-Refaktorierung

Verfolge unseren Fortschritt und trage bei auf unserem [Projektboard](https://github.com/orgs/CherryHQ/projects/7).

Möchtest du unsere Roadmap beeinflussen? Nimm an unseren [GitHub Discussions](https://github.com/CherryHQ/cherry-studio/discussions) teil, um deine Ideen und dein Feedback zu teilen!

# 🌈 Theme

- Theme Galerie: <https://cherrycss.com>
- Aero Theme: <https://github.com/hakadao/CherryStudio-Aero>
- PaperMaterial Theme: <https://github.com/rainoffallingstar/CherryStudio-PaperMaterial>
- Claude dynamic-style: <https://github.com/bjl101501/CherryStudio-Claudestyle-dynamic>
- Maple Neon Theme: <https://github.com/BoningtonChen/CherryStudio_themes>

PRs für weitere Themes sind willkommen

# 🤝 Beitrag leisten

Wir freuen uns über Beiträge zu Cherry Studio! Hier sind einige Möglichkeiten, wie du mitmachen kannst:

1. **Code beitragen**: Entwickle neue Funktionen oder optimiere bestehenden Code.
2. **Fehler beheben**: Reiche Korrekturen für gefundene Fehler ein.
3. **Issues pflegen**: Hilf bei der Verwaltung von GitHub-Issues.
4. **Produktdesign**: Nimm an Design-Diskussionen teil.
5. **Dokumentation schreiben**: Verbessere Benutzerhandbücher und Leitfäden.
6. **Community-Engagement**: Nimm an Diskussionen teil und unterstütze Nutzer.
7. **Nutzung fördern**: Verbreite Cherry Studio weiter.

Siehe die [Branching-Strategie](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/branching-strategy-en.md) für Beitragsrichtlinien

## Erste Schritte

1. **Repository forken**: Forke und klone es auf deinen lokalen Rechner.
2. **Branch erstellen**: Für deine Änderungen.
3. **Änderungen einreichen**: Änderungen committen und pushen.
4. **Pull Request öffnen**: Beschreibe deine Änderungen und Gründe.

Für detailliertere Richtlinien lies bitte unseren [Contributing Guide](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/CONTRIBUTING.md).

Danke für deine Unterstützung und Beiträge!

# 🔧 Entwickler Co-Creation-Programm

Wir starten das Cherry Studio Entwickler Co-Creation-Programm, um einen gesunden und positiven Feedback-Kreislauf im Open-Source-Ökosystem zu fördern. Wir glauben, dass großartige Software gemeinsam entsteht und jeder gemergte Pull Request dem Projekt neues Leben einhaucht.

Wir laden dich herzlich ein, Teil unserer Contributor-Community zu werden und gemeinsam mit uns die Zukunft von Cherry Studio zu gestalten.

## Contributor-Belohnungsprogramm

Um unseren Kernbeiträgern etwas zurückzugeben und einen positiven Kreislauf zu schaffen, haben wir den folgenden langfristigen Anreizplan aufgestellt.

**Die erste Nachverfolgungsperiode für dieses Programm ist Q3 2025 (Juli, August, September). Die Belohnungen für diesen Zyklus werden am 1. Oktober verteilt.**
Innerhalb eines beliebigen Tracking-Zeitraums (z. B. 1. Juli bis 30. September für den ersten Zyklus) ist jeder Entwickler, der mehr als **30 sinnvolle Commits** zu einem der Open-Source-Projekte von Cherry Studio auf GitHub beiträgt, für folgende Vorteile berechtigt:

- **Cursor-Abonnement-Sponsoring**: Erhalte einen **$70 USD**-Gutschein oder eine Rückerstattung für dein [Cursor](https://cursor.sh/)-Abonnement und mache KI zu deinem effizientesten Coding-Partner.
- **Unbegrenzter Modellzugriff**: Erhalte **unbegrenzte** API-Aufrufe für die **DeepSeek**- und **Qwen**-Modelle.
- **Zugang zu modernster Technologie**: Profitiere gelegentlich von Vorteilen wie API-Zugriff auf Modelle wie **Claude**, **Gemini** und **OpenAI** und bleibe an der technologischen Spitze.

## Gemeinsam wachsen & Zukunftspläne

Eine lebendige Community ist die treibende Kraft hinter jedem nachhaltigen Open-Source-Projekt. Während Cherry Studio wächst, wird auch unser Belohnungsprogramm wachsen. Wir verpflichten uns, unsere Vorteile kontinuierlich an die besten Tools und Ressourcen der Branche anzupassen. So stellen wir sicher, dass unsere Kernbeiträger sinnvolle Unterstützung erhalten und einen positiven Kreislauf schaffen, in dem Entwickler, Community und Projekt gemeinsam wachsen.

**Zukünftig wird das Projekt zudem eine zunehmend offene Haltung einnehmen, um der gesamten Open-Source-Community etwas zurückzugeben.**

## Wie starten?

Wir freuen uns auf deinen ersten Pull Request!

Du kannst beginnen, indem du unsere Repositories erkundest, ein `good first issue` aufgreifst oder eigene Verbesserungen vorschlägst. Jeder Commit ist ein Ausdruck des Open-Source-Gedankens.

Danke für dein Interesse und deine Beiträge.

Lass uns gemeinsam etwas aufbauen.

# 🏢 Enterprise Edition

Basierend auf der Community Edition stellen wir stolz die **Cherry Studio Enterprise Edition** vor—eine privat einsetzbare KI-Produktivitäts- und Managementplattform für moderne Teams und Unternehmen.

Die Enterprise Edition adressiert zentrale Herausforderungen der Teamzusammenarbeit, indem sie das Management von KI-Ressourcen, Wissen und Daten zentralisiert. Sie ermöglicht es Organisationen, Effizienz zu steigern, Innovation zu fördern und Compliance sicherzustellen—bei 100 % Kontrolle über die eigenen Daten in einer sicheren Umgebung.

## Zentrale Vorteile

- **Zentrales Modellmanagement**: Integration und Verwaltung verschiedenster Cloud-basierter LLMs (z. B. OpenAI, Anthropic, Google Gemini) sowie lokal bereitgestellter privater Modelle an einem Ort. Mitarbeitende können sie sofort nutzen, ohne individuelle Konfiguration.
- **Wissensdatenbank auf Unternehmensniveau**: Aufbau, Verwaltung und Teilen teamweiter Wissensdatenbanken. Sichert Wissen und Konsistenz und ermöglicht KI-Interaktion auf Basis einheitlicher, korrekter Informationen.
- **Feingranulares Zugriffsmanagement**: Verwaltung von Mitarbeitenden und Zuweisung rollenbasierter Berechtigungen für Modelle, Wissensdatenbanken und Funktionen über ein einheitliches Admin-Backend.
- **Vollständige private Bereitstellung**: Installation des gesamten Backend-Services auf eigenen Servern oder privater Cloud, damit Ihre Daten zu 100 % privat und unter Kontrolle bleiben—auch für höchste Sicherheits- und Compliance-Anforderungen.
- **Zuverlässige Backend-Services**: Bietet stabile API-Dienste sowie Backup- und Wiederherstellungsmechanismen auf Unternehmensniveau für kontinuierlichen Geschäftsbetrieb.

## ✨ Online-Demo

> 🚧 **Hinweis zur öffentlichen Beta**
>
> Die Enterprise Edition befindet sich derzeit in einer frühen öffentlichen Betaphase. Wir entwickeln und optimieren die Funktionen aktiv weiter. Es kann sein, dass die Stabilität noch nicht perfekt ist. Solltest du Probleme oder wertvolle Verbesserungsvorschläge haben, freuen wir uns sehr über Feedback per E-Mail.

**🔗 [Cherry Studio Enterprise](https://www.cherry-ai.com/enterprise)**

## Versionsvergleich

| Funktion          | Community Edition                          | Enterprise Edition                                                                                                                      |
| :---------------- | :----------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **Open Source**   | ✅ Ja                                      | ⭕️ Teilweise für Kunden veröffentlicht                                                                                                  |
| **Kosten**        | Kostenlos für Privatgebrauch / kommerzielle Lizenz | Kauf / Abonnement                                                                                                                |
| **Admin-Backend** | —                                          | ● Zentrale **Modell**-Verwaltung<br>● **Mitarbeiter**-Management<br>● Geteilte **Wissensdatenbank**<br>● **Zugriffs**-Kontrolle<br>● **Daten**-Backup |
| **Server**        | —                                          | ✅ Dedizierte private Bereitstellung                                                                                                  |

## Enterprise Edition erhalten

Wir sind überzeugt, dass die Enterprise Edition zum Produktivitätsmotor deines Teams wird. Wenn du an der Cherry Studio Enterprise Edition interessiert bist und mehr erfahren, ein Angebot anfordern oder eine Demo vereinbaren möchtest, kontaktiere uns gerne.

- **Für Geschäftsanfragen & Kauf**:
  **📧 [bd@cherry-ai.com](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/mailto:bd@cherry-ai.com)**

# 🔗 Verwandte Projekte

- [one-api](https://github.com/songquanpeng/one-api): LLM-API-Management- und Verteilungssystem, das gängige Modelle wie OpenAI, Azure und Anthropic unterstützt. Einheitliches API-Interface—ideal für Key-Management und Zweitverteilung.

- [ublacklist](https://github.com/iorate/ublacklist): Blockiert bestimmte Seiten in den Google-Suchergebnissen.

# 🚀 Mitwirkende

<a href="https://github.com/CherryHQ/cherry-studio/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=CherryHQ/cherry-studio" />
</a>
<br /><br />

# 📊 GitHub-Statistiken

![Stats](https://repobeats.axiom.co/api/embed/a693f2e5f773eed620f70031e974552156c7f397.svg 'Repobeats analytics image')

# ⭐️ Stern-Verlauf

<a href="https://www.star-history.com/#CherryHQ/cherry-studio&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
 </picture>
</a>

<!-- Links & Bilder -->

[deepwiki-shield]: https://img.shields.io/badge/Deepwiki-CherryHQ-0088CC?logo=data:image/svg+xml;base64,
[deepwiki-link]: https://deepwiki.com/CherryHQ/cherry-studio
[twitter-shield]: https://img.shields.io/badge/Twitter-CherryStudioApp-0088CC?logo=x
[twitter-link]: https://twitter.com/CherryStudioHQ
[discord-shield]: https://img.shields.io/badge/Discord-@CherryStudio-0088CC?logo=discord
[discord-link]: https://discord.gg/wez8HtpxqQ
[telegram-shield]: https://img.shields.io/badge/Telegram-@CherryStudioAI-0088CC?logo=telegram
[telegram-link]: https://t.me/CherryStudioAI

<!-- Links & Bilder -->

[github-release-shield]: https://img.shields.io/github/v/release/CherryHQ/cherry-studio?logo=github
[github-release-link]: https://github.com/CherryHQ/cherry-studio/releases
[github-nightly-shield]: https://img.shields.io/github/actions/workflow/status/CherryHQ/cherry-studio/nightly-build.yml?label=nightly%20build&logo=github
[github-nightly-link]: https://github.com/CherryHQ/cherry-studio/actions/workflows/nightly-build.yml
[github-contributors-shield]: https://img.shields.io/github/contributors/CherryHQ/cherry-studio?logo=github
[github-contributors-link]: https://github.com/CherryHQ/cherry-studio/graphs/contributors

<!-- Links & Bilder -->

[license-shield]: https://img.shields.io/badge/License-AGPLv3-important.svg?logo=gnu
[license-link]: https://www.gnu.org/licenses/agpl-3.0
[commercial-shield]: https://img.shields.io/badge/License-Contact-white.svg?logoColor=white&logo=telegram&color=blue
[commercial-link]: mailto:license@cherry-ai.com?subject=Commercial%20License%20Inquiry
[sponsor-shield]: https://img.shields.io/badge/Sponsor-FF6699.svg?logo=githubsponsors&logoColor=white
[sponsor-link]: https://github.com/CherryHQ/cherry-studio/blob/main/docs/sponsor.md



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2025-07-22

---