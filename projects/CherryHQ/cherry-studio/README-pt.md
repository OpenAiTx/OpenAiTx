<div align="right" >
  <details>
    <summary >🌐 Idioma</summary>
    <div>
      <div align="right">
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=en">English</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-CN">简体中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=zh-TW">繁體中文</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ja">日本語</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ko">한국어</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=hi">हिन्दी</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=th">ไทย</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fr">Français</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=de">Deutsch</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=es">Español</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=it">Italiano</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ru">Русский</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pt">Português</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=nl">Nederlands</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=pl">Polski</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=ar">العربية</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=fa">فارسی</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=tr">Türkçe</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=vi">Tiếng Việt</a></p>
        <p><a href="https://openaitx.github.io/view.html?user=CherryHQ&project=cherry-studio&lang=id">Bahasa Indonesia</a></p>
      </div>
    </div>
  </details>
</div>

<h1 align="center">
  <a href="https://github.com/CherryHQ/cherry-studio/releases">
    <img src="https://github.com/CherryHQ/cherry-studio/blob/main/build/icon.png?raw=true" width="150" height="150" alt="banner" /><br>
  </a>
</h1>

<p align="center">English | <a href="./docs/README.zh.md">中文</a> | <a href="https://cherry-ai.com">Site Oficial</a> | <a href="https://docs.cherry-ai.com/cherry-studio-wen-dang/en-us">Documentos</a> | <a href="./docs/dev.md">Desenvolvimento</a> | <a href="https://github.com/CherryHQ/cherry-studio/issues">Feedback</a><br></p>

<div align="center">
  
[![][deepwiki-shield]][deepwiki-link]
[![][twitter-shield]][twitter-link]
[![][discord-shield]][discord-link]
[![][telegram-shield]][telegram-link]

</div>
<div align="center">
  
[![][github-release-shield]][github-release-link]
[![][github-nightly-shield]][github-nightly-link]
[![][github-contributors-shield]][github-contributors-link]
[![][license-shield]][license-link]
[![][commercial-shield]][commercial-link]
[![][sponsor-shield]][sponsor-link]

</div>

<div align="center">
 <a href="https://hellogithub.com/repository/1605492e1e2a4df3be07abfa4578dd37" target="_blank" style="text-decoration: none"><img src="https://api.hellogithub.com/v1/widgets/recommend.svg?rid=1605492e1e2a4df3be07abfa4578dd37" alt="Destaque｜HelloGitHub"  width="220" height="55" /></a>
 <a href="https://trendshift.io/repositories/11772" target="_blank" style="text-decoration: none"><img src="https://trendshift.io/api/badge/repositories/11772" alt="kangfenmao%2Fcherry-studio | Trendshift" width="220" height="55" /></a>
 <a href="https://www.producthunt.com/posts/cherry-studio?embed=true&utm_source=badge-featured&utm_medium=badge&utm_souce=badge-cherry&#0045;studio" target="_blank"><img src="https://api.producthunt.com/widgets/embed-image/v1/featured.svg?post_id=496640&theme=light" alt="Cherry&#0032;Studio - AI&#0032;Chatbots&#0044;&#0032;AI&#0032;Desktop&#0032;Client | Product Hunt" width="220" height="55" /></a>
</div>

# 🍒 Cherry Studio

Cherry Studio é um cliente desktop que suporta múltiplos provedores LLM, disponível para Windows, Mac e Linux.

👏 Participe do [Grupo Telegram](https://t.me/CherryStudioAI)｜[Discord](https://discord.gg/wez8HtpxqQ) | [Grupo QQ(575014769)](https://qm.qq.com/q/lo0D4qVZKi)

❤️ Gosta do Cherry Studio? Dê uma estrela 🌟 ou [Seja um Patrocinador](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/sponsor.md) para apoiar o desenvolvimento!

# 🌠 Captura de Tela

![](https://github.com/user-attachments/assets/36dddb2c-e0fb-4a5f-9411-91447bab6e18)

![](https://github.com/user-attachments/assets/f549e8a0-2385-40b4-b52b-2039e39f2930)

![](https://github.com/user-attachments/assets/58e0237c-4d36-40de-b428-53051d982026)

# 🌟 Principais Recursos

1. **Suporte a Diversos Provedores LLM**:

- ☁️ Principais Serviços Cloud LLM: OpenAI, Gemini, Anthropic e mais
- 🔗 Integração com Serviços Web de IA: Claude, Peplexity, Poe e outros
- 💻 Suporte a Modelos Locais com Ollama, LM Studio

2. **Assistentes de IA & Conversas**:

- 📚 Mais de 300 Assistentes de IA pré-configurados
- 🤖 Criação de Assistentes Personalizados
- 💬 Conversas Simultâneas com Vários Modelos

3. **Processamento de Documentos & Dados**:

- 📄 Suporte a Texto, Imagens, Office, PDF e mais
- ☁️ Gerenciamento e Backup de Arquivos via WebDAV
- 📊 Visualização de Gráficos Mermaid
- 💻 Realce de Sintaxe de Código

4. **Integração de Ferramentas Práticas**:

- 🔍 Funcionalidade de Pesquisa Global
- 📝 Sistema de Gerenciamento de Tópicos
- 🔤 Tradução com IA
- 🎯 Ordenação por Arrastar e Soltar
- 🔌 Suporte a Mini Programas
- ⚙️ Servidor MCP (Model Context Protocol)

5. **Experiência do Usuário Aprimorada**:

- 🖥️ Suporte Multiplataforma para Windows, Mac e Linux
- 📦 Pronto para Usar - Sem Necessidade de Configuração de Ambiente
- 🎨 Temas Claro/Escuro e Janela Transparente
- 📝 Renderização Completa de Markdown
- 🤲 Compartilhamento de Conteúdo Facilitado

# 📝 Roteiro

Estamos trabalhando ativamente nos seguintes recursos e melhorias:

1. 🎯 **Recursos Principais**

- Assistente de Seleção com aprimoramento inteligente de seleção de conteúdo
- Pesquisa Avançada com capacidades de investigação aprofundada
- Sistema de Memória com consciência de contexto global
- Pré-processamento de Documentos com melhor manipulação de arquivos
- MCP Marketplace para o ecossistema do Model Context Protocol

2. 🗂 **Gestão do Conhecimento**

- Notas e Coleções
- Visualização em Tela Dinâmica (Canvas)
- Capacidades de OCR
- Suporte a TTS (Texto para Fala)

3. 📱 **Suporte a Plataformas**

- Edição HarmonyOS (PC)
- App Android (Fase 1)
- App iOS (Fase 1)
- Suporte a Multi-Janela
- Funcionalidade de Fixar Janela

4. 🔌 **Recursos Avançados**

- Sistema de Plugins
- ASR (Reconhecimento Automático de Fala)
- Refatoração de Interação entre Assistente e Tópicos

Acompanhe nosso progresso e contribua no nosso [quadro do projeto](https://github.com/orgs/CherryHQ/projects/7).

Quer influenciar nosso roteiro? Participe das nossas [Discussões no GitHub](https://github.com/CherryHQ/cherry-studio/discussions) para compartilhar suas ideias e feedback!

# 🌈 Tema

- Galeria de Temas: <https://cherrycss.com>
- Tema Aero: <https://github.com/hakadao/CherryStudio-Aero>
- Tema PaperMaterial: <https://github.com/rainoffallingstar/CherryStudio-PaperMaterial>
- Claude dynamic-style: <https://github.com/bjl101501/CherryStudio-Claudestyle-dynamic>
- Tema Maple Neon: <https://github.com/BoningtonChen/CherryStudio_themes>

PRs são bem-vindos para mais temas

# 🤝 Contribuindo

Agradecemos contribuições para o Cherry Studio! Veja algumas formas de contribuir:

1. **Contribuir com Código**: Desenvolva novos recursos ou otimize o código existente.
2. **Corrigir Bugs**: Envie correções para qualquer bug encontrado.
3. **Manter Issues**: Ajude a gerenciar issues no GitHub.
4. **Design de Produto**: Participe das discussões de design.
5. **Escrever Documentação**: Melhore manuais e guias do usuário.
6. **Engajamento Comunitário**: Participe de discussões e ajude usuários.
7. **Promover o Uso**: Divulgue o Cherry Studio.

Consulte a [Estratégia de Branches](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/docs/branching-strategy-en.md) para orientações de contribuição

## Primeiros Passos

1. **Faça um Fork do Repositório**: Faça um fork e clone para sua máquina local.
2. **Crie um Branch**: Para suas alterações.
3. **Envie as Alterações**: Faça commit e push das suas mudanças.
4. **Abra um Pull Request**: Descreva suas alterações e motivos.

Para diretrizes mais detalhadas, consulte nosso [Guia de Contribuição](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/CONTRIBUTING.md).

Obrigado pelo seu apoio e contribuições!

# 🔧 Programa de Cocriação para Desenvolvedores

Estamos lançando o Programa de Cocriação Cherry Studio para fomentar um ciclo saudável e de feedback positivo dentro do ecossistema open-source. Acreditamos que grandes softwares são construídos colaborativamente, e cada pull request aceito traz nova vida ao projeto.

Convidamos sinceramente você a se juntar ao nosso time de colaboradores e construir o futuro do Cherry Studio conosco.

## Programa de Recompensas para Contribuidores

Para valorizar nossos principais colaboradores e criar um ciclo virtuoso, estabelecemos o seguinte plano de incentivos de longo prazo.

**O primeiro período de acompanhamento deste programa será o 3º trimestre de 2025 (julho, agosto e setembro). As recompensas deste ciclo serão distribuídas em 1º de outubro.**

Dentro de qualquer período de acompanhamento (por exemplo, de 1º de julho a 30 de setembro para o primeiro ciclo), qualquer desenvolvedor que contribuir com mais de **30 commits significativos** em qualquer projeto open-source da Cherry Studio no GitHub será elegível para os seguintes benefícios:

- **Patrocínio de Assinatura Cursor**: Receba um crédito ou reembolso de **$70 USD** para sua assinatura [Cursor](https://cursor.sh/), tornando a IA sua parceira de codificação mais eficiente.
- **Acesso Ilimitado a Modelos**: Tenha chamadas de API **ilimitadas** para os modelos **DeepSeek** e **Qwen**.
- **Acesso a Tecnologias de Ponta**: Desfrute de benefícios ocasionais, incluindo acesso à API de modelos como **Claude**, **Gemini** e **OpenAI**, mantendo você na vanguarda da tecnologia.

## Crescendo Juntos & Planos Futuros

Uma comunidade vibrante é a força motriz por trás de qualquer projeto open-source sustentável. À medida que a Cherry Studio cresce, nosso programa de recompensas também evoluirá. Estamos comprometidos em alinhar continuamente nossos benefícios com as melhores ferramentas e recursos do setor. Isso garante que nossos principais colaboradores recebam suporte significativo, criando um ciclo positivo onde desenvolvedores, comunidade e projeto crescem juntos.

**A partir de agora, o projeto também adotará uma postura cada vez mais aberta para retribuir a toda a comunidade open-source.**

## Como Começar?

Aguardamos ansiosamente seu primeiro Pull Request!

Você pode começar explorando nossos repositórios, escolhendo uma `good first issue`, ou propondo suas próprias melhorias. Cada commit é um testemunho do espírito open source.

Obrigado pelo seu interesse e contribuições.

Vamos construir juntos.

# 🏢 Edição Empresarial

Com base na Edição Comunitária, temos o orgulho de apresentar a **Cherry Studio Enterprise Edition**—uma plataforma de produtividade e gestão de IA, implantável de forma privada, projetada para equipes e empresas modernas.

A Edição Empresarial aborda desafios centrais na colaboração em equipe ao centralizar a gestão de recursos, conhecimento e dados de IA. Ela capacita organizações a aumentar a eficiência, fomentar inovação e garantir conformidade, mantendo 100% do controle sobre seus dados em um ambiente seguro.

## Vantagens Principais

- **Gestão Unificada de Modelos**: Integre e gerencie centralmente diversos LLMs na nuvem (ex: OpenAI, Anthropic, Google Gemini) e modelos privados implantados localmente. Funcionários podem utilizá-los imediatamente, sem configuração individual.
- **Base de Conhecimento Empresarial**: Construa, gerencie e compartilhe bases de conhecimento em toda a equipe. Garante retenção e consistência do conhecimento, permitindo interação com IA baseada em informações unificadas e precisas.
- **Controle de Acesso Granular**: Gerencie facilmente contas de funcionários e atribua permissões baseadas em funções para diferentes modelos, bases de conhecimento e recursos através de um painel administrativo unificado.
- **Implantação Totalmente Privada**: Implemente todo o serviço backend em seus servidores locais ou nuvem privada, garantindo que seus dados permaneçam 100% privados e sob seu controle, atendendo aos mais rigorosos padrões de segurança e conformidade.
- **Serviços Backend Confiáveis**: Oferece serviços de API estáveis e mecanismos de backup e recuperação de dados em nível empresarial para garantir a continuidade dos negócios.

## ✨ Demo Online

> 🚧 **Aviso de Beta Pública**
>
> A Edição Empresarial está atualmente em estágio inicial de beta pública, e estamos ativamente iterando e otimizando seus recursos. Sabemos que pode não estar perfeitamente estável ainda. Caso encontre algum problema ou tenha sugestões valiosas durante o teste, agradecemos se puder entrar em contato por e-mail para enviar seu feedback.

**🔗 [Cherry Studio Enterprise](https://www.cherry-ai.com/enterprise)**

## Comparativo de Versões

| Recurso           | Edição Comunitária                         | Edição Empresarial                                                                                                                       |
| :---------------- | :----------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------- |
| **Open Source**   | ✅ Sim                                     | ⭕️ Parcialmente liberado para clientes                                                                                                   |
| **Custo**         | Gratuito para uso pessoal / Licença Comercial | Compra / Taxa de Assinatura                                                                                                             |
| **Painel Admin**  | —                                          | ● Acesso Centralizado a **Modelos**<br>● Gestão de **Funcionários**<br>● **Base de Conhecimento** Compartilhada<br>● Controle de **Acesso**<br>● **Backup de Dados** |
| **Servidor**      | —                                          | ✅ Implantação Privada Dedicada                                                                                                          |

## Obtenha a Edição Empresarial

Acreditamos que a Edição Empresarial será o motor de produtividade em IA de sua equipe. Se você tem interesse na Cherry Studio Enterprise Edition e deseja saber mais, solicitar uma cotação ou agendar uma demonstração, fique à vontade para entrar em contato.

- **Para Consultas Comerciais & Compras**:
  **📧 [bd@cherry-ai.com](https://raw.githubusercontent.com/CherryHQ/cherry-studio/main/mailto:bd@cherry-ai.com)**

# 🔗 Projetos Relacionados

- [one-api](https://github.com/songquanpeng/one-api): Sistema de gestão e distribuição de APIs LLM compatível com modelos como OpenAI, Azure e Anthropic. Possui interface API unificada, ideal para gestão de chaves e distribuição secundária.

- [ublacklist](https://github.com/iorate/ublacklist): Bloqueia sites específicos de aparecer nos resultados de busca do Google

# 🚀 Colaboradores

<a href="https://github.com/CherryHQ/cherry-studio/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=CherryHQ/cherry-studio" />
</a>
<br /><br />

# 📊 Estatísticas do GitHub

![Stats](https://repobeats.axiom.co/api/embed/a693f2e5f773eed620f70031e974552156c7f397.svg 'Repobeats analytics image')

# ⭐️ Histórico de Estrelas

<a href="https://www.star-history.com/#CherryHQ/cherry-studio&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
   <img alt="Star History Chart" src="https://api.star-history.com/svg?repos=CherryHQ/cherry-studio&type=Date" />
 </picture>
</a>

<!-- Links & Images -->

[deepwiki-shield]: https://img.shields.io/badge/Deepwiki-CherryHQ-0088CC?logo=data:image/svg+xml;base64,
[deepwiki-link]: https://deepwiki.com/CherryHQ/cherry-studio
[twitter-shield]: https://img.shields.io/badge/Twitter-CherryStudioApp-0088CC?logo=x
[twitter-link]: https://twitter.com/CherryStudioHQ
[discord-shield]: https://img.shields.io/badge/Discord-@CherryStudio-0088CC?logo=discord
[discord-link]: https://discord.gg/wez8HtpxqQ
[telegram-shield]: https://img.shields.io/badge/Telegram-@CherryStudioAI-0088CC?logo=telegram
[telegram-link]: https://t.me/CherryStudioAI

<!-- Links & Images -->
[github-release-shield]: https://img.shields.io/github/v/release/CherryHQ/cherry-studio?logo=github
[github-release-link]: https://github.com/CherryHQ/cherry-studio/releases
[github-nightly-shield]: https://img.shields.io/github/actions/workflow/status/CherryHQ/cherry-studio/nightly-build.yml?label=nightly%20build&logo=github
[github-nightly-link]: https://github.com/CherryHQ/cherry-studio/actions/workflows/nightly-build.yml
[github-contributors-shield]: https://img.shields.io/github/contributors/CherryHQ/cherry-studio?logo=github
[github-contributors-link]: https://github.com/CherryHQ/cherry-studio/graphs/contributors

<!-- Links & Images -->

[license-shield]: https://img.shields.io/badge/License-AGPLv3-important.svg?logo=gnu
[license-link]: https://www.gnu.org/licenses/agpl-3.0
[commercial-shield]: https://img.shields.io/badge/License-Contact-white.svg?logoColor=white&logo=telegram&color=blue
[commercial-link]: mailto:license@cherry-ai.com?subject=Commercial%20License%20Inquiry
[sponsor-shield]: https://img.shields.io/badge/Sponsor-FF6699.svg?logo=githubsponsors&logoColor=white
[sponsor-link]: https://github.com/CherryHQ/cherry-studio/blob/main/docs/sponsor.md



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2025-07-25

---