...
- 在 ms-windows 上权限有点奇怪
  - 如果遇到无法复制元素或类似问题，可能需要以管理员身份运行程序
- 在 Linux 系统上程序无法启动时，可能需要安装 openssl1.1
...
## 如何贡献
为开发 tauri v1 应用配置你的设备：[Tauri 先决条件](https://tauri.app/v1/guides/getting-started/prerequisites)
</br></br>
完成后只需在你的设备上的某个位置执行 ```git clone https://github.com/RickyDane/CoDriver``` 或 ```gh repo clone RickyDane/CoDriver```
</br></br>
你应该可以在该项目根目录下运行 ```cargo tauri dev``` 来开始构建和运行 CoDriver。
</br>
请确保已安装 tauri-cli：```cargo install tauri-cli```
</br>

## 发布签名
macOS 发布构件必须经过签名和公证，否则 Gatekeeper 会报告已安装的应用已损坏。请参阅 [macOS 签名与公证](https://raw.githubusercontent.com/RickyDane/CoDriver/master/docs/macos-signing-notarization.md) 获取所需的 GitHub Actions 密钥和验证命令。
