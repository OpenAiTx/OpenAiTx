...
> 🌐 파생 프로젝트: [asmr.furina.in](https://asmr.furina.in) — 심플하고 깔끔한 온라인 ASMR 청취 페이지
...
| **Web 인터페이스** | **Web 인터페이스 2** |
| ![Web 인터페이스](https://raw.githubusercontent.com/fireinrain/asmr-downloader/v2/dist/listen.png) | ![Web 인터페이스2](https://raw.githubusercontent.com/fireinrain/asmr-downloader/v2/dist/listen2.png) |
| **export 인터페이스** | **export 인터페이스 2** |
| ![export 인터페이스](https://raw.githubusercontent.com/fireinrain/asmr-downloader/v2/dist/export1.png) | ![export 인터페이스2](https://raw.githubusercontent.com/fireinrain/asmr-downloader/v2/dist/export2.png) |

<details>
<summary><b>✨ 기능 특성</b></summary>

- **검색**: 단일/대량 RJID, 고급 검색 문법, 결과 내보내기 CSV/JSON
- **다운로드**: 단일/대량/인기 작품 다운로드, 자동 속도제한, 재시도, 지수 백오프
- **동기화**: 메타데이터 동기화, 대량 다운로드 제어, 상태 추적, 실패 재시도
- **Web 인터페이스**: 시각적 탐색, 브라우저 내 재생, 반응형 디자인
- **설정**: 대화형 초기화, 프록시, 속도제한, 지터 등 고급 설정 지원
...
<details>
<summary><b>📋 명령어 옵션 빠른 참고</b></summary>

| 명령어 | 옵션 | 설명 |
|--------|------|------|
| `search` | `-c` | 검색 결과 개수(기본값 10) |
| `search download` | `-d`, `-s` | 다운로드 디렉토리, 다운로드 개수 |
| `search export` | `-f`, `-n` | 내보낼 파일명(.csv/.json), 내보내기 개수 |
| `download` | `-d`, `-n` | 다운로드 디렉토리, hot100 개수 |
| `sync download` | `-d` | 다운로드 디렉토리 |
| `sync retry` | `-d` | 실패 파일이 있는 디렉토리 |
| `sync export` | `-s`, `-f` | 상태(failed/success), 내보낼 파일 |
| `listen` | `-p` | 포트(기본값 9999) |
| `export` | `-o`, `-n` | 내보낼 디렉토리, hot100 개수 |
...
| Gin | 웹 서비스 |
| Tailwind + Plyr | 프론트엔드 인터페이스 + 오디오 재생 |
...
**웹 인터페이스 접속 불가** → 포트 점유 여부 확인, `-p`로 다른 포트 지정 시도

**검색 결과 없음** → 쿼리 구문 확인, 조건을 단순화하여 시도

**export 명령어와 연계된 다운로드 방식** → [guide](/dist/guide.pdf) 참고

</details>

## 🤝 기여

Pull Request 제출을 환영합니다! Fork → 새 브랜치 생성 → 변경 사항 커밋 → PR 생성.

## 📄 라이선스

본 프로젝트는 MIT 라이선스를 사용합니다. 자세한 내용은 [LICENSE](/LICENSE) 파일을 확인하세요.
