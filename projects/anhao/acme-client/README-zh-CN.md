...
> **语言 / Language**: [英文](https://raw.githubusercontent.com/anhao/acme-client/main/README.md) | [中文](https://raw.githubusercontent.com/anhao/acme-client/main/README_ZH.md)
...
1. **私钥**：使用适当的文件权限（600）安全存储私钥  
2. **账户密钥**：将账户密钥与证书密钥分开保存  
3. **测试环境**：使用测试环境进行测试  
4. **速率限制**：注意证书颁发机构的速率限制  
5. **验证**：在触发 ACME 验证前，始终先本地验证挑战  
...
1. 查看 [文档](#quick-start)  
2. 搜索已有的 [问题](https://github.com/anhao/acme-client/issues)  
3. 如有需要，创建新的 [问题](https://github.com/anhao/acme-client/issues/new)  
