...
* Built-in fault tolerance and high availability configuration options base on [DLedger Controller](https://raw.githubusercontent.com/apache/rocketmq/develop/docs/en/controller/quick_start.md)
...
For macOS and Linux users, execute the following commands:
...
the `host` network will be used to expose the listening port in the container.
...
Before your operations, make sure that `kubectl` and related kubeconfig file are installed on your machine.
