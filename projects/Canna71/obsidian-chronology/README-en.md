...
The calendar allows you to navigate and select a daily, weekly, monthly, or custom range of notes. Daily and Weekly ranges are displayed in a timeline but you can force the display of the notes in a simple list by enabling the setting `Display Simple List`.
...
To select a range of dates to display: select one date then shift-click the other date of the range.

The notes in the timeline will be shown with a colored mark indicating if they were **created** or **modified** at that moment.
...
The days tiles background gives an indication about how many notes were created or edited that day. The scale also depends on the average number of daily notes set in the settings.

# Obsidian Sync Issues
If you have Obsidian Sync, you might have noticed that synced files will have a creation timestamp corresponding to when they were synced to the other machines.
