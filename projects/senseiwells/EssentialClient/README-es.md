# <img src="https://raw.githubusercontent.com/senseiwells/EssentialClient/26.1/./src/main/resources/assets/essential-client/icon.png" align="center" width="64px"/> EssentialClient

[![Discord](https://badgen.net/discord/online-members/7R9SfktZxH?icon=discord&label=Discord&list=what)](https://discord.gg/7R9SfktZxH)
[![Modrinth downloads](https://img.shields.io/modrinth/dt/essentialclient?label=Modrinth%20downloads&logo=data:image/svg+xml;base64,)](https://modrinth.com/mod/essentialclient)
EssentialClient es un mod del lado del cliente que añade muchas utilidades para el cliente así como
proporciona una interfaz gráfica para modificar las reglas de carpet!

> [!NOTA]
> EssentialClient está actualmente en proceso de reescritura.
> Algunas funciones de versiones anteriores han sido eliminadas, ya sea porque no creo
> que encajen en este mod o porque son difíciles de mantener.
> 
> He eliminado completamente la capacidad de scripting de EssentialClient ya que tenía
> fallos importantes en su diseño que lo hacían incómodo de mantener y también de usar.
> El objetivo a largo plazo es implementar una solución mejor; sin embargo, esto tomará
> algo de tiempo.
> 
> De ahora en adelante, solo se soportará la versión más reciente de Minecraft.


## Uso

Este mod requiere fabric-launcher, fabric-api, fabric-kotlin, y
YetAnotherConfigLib.

Opcionalmente, puedes instalar chunk-debug, fabric-carpet, y mod-menu.

Una vez instalado, puedes acceder al menú de configuración a través de mod-menu, o
presionando la tecla de acceso rápido del Menú de EssentialClient (que puede cambiarse en opciones).

Desde aquí puedes ver y modificar los ajustes de EssentialClient, los ajustes del servidor Carpet,
o abrir el mapa de ChunkDebug (si tienes ChunkDebug instalado).


---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2026-04-23

---