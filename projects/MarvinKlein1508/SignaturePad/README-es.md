# SignaturePad
Un componente Blazor fácil de usar para dibujar una firma. Soporta entradas tanto de ratón como táctiles y funciona en Blazor Server y Blazor WebAssembly.

Vea una [demostración](https://marvinklein1508.github.io/SignaturePad/) en vivo aquí mismo en GitHub.

![SignaturePad Demo](https://raw.githubusercontent.com/MarvinKlein1508/SignaturePad/master/images/demo.png)

## Instalación

Puede instalarlo desde Nuget usando el siguiente comando:

`Install-Package Blazor.SignaturePad`

O a través del gestor de paquetes de Visual Studio.

## Uso básico
Comience añadiendo la siguiente declaración using a su archivo raíz `_Imports.razor`.
```razor
@using SigPad
```

A continuación, debe definir una propiedad en su clase. Por ejemplo:
```csharp
public class MyInput
{
    public byte[] Signature { get; set; } = Array.Empty<byte>();
}
```

Luego puedes usarlo donde quieras.
```razor
<SignaturePad @bind-Value="Input.Signature" style="width: 100%" />
```

El control te proporciona los datos de la imagen como un `byte[]` en base64

Para obtener la imagen, necesitarás convertir el `byte[]` en una cadena. Por ejemplo:

```csharp
public class MyInput
{
    public byte[] Signature { get; set; } = Array.Empty<byte>();
    public string SignatureAsBase64 => System.Text.Encoding.UTF8.GetString(Signature);
}
```

## Proporcionar opciones
Puedes configurar el SignaturePad proporcionando una instancia de `SignaturePadOptions` al componente. 

```razor
<SignaturePad @bind-Value="Input.Signature" Options="_options" style="width: 100%" />

@code {
    public MyInput Input { get; set; } = new();

    private SignaturePadOptions _options = new SignaturePadOptions
    {
        LineCap = LineCap.Round,
        LineJoin = LineJoin.Round,
        LineWidth = 20
    };
}
```

## Usando cadenas base64 existentes
Por defecto, SignaturePad antepone la firma con `data:image/png;base64, `. Al cargar cadenas base64 existentes, este prefijo debe aplicarse así:

```cs
Input.Signature = Encoding.UTF8.GetBytes("data:image/png;base64, ");
```

## Estilizado personalizado
Puedes personalizar la apariencia de SignaturePad ya sea sobrescribiendo las clases CSS o especificando tus propias clases.

Para el SignaturePad en sí, puedes usar el parámetro `Class` y para el botón que limpia el SignaturePad puedes usar el parámetro `ClearButtonClass`.



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2026-01-16

---