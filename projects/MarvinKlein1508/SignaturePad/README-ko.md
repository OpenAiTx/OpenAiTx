# SignaturePad
서명 그리기를 위한 간단한 Blazor 컴포넌트입니다. 마우스와 터치 입력을 모두 지원하며 Blazor Server와 Blazor WebAssembly에서 작동합니다.

Github에서 바로 볼 수 있는 [라이브 데모](https://marvinklein1508.github.io/SignaturePad/)를 확인하세요.

![SignaturePad Demo](https://raw.githubusercontent.com/MarvinKlein1508/SignaturePad/master/images/demo.png)

## 설치

다음 명령어를 사용하여 Nuget에서 설치할 수 있습니다:

`Install-Package Blazor.SignaturePad`

또는 Visual Studio 패키지 관리자를 통해 설치할 수 있습니다.

## 기본 사용법
루트 `_Imports.razor`에 다음 using 문을 추가하는 것부터 시작하세요.
```razor
@using SigPad
```
다음으로 클래스에 속성을 정의해야 합니다. 예를 들어:

```csharp
public class MyInput
{
    public byte[] Signature { get; set; } = Array.Empty<byte>();
}
```
그 다음에는 원하는 곳에 그것을 사용할 수 있습니다.

```razor
<SignaturePad @bind-Value="Input.Signature" style="width: 100%" />
```

컨트롤은 이미지 데이터를 base64 `byte[]` 형식으로 제공합니다.

이미지를 얻으려면 `byte[]`를 문자열로 변환해야 합니다. 예를 들어:

```csharp
public class MyInput
{
    public byte[] Signature { get; set; } = Array.Empty<byte>();
    public string SignatureAsBase64 => System.Text.Encoding.UTF8.GetString(Signature);
}
```

## 옵션 제공하기
`SignaturePadOptions` 인스턴스를 컴포넌트에 제공하여 SignaturePad를 구성할 수 있습니다. 

```razor
<SignaturePad @bind-Value="Input.Signature" Options="_options" style="width: 100%" />

@code {
    public MyInput Input { get; set; } = new();

    private SignaturePadOptions _options = new SignaturePadOptions
    {
        LineCap = LineCap.Round,
        LineJoin = LineJoin.Round,
        LineWidth = 20
    };
}
```

## 기존 base64 문자열 사용하기
기본적으로 SignaturePad는 서명 앞에 `data:image/png;base64, `를 붙입니다. 기존 base64 문자열을 불러올 때는 이 접두사를 다음과 같이 적용해야 합니다:

```cs
Input.Signature = Encoding.UTF8.GetBytes("data:image/png;base64, ");
```

## 사용자 정의 스타일
SignaturePad의 모양은 CSS 클래스를 재정의하거나 직접 클래스를 지정하여 사용자 정의할 수 있습니다.

SignaturePad 자체에는 `Class` 매개변수를 사용할 수 있으며, SignaturePad를 지우는 버튼에는 `ClearButtonClass` 매개변수를 사용할 수 있습니다.



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2026-01-16

---