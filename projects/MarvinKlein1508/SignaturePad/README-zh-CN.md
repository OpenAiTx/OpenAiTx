# SignaturePad
一个简单易用的 Blazor 组件，用于绘制签名。它支持鼠标和触摸输入，适用于 Blazor Server 和 Blazor WebAssembly。

在 GitHub 上查看实时[演示](https://marvinklein1508.github.io/SignaturePad/)。

![SignaturePad 演示](https://raw.githubusercontent.com/MarvinKlein1508/SignaturePad/master/images/demo.png)
...
或者通过 Visual Studio 包管理器安装。

## 基本用法
首先，在您的根目录 `_Imports.razor` 中添加以下 using 语句。
```razor
@using SigPad
```
接下来，您应该在类中定义一个属性。例如：
...
```
控件以base64格式提供图像数据，类型为`byte[]`

要获取图像，您需要将`byte[]`转换为字符串。例如：
...
## 提供选项
您可以通过向组件提供一个 `SignaturePadOptions` 实例来配置 SignaturePad。 
...
## 使用现有的 base64 字符串
默认情况下，SignaturePad 会在签名前加上前缀 `data:image/png;base64, `。加载现有的 base64 字符串时，应如下应用此前缀：
...
## 自定义样式
您可以通过覆盖 CSS 类或指定您自己的类来自定义 SignaturePad 的外观。

对于 SignaturePad 本身，您可以使用 `Class` 参数，对于清除 SignaturePad 的按钮，您可以使用 `ClearButtonClass` 参数。
