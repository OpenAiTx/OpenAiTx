# SignaturePad
署名を描くための使いやすいBlazorコンポーネントです。マウスとタッチ入力の両方に対応しており、Blazor ServerおよびBlazor WebAssemblyで動作します。

こちらのGitHubでライブの[デモ](https://marvinklein1508.github.io/SignaturePad/)をご覧いただけます。

![SignaturePad Demo](https://raw.githubusercontent.com/MarvinKlein1508/SignaturePad/master/images/demo.png)

## インストール

以下のコマンドを使用してNugetからインストールできます：

`Install-Package Blazor.SignaturePad`

またはVisual Studioのパッケージマネージャーからも可能です。

## 基本的な使い方
ルートの`_Imports.razor`に次のusingステートメントを追加して開始します。
```razor
@using SigPad
```
次に、クラスにプロパティを定義する必要があります。例えば：

```csharp
public class MyInput
{
    public byte[] Signature { get; set; } = Array.Empty<byte>();
}
```
それを好きな場所で使用できます。

```razor
<SignaturePad @bind-Value="Input.Signature" style="width: 100%" />
```

コントロールは画像データをbase64の`byte[]`として提供します

画像を取得するには、`byte[]`を文字列に変換する必要があります。例えば:

```csharp
public class MyInput
{
    public byte[] Signature { get; set; } = Array.Empty<byte>();
    public string SignatureAsBase64 => System.Text.Encoding.UTF8.GetString(Signature);
}
```

## オプションの提供
`SignaturePadOptions` インスタンスをコンポーネントに渡すことで、SignaturePad を設定できます。 

```razor
<SignaturePad @bind-Value="Input.Signature" Options="_options" style="width: 100%" />

@code {
    public MyInput Input { get; set; } = new();

    private SignaturePadOptions _options = new SignaturePadOptions
    {
        LineCap = LineCap.Round,
        LineJoin = LineJoin.Round,
        LineWidth = 20
    };
}
```

## 既存のbase64文字列の使用
デフォルトでは、SignaturePadは署名の前に`data:image/png;base64, `を付加します。既存のbase64文字列を読み込む場合は、この接頭辞を次のように適用する必要があります：

```cs
Input.Signature = Encoding.UTF8.GetBytes("data:image/png;base64, ");
```

## カスタムスタイリング
SignaturePadの見た目は、CSSクラスを上書きするか、独自のクラスを指定することでカスタマイズできます。

SignaturePad自体には`Class`パラメータを使用し、SignaturePadをクリアするボタンには`ClearButtonClass`パラメータを使用できます。



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2026-01-16

---