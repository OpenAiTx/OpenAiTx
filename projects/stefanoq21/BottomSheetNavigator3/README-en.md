...
...
```
Define your routes as strings or data classes (depending on the version of compose navigation you are using):
```
...
...
To implement a back or close button in your bottom sheet, I suggest using `onBackPressedDispatcher.onBackPressed()`. This is because if you use `navController.popBackStack()` the animation will not appear. The animation was disabled, in this case, to avoid problems during the navigation that starts from bottom sheets.

### Customization

The library currently supports the same customization options as the standard `androidx.compose.material3.ModalBottomSheet`. You can customize the appearance of all the bottom sheets used in your navigation graph by passing the parameters to the `ModalBottomSheetLayout`.
