...
// transitional options for backward compatibility that may be removed in newer versions
transitional: {
...
Config will be merged with an order of precedence. The order is library defaults found in [lib/defaults/index.js](https://github.com/axios/axios/blob/main/lib/defaults/index.js#L49), then the `defaults` property of the instance, and finally the `config` argument for the request. The latter will take precedence over the former. Here's an example.
...
Starting from `v0.22.0`, Axios supports AbortController to cancel requests in the same way as the fetch API:
...
Pass the HTML Form element as a payload to submit it as `multipart/form-data` content.
...
```js
      ...
      download: true; // download indicator
    }*/
  }
});  
```
...
Axios has its own `AxiosHeaders` class to manipulate headers using a Map-like API that guarantees case-insensitive operation.
Although HTTP is case-insensitive for headers, Axios will retain the case of the original header for stylistic reasons
and as a workaround for when servers mistakenly consider header case.
The old approach of directly manipulating the headers object is still available, but deprecated and not recommended for future use.
...
- `false` - skip header when rendering to JSON, additionally indicates that the `set` method must be called with the `rewrite` option set to `true`
...
```ts
  axios.interceptors.request.use((request: InternalAxiosRequestConfig) => {
      ...
      request.headers.set('User-Agent', false); // disable subsequent setting of the header by Axios
...
The Fetch adapter was introduced in `v1.7.0`. By default, it will be used if the `xhr` and `http` adapters are not available in the build,
