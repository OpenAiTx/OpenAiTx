...
Bibliothèque de pilotes ESP32 pour TWAI / CAN pour Arduino utilisant les pilotes ESP-IDF.

Testé sur ESP32 et ESP32-S3.

# Utilisation

La bibliothèque contient tout dans son en-tête, il suffit de l'inclure puis d'utiliser l'objet `ESP32Can` pour envoyer ou recevoir des `CanFrame`.


Voici un exemple simple de comment interroger et recevoir des trames PID OBD2 :
```cpp
...
# Avancé
Vous pouvez également configurer vos propres masques et configurations :
...
Consultez `soc/twai_types.h` pour plus d'informations :
