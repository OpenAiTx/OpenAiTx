...
ESP-IDFドライバーを使用したArduino用ESP32のTWAI / CANドライバライブラリ。

ESP32およびESP32-S3でテスト済み。

# 使用方法

ライブラリはヘッダー内にすべて含まれているので、それをインクルードし、`ESP32Can`オブジェクトを使用して`CanFrame`を送受信します。


以下はOBD2 PIDフレームをクエリして受信する簡単な例です：
```cpp
...
詳細は `soc/twai_types.h` を参照してください：
