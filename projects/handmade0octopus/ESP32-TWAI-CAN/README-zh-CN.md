...
用于 Arduino 的 ESP32 TWAI / CAN 驱动库，基于 ESP-IDF 驱动。

已在 ESP32 和 ESP32-S3 上测试。

# 使用方法

库的所有内容都包含在头文件中，只需包含该头文件，然后使用 `ESP32Can` 对象发送或接收 `CanFrame`。

下面是一个查询和接收 OBD2 PID 帧的简单示例：
