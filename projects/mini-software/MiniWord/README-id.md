...
<div align="center">
 <a href="https://github.com/mini-software/MiniWord">Star</a> dan <a href="https://miniexcel.github.io">dotnate</a> Anda dapat membuat MiniWord menjadi lebih baik
</div>
...
Template mengikuti desain "WHAT you see is what you get", dan gaya tag template sepenuhnya dipertahankan.
...
Nilai tag berupa tipe `string[]` atau `IList<string>`
...
Nilai tag adalah tipe `IEmerable<Dictionary<string,object>>`
...
Nilai tag adalah tipe `IEnumerable<MiniWordForeach>`. Penambahan tag `{{foreach` dan `endforeach}}` pada template diperlukan.
...
Untuk multi paragraf, gunakan tag @if dan @endif.
Untuk satu paragraf dan di dalam foreach, gunakan tag `{{if` dan `endif}}` sesuai kebutuhan template.
...
Jika tipe value adalah `MiniWordHyperLink` maka sistem akan mengganti string template dengan hyperlink.
