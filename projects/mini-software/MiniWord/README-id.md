...
<div align="center">
 Star <a href="https://github.com/mini-software/MiniWord">Anda</a> dan <a href="https://miniexcel.github.io">donasi</a> Anda dapat membuat MiniWord menjadi lebih baik
</div>
...
MiniWord adalah pustaka Template Word .NET yang mudah dan efektif.
...
- tautan nuget : https://www.nuget.org/packages/MiniWord

### Mulai Cepat

Template mengikuti desain "APA yang Anda lihat adalah apa yang Anda dapatkan", dan gaya tag template sepenuhnya terjaga.
...
- Input mendukung file path, byte[]
- Output mendukung file path, byte[], stream
...
Format string template MiniWord seperti Vue, React `{{tag}}`, pengguna hanya perlu memastikan tag dan key parameter value sama maka sistem akan menggantinya secara otomatis.
...
Tipe value adalah `MiniWordPicture`
...
Nilai tag bertipe `string[]` atau `IList<string>`
...
Nilai tag bertipe `IEmerable<Dictionary<string,object>>`
...
var value = new Dictionary<string, object>()
{
    ["TripHs"] = new List<Dictionary<string, object>>
    {
        new Dictionary<string, object>
        {
            ...
            { "How","Diskusi kebutuhan bagian 1"},
        ...
        new Dictionary<string, object>
        {
            ...
            { "How","Diskusi kebutuhan bagian 2 dan pengembangan"},
...
Nilai tag bertipe `IEnumerable<MiniWordForeach>`. Menambahkan tag `{{foreach` dan `endforeach}}` ke template diperlukan.
...
### Pernyataan If di dalam template

Untuk beberapa paragraf, gunakan tag @if dan @endif.
Untuk satu paragraf dan di dalam foreach, gunakan tag `{{if` dan `endif}}` agar template diperlukan.
...
### Parameter POCO atau dynamic

v0.5.0 mendukung parameter POCO atau dynamic
...
Jika tipe value adalah `MiniWordHyperLink`, sistem akan mengganti string template dengan hyperlink.

* Url: Target path URI HyperLink
* Text: Deskripsi
