...
<div align="center">
 あなたの<a href="https://github.com/mini-software/MiniWord">スター</a>や<a href="https://miniexcel.github.io">寄付</a>がMiniWordをより良くします
</div>
...
テンプレートは「見たまま」を設計思想としており、テンプレートタグのスタイルも完全に保持されます。
...
MiniWordのテンプレートはVueやReactの`{{tag}}`のような形式の文字列で、ユーザーはタグとvalueパラメータのキーが一致していれば自動的に置換されます。
...
タグ値は `string[]` または `IList<string>` 型です。
...
タグ値は `IEnumerable<MiniWordForeach>` 型です。テンプレートに `{{foreach` および `endforeach}}` タグを追加する必要があります。
...
複数段落の場合は、@if および @endif タグを使用します。
単一段落やforeach内では、テンプレートに `{{if` および `endif}}` タグが必要です。
...
### POCO または dynamic パラメータ

v0.5.0 から POCO または dynamic パラメータをサポート
