...
<div align="center">
 あなたの<a href="https://github.com/mini-software/MiniWord">Star</a>と<a href="https://miniexcel.github.io">dotnate</a>がMiniWordをより良くします
</div>
...
MiniWordは、簡単かつ効果的な.NET用のWordテンプレートライブラリです。
...
- nugetリンク : https://www.nuget.org/packages/MiniWord

### クイックスタート

テンプレートは「見たまま（WHAT you see is what you get）」デザインに従い、テンプレートタグのスタイルは完全に保持されます。
...
MiniWordのテンプレートフォーマット文字列は、VueやReactのように`{{tag}}`形式です。ユーザーはタグとvalueパラメータのキーを一致させるだけで、自動的に置換されます。
...
タグの値は`string[]`または`IList<string>`型です。
...
タグの値は`IEmerable<Dictionary<string,object>>`型です。
...
タグの値は`IEnumerable<MiniWordForeach>`型です。テンプレートに`{{foreach`と`endforeach}}`タグを追加する必要があります。
...
複数段落の場合、@if および @endif タグを使用してください。
単一段落および foreach 内では、テンプレートが必要な場合、`{{if` および `endif}}` タグを使用してください。
...
v0.5.0 でPOCOまたはdynamicパラメータをサポート
...
値の型が `MiniWordHyperLink` の場合、システムはテンプレート文字列をハイパーリンクで置き換えます。
