...
MiniWord ist eine einfache und effektive .NET Word-Vorlagenbibliothek.
...
Die Vorlage folgt dem „What you see is what you get“-Prinzip, und die Formatierung der Vorlagen-Tags bleibt vollständig erhalten.
...
Das MiniWord-Vorlagenformat verwendet Zeichenfolgen wie bei Vue, React `{{tag}}`. Benutzer müssen lediglich sicherstellen, dass Tag und Schlüssel im value-Parameter übereinstimmen, dann werden sie vom System automatisch ersetzt.
...
Der Tag-Wert ist vom Typ `IEmerable<Dictionary<string,object>>`
...
Der Tag-Wert ist vom Typ `IEnumerable<MiniWordForeach>`. Das Hinzufügen von `{{foreach` und `endforeach}}` Tags zur Vorlage ist erforderlich.
...
Für einzelne Absätze und innerhalb von foreach verwenden Sie die Tags `{{if` und `endif}}`, um die Vorlage zu definieren.
...
Ab Version 0.5.0 werden POCO oder dynamische Parameter unterstützt.
...
Wenn der Wert vom Typ `MiniWordHyperLink` ist, ersetzt das System den Platzhalter im Template durch einen Hyperlink.
