...
Şablon, "NE görüyorsan onu alırsın" tasarımını takip eder ve şablon etiketinin stilleri tamamen korunur.
...
- Girdi dosya yolu, byte[] destekler
- Çıktı dosya yolu, byte[], stream destekler
...
MiniWord şablon formatı dizesi Vue, React `{{tag}}` gibi, kullanıcıların yalnızca etiket ve değer parametre anahtarının aynı olmasını sağlaması gerekir, ardından sistem bunları otomatik olarak değiştirecektir.
...
Tek paragraf ve foreach içinde, şablonda `{{if` ve `endif}}` etiketlerini kullanmak gereklidir.
...
Eğer değer tipi `MiniWordHyperLink` ise sistem, şablon dizesini bir hiperbağlantı ile değiştirecektir.

* Url： HiperBağlantı URI hedef yolu
* Metin：Açıklama
