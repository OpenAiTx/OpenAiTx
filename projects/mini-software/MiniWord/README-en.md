...
Template follows the "WHAT you see is what you get" design, and the template tag styles are fully preserved.
...
The tag value is of the `IEnumerable<MiniWordForeach>` type. It is necessary to add `{{foreach` and `endforeach}}` tags to the template.
...
For a single paragraph and inside foreach, use `{{if` and `endif}}` tags to template as required.
