...
<div align="center">
 ستاره‌زدن به <a href="https://github.com/mini-software/MiniWord">پروژه</a> و <a href="https://miniexcel.github.io">حمایت مالی</a> شما می‌تواند MiniWord را بهتر کند
</div>
...
MiniWord یک کتابخانه قالب ورد برای دات‌نت است که ساده و کارآمد می‌باشد.
...
- لینک نوگت : https://www.nuget.org/packages/MiniWord

### شروع سریع

الگو بر اساس طراحی "هر آنچه می‌بینید همان است که دریافت می‌کنید" ساخته شده و سبک تگ‌های قالب کاملاً حفظ می‌شود.

```csharp
var value = new Dictionary<string, object>(){["title"] = "Hello MiniWord"};
MiniSoftware.MiniWord.SaveAsByTemplate(outputPath, templatePath, value);
```
![تصویر](https://user-images.githubusercontent.com/12729184/190875707-6c5639ab-9518-4dc1-85d8-81e20af465e8.png)

### ورودی، خروجی

- ورودی از مسیر فایل، byte[] پشتیبانی می‌کند
- خروجی از مسیر فایل، byte[]، stream پشتیبانی می‌کند
...
فرمت رشته قالب MiniWord مانند Vue، React به صورت `{{tag}}` است، کاربران فقط باید مطمئن شوند که کلید برچسب و پارامتر مقدار یکسان باشد تا سیستم به طور خودکار آن‌ها را جایگزین کند.
...
![تصویر](https://user-images.githubusercontent.com/12729184/190834360-39b4b799-d523-4b7e-9331-047a61fd5eb9.png)

##### نتیجه

![تصویر](https://user-images.githubusercontent.com/12729184/190834455-ba065211-0f9d-41d1-9b7a-5d9e96ac2eff.png)

### تصویر

نوع مقدار `MiniWordPicture` است
...
مقدار tag از نوع `string[]` یا `IList<string>` است
...
مقدار تگ از نوع `IEmerable<Dictionary<string,object>>` است
...
![تصویر](https://user-images.githubusercontent.com/12729184/190843632-05bb6459-f1c1-4bdc-a79b-54889afdfeea.png)


##### نتیجه

![تصویر](https://user-images.githubusercontent.com/12729184/190843663-c00baf16-21f2-4579-9d08-996a2c8c549b.png)

### لیست داخل لیست

مقدار تگ از نوع `IEnumerable<MiniWordForeach>` است. افزودن تگ‌های `{{foreach` و `endforeach}}` به قالب الزامی است.
...
برای پاراگراف‌های چندگانه، از تگ‌های @if و @endif استفاده کنید.
برای پاراگراف تکی و داخل foreach، از تگ‌های `{{if` و `endif}}` استفاده کنید تا قالب مورد نیاز باشد.
...
نسخه 0.5.0 از پارامتر POCO یا داینامیک پشتیبانی می‌کند
...
* آدرس اینترنتی: مسیر هدف URI هایپرلینک
* متن: توضیحات
