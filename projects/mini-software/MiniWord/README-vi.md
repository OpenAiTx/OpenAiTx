...
<div align="center">
 Ngôi sao <a href="https://github.com/mini-software/MiniWord">Star</a> và <a href="https://miniexcel.github.io">ủng hộ</a> của bạn có thể làm cho MiniWord tốt hơn
</div>
...
MiniWord là một thư viện mẫu Word .NET đơn giản và hiệu quả.
...
Mẫu theo thiết kế "CÁI gì bạn thấy là cái bạn nhận được", và kiểu dáng thẻ mẫu được giữ nguyên hoàn toàn.
...
Định dạng chuỗi mẫu MiniWord giống như Vue, React `{{tag}}`, người dùng chỉ cần đảm bảo tên thẻ và key tham số value giống nhau thì hệ thống sẽ tự động thay thế.
...
var value = new Dictionary<string, object>()
{
    ...
    ["Purpose"] = "Cơ sở Thượng Hải cần một hệ thống mới để kiểm soát hệ thống nhân sự.",
...
Giá trị tag là kiểu `string[]` hoặc `IList<string>`
...
Giá trị tag là kiểu `IEmerable<Dictionary<string,object>>`
...
### Danh sách lồng nhau (List inside list)

Giá trị tag là kiểu `IEnumerable<MiniWordForeach>`. Yêu cầu thêm tag `{{foreach` và `endforeach}}` vào mẫu.
...
### Câu lệnh If trong template

Đối với nhiều đoạn văn, sử dụng thẻ @if và @endif.
Đối với một đoạn văn và bên trong foreach, sử dụng thẻ `{{if` và `endif}}` khi template yêu cầu.
...
Nếu kiểu giá trị là `MiniWordHyperLink` hệ thống sẽ thay thế chuỗi mẫu bằng hyperlink.

* Url： Đường dẫn URI HyperLink đích
* Text：Mô tả
