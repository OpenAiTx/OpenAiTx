...
MiniWord es una biblioteca de plantillas Word para .NET fácil y eficaz.
...
La plantilla sigue el diseño "LO QUE ves es lo que obtienes", y los estilos de las etiquetas de plantilla se conservan completamente.
...
El formato de cadena de la plantilla MiniWord es similar a Vue, React `{{tag}}`, los usuarios solo necesitan asegurarse de que la etiqueta y la clave del parámetro value sean iguales, entonces el sistema las reemplazará automáticamente.
...
Para un solo párrafo y dentro de foreach, use las etiquetas `{{if` y `endif}}` para que la plantilla sea requerida.
...
v0.5.0 soporta parámetros POCO o dinámicos
...
Si el tipo de valor es `MiniWordHyperLink` el sistema reemplazará la cadena de plantilla por un hipervínculo.

* Url： Ruta de destino URI del Hipervínculo
* Text：Descripción
