...
<div align="center">
 Tu <a href="https://github.com/mini-software/MiniWord">estrella</a> y tu <a href="https://miniexcel.github.io">donación</a> pueden hacer MiniWord mejor
</div>
...
MiniWord es una biblioteca de plantillas Word para .NET fácil y efectiva.
...
La plantilla sigue el diseño "LO QUE ves es lo que obtienes", y los estilos de las etiquetas de la plantilla se conservan completamente.
...
- Entrada soporta ruta de archivo, byte[]
- Salida soporta ruta de archivo, byte[], stream
...
El formato de plantilla de MiniWord es una cadena similar a Vue, React `{{tag}}`, los usuarios solo necesitan asegurarse de que la etiqueta y la clave del parámetro value sean iguales y el sistema las reemplazará automáticamente.
...
El valor de la etiqueta es de tipo `string[]` o `IList<string>`
...
El valor de la etiqueta es de tipo `IEmerable<Dictionary<string,object>>`
...
El valor de la etiqueta es de tipo `IEnumerable<MiniWordForeach>`. Es necesario agregar las etiquetas `{{foreach` y `endforeach}}` en la plantilla.
...
Para varios párrafos, use las etiquetas @if y @endif.
Para un solo párrafo y dentro de foreach, use las etiquetas `{{if` y `endif}}` cuando sea necesario en la plantilla.
...
var value = new Dictionary<string, object>()
{
    ["Name"] = new List<MiniWordHyperLink>(){
        new MiniWordHyperLink(){
            ...
            Text = "¡Enlace de prueba 22!"
        },
        new MiniWordHyperLink(){
            Url = "https://google1.com",
            Text = "¡Enlace de prueba 11!"
        }
    },
...
La versión v0.5.0 soporta parámetros POCO o dinámicos
...
* Url： URI de destino del hipervínculo
* Text：Descripción
...
#### Exportar API en ASP.NET Core 3.1
...
## Soporte : [Enlace para Donar](https://miniexcel.github.io/)
