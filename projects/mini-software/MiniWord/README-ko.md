...
<div align="center">
 여러분의 <a href="https://github.com/mini-software/MiniWord">Star</a> 및 <a href="https://miniexcel.github.io">dotnate</a>는 MiniWord를 더 나은 라이브러리로 만듭니다.
</div>
...
템플릿은 "보는 대로 얻는다(WHAT you see is what you get)" 디자인을 따르며, 템플릿 태그 스타일이 완전히 보존됩니다.
...
MiniWord 템플릿 문자열 형식은 Vue, React의 `{{tag}}`와 유사하며, 사용자는 태그와 value 파라미터의 key만 일치시키면 시스템이 자동으로 값을 치환합니다.
...
Tag 값은 `IEmerable<Dictionary<string,object>>` 타입입니다.
...
Tag 값은 `IEnumerable<MiniWordForeach>` 타입입니다. 템플릿에 `{{foreach` 및 `endforeach}}` 태그를 추가해야 합니다.
...
여러 문단을 사용할 경우, @if 와 @endif 태그를 사용합니다.  
한 문단이거나 foreach 내부에서는 `{{if` 와 `endif}}` 태그를 반드시 템플릿에 사용해야 합니다.
...
v0.5.0부터 POCO 또는 dynamic 파라미터를 지원합니다.
...
값의 타입이 `MiniWordHyperLink`일 경우, 시스템이 템플릿 문자열을 하이퍼링크로 대체합니다.

* Url： HyperLink URI 대상 경로
* Text：설명
