...
Template volgt het "WHAT you see is what you get"-ontwerp, en de stijlen van de sjabloontags worden volledig behouden.
...
- Invoer ondersteunt bestandsnaam, byte[]
- Uitvoer ondersteunt bestandsnaam, byte[], stream
...
MiniWord-sjabloonformaatstring zoals Vue, React `{{tag}}`, gebruikers hoeven er alleen voor te zorgen dat de tag en de sleutel van de value-parameter hetzelfde zijn, daarna zal het systeem ze automatisch vervangen.
...
var value = new Dictionary<string, object>()
{
    ...
    ["Purpose"] = "Locatie Shanghai heeft een nieuw systeem nodig om het HR-systeem te beheren.",
...
Waarde type is `MiniWordPicture`
...
### If-statement binnen sjabloon

Voor meerdere paragrafen, gebruik de @if en @endif tags.
Voor een enkele paragraaf en binnen foreach, gebruik de `{{if` en `endif}}` tags om te templaten is vereist.
...
##### Resultaat Voor Enkele Paragraaf
...
Als de waarde van het type `MiniWordHyperLink` is, zal het systeem de sjabloonstring vervangen door een hyperlink.
