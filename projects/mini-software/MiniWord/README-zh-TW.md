...
<div align="center">
 您的 <a href="https://github.com/mini-software/MiniWord">Star</a> 和 <a href="https://miniexcel.github.io">dotnate</a> 可以讓 MiniWord 更好
</div>
...
- nuget 連結 : https://www.nuget.org/packages/MiniWord

### 快速入門

範本遵循「所見即所得」設計，並且範本標籤樣式完全保留。
...
MiniWord 範本格式字串類似 Vue、React 的 `{{tag}}`，用戶只需確保標籤與 value 參數的 key 相同，系統就會自動替換。
...
標籤值為 `IEmerable<Dictionary<string,object>>` 類型
...
標籤值為 `IEnumerable<MiniWordForeach>` 類型。範本中需加入 `{{foreach` 與 `endforeach}}` 標籤。
...
對於多段落的情境，請使用 @if 與 @endif 標籤。
若為單一段落且在 foreach 內，需使用 `{{if` 及 `endif}}` 標籤。
...
若值的型別為 `MiniWordHyperLink`，系統會以超連結方式取代範本字串。
...
## 支持 : [捐贈連結](https://miniexcel.github.io/)
