...
<div align="center">
 您的<a href="https://github.com/mini-software/MiniWord">Star</a> 和 <a href="https://miniexcel.github.io">dotnate</a> 能讓 MiniWord 更好
</div>
...
Tag 值為 `IEmerable<Dictionary<string,object>>` 類型
...
標籤值為 `IEnumerable<MiniWordForeach>` 類型。必須在範本中添加 `{{foreach` 和 `endforeach}}` 標籤。
...
對於多段落，請使用 @if 和 @endif 標籤。
對於單一段落以及 foreach 內部，則必須使用 `{{if` 和 `endif}}` 標籤。
