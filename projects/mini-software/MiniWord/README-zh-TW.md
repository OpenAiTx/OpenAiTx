...
<div align="center">
 您的<a href="https://github.com/mini-software/MiniWord">Star</a> 和 <a href="https://miniexcel.github.io">捐贈</a>可以讓 MiniWord 變得更好
</div>
...
MiniWord 是一個簡單且高效的 .NET Word 範本函式庫。
...
MiniWord 範本格式字串類似於 Vue、React 的 `{{tag}}`，用戶只需確保標籤與 value 參數的 key 相同，系統就會自動進行替換。
...
tag value 為 `string[]` 或 `IList<string>` 型別
...
Tag value 為 `IEmerable<Dictionary<string,object>>` 型別
...
Tag value 為 `IEnumerable<MiniWordForeach>` 型別。範本中需加入 `{{foreach` 與 `endforeach}}` 標籤。
...
多段落請使用 @if 與 @endif 標籤。
單一段落與在 foreach 內時，請使用 `{{if` 與 `endif}}` 標籤，範本必須這樣寫。
