...
<div align="center">
 Ваша <a href="https://github.com/mini-software/MiniWord">звезда</a> и <a href="https://miniexcel.github.io">пожертвование</a> могут сделать MiniWord лучше
</div>
...
- nuget ссылка : https://www.nuget.org/packages/MiniWord

### Быстрый старт

Шаблон использует дизайн "что видишь, то и получаешь", а стили тегов шаблона полностью сохраняются.
...
- Вход поддерживает путь к файлу, byte[]
- Выход поддерживает путь к файлу, byte[], stream
...
Формат строки шаблона MiniWord как у Vue, React `{{tag}}`, пользователям нужно лишь убедиться, что имя тега и ключ параметра value совпадают, тогда система заменит их автоматически.
...
Значение тега — тип `string[]` или `IList<string>`
...
### Условные операторы внутри шаблона

Для нескольких абзацев используйте теги @if и @endif.
Для одного абзаца и внутри foreach используйте теги `{{if` и `endif}}`, чтобы шаблон был обязательным.
...
#### Экспорт через ASP.NET Core 3.1 API
