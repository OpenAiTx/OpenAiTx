...
<div align="center">
<p>Proyek ini merupakan bagian dari <a href="https://www.dotnetfoundation.org/">.NET Foundation</a> dan beroperasi di bawah <a href="https://www.dotnetfoundation.org/code-of-conduct">kode etik</a> mereka. </p>
</div>
...
Jika Anda melakukannya, pastikan juga untuk melihat [dokumentasi baru](https://raw.githubusercontent.com/mini-software/MiniExcel/master/README-V2.md) dan [catatan upgrade](https://raw.githubusercontent.com/mini-software/MiniExcel/master/V2-Upgrade-Notes.md).
...
- [Nama/Indeks Kolom Excel/Attribute Abaikan](#getstart4)
...
Anda dapat menginstal paket [dari NuGet](https://www.nuget.org/packages/MiniExcel)

### Catatan Rilis

Silakan cek [Catatan Rilis](docs)
...
File yang digunakan untuk menguji performa adalah [**Test1,000,000x10.xlsx**](https://raw.githubusercontent.com/mini-software/MiniExcel/master/benchmarks/MiniExcel.Benchmarks/Test1%2C000%2C000x10.xlsx), sebuah dokumen 32MB yang berisi 1.000.000 baris * 10 kolom yang setiap selnya diisi string "HelloWorld".
...
```
#### 8. Dynamic Query mengubah baris menjadi `IDictionary<string,object>`
...
1. Harus merupakan tipe non-abstrak dengan konstruktor publik tanpa parameter.

2. MiniExcel mendukung parameter IEnumerable Deferred Execution, Jika Anda ingin menggunakan memori seminimal mungkin, mohon jangan gunakan metode seperti ToList
...
Fungsi ini hanya didukung dalam format `xlsx` dan menggabungkan sel secara vertikal di antara tag @merge dan @endmerge.
...
MiniExcel.SaveAs(@"C:\temp\Book1.xlsx", dt, configuration: configuration);
```
![gambar](https://user-images.githubusercontent.com/31481586/241419441-c4f27e8f-3f87-46db-a10f-08665864c874.png)
...
### Isi Data ke Template Excel <a name="getstart3"></a>
...
3. Setiap pernyataan harus baris baru.
...
##### 1. Contoh
Awali rumus Anda dengan `$` dan gunakan `$enumrowstart` serta `$enumrowend` untuk menandai referensi ke baris awal dan akhir enumerasi:
...
|              |                                                                                           |
|--------------|-------------------------------------------------------------------------------------------|
| Jumlah       | `$=SUM(C{{$enumrowstart}}:C{{$enumrowend}})`                                              |
| Alt. Rata-rata | `$=SUM(C{{$enumrowstart}}:C{{$enumrowend}}) / COUNT(C{{$enumrowstart}}:C{{$enumrowend}})`|
| Rentang      | `$=MAX(C{{$enumrowstart}}:C{{$enumrowend}}) - MIN(C{{$enumrowstart}}:C{{$enumrowend}})`   |
...
### Atribut Nama Kolom/Indeks/Abaikan pada Excel <a name="getstart4"></a>
...
### Pemeriksaan Otomatis Tipe Excel <a name="getstart5"></a>
...
#### 3. Konversi Excel ke PDF

Jika Anda perlu mengonversi file Excel ke PDF, Anda dapat menggunakan [MiniPdf](https://github.com/mini-software/MiniPdf).

#### 4. Kustomisasi CultureInfo

Sejak versi 1.22.0, Anda dapat mengkustomisasi CultureInfo seperti di bawah ini, default sistem adalah `CultureInfo.InvariantCulture`.
...
#### 7. Tambah Gambar Secara Massal (MiniExcel.AddPicture)

Silakan tambahkan gambar sebelum membuat data baris secara massal, atau sistem akan menggunakan memori besar saat memanggil AddPicture.
...
Seperti pada contoh, buatlah sebuah metode untuk menangani i18n dan manajemen izin, dan gunakan `yield return untuk mengembalikan IEnumerable<Dictionary<string, object>>` agar mendapatkan efek pemrosesan yang dinamis dan hemat memori
...
#### Q. Bagaimana cara mengisi data secara horizontal (kiri-ke-kanan) dengan template?

A. Perenderan koleksi template MiniExcel berkembang secara vertikal (atas-ke-bawah). Pengisian horizontal (kiri-ke-kanan) belum didukung (lihat https://github.com/mini-software/MiniExcel/issues/619).

Jika Anda hanya membutuhkan tata letak akhir, transpose data Anda ke dalam matriks dan ekspor dengan `printHeader: false`:
...
Jika Anda harus menggunakan template untuk styling, salah satu opsinya adalah menggunakan placeholder skalar (mis. `{{Name_1}}`, `{{Name_2}}` ...) dan mengisi kamus (memerlukan jumlah kolom maksimum yang tetap).


#### Q. Apakah penggunaan Count akan memuat semua data ke dalam memori?

Tidak, pengujian gambar memiliki 1 juta baris*10 kolom data, penggunaan memori maksimum <60MB, dan memerlukan waktu 13,65 detik

![image](https://user-images.githubusercontent.com/12729184/117118518-70586000-adc3-11eb-9ce3-2ba76cf8b5e5.png)

#### Q. Bagaimana Query menggunakan indeks integer?

Indeks default Query adalah string Key: A,B,C.... Jika Anda ingin mengubah menjadi indeks numerik, silakan buat metode berikut untuk mengonversi
...
```
#### Q. Tidak ada judul, Excel kosong dihasilkan ketika nilai kosong saat mengekspor Excel

Karena MiniExcel menggunakan logika yang mirip dengan JSON.NET untuk secara dinamis mendapatkan tipe dari nilai guna menyederhanakan operasi API, tipe tidak dapat diketahui tanpa data. Anda dapat melihat [issue #133](https://github.com/mini-software/MiniExcel/issues/133) untuk pemahaman.
...
Terima kasih telah menyediakan All product IDE secara gratis untuk proyek ini ([Lisensi](https://user-images.githubusercontent.com/12729184/123988233-6ab17c00-d9fa-11eb-8739-2a08c6a4a263.png))
