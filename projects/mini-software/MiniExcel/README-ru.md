...
В настоящее время большинство популярных фреймворков требуют загрузки всех данных из Excel-документа в память для облегчения работы, однако это может привести к проблемам с потреблением памяти. Подход MiniExcel отличается: данные обрабатываются построчно в потоковом режиме, что снижает исходное потребление с потенциальных сотен мегабайт до всего лишь нескольких мегабайт, эффективно предотвращая проблемы с нехваткой памяти (OOM).
...
- Обеспечивает выполнение операций с данными на уровне строк в реальном времени для повышения производительности при работе с большими наборами данных
- Поддерживает LINQ с отложенным выполнением, что позволяет быстро и эффективно использовать память при постраничной выборке и выполнении сложных запросов
- Легковесный, не требует Microsoft Office или компонентов COM+, размер DLL менее 500KB
- Простой и интуитивно понятный API-стиль для чтения/записи/заполнения excel
...
Вы можете установить пакет [с помощью NuGet](https://www.nuget.org/packages/MiniExcel)

### Примечания к релизу

Пожалуйста, ознакомьтесь с [Примечаниями к релизу](docs)
...
Исходный код для тестирования производительности находится в [MiniExcel.Benchmarks](https://raw.githubusercontent.com/mini-software/MiniExcel/master/benchmarks/MiniExcel.Benchmarks/Program.cs).

Файл, используемый для тестирования производительности — [**Test1,000,000x10.xlsx**](https://raw.githubusercontent.com/mini-software/MiniExcel/master/benchmarks/MiniExcel.Benchmarks/Test1%2C000%2C000x10.xlsx), 32MB документ, содержащий 1 000 000 строк * 10 столбцов, в ячейках которых записано "HelloWorld".
...
```
Вы можете найти результаты тестов производительности для последнего релиза [здесь](benchmarks/results).
...
Рекомендуется использовать Stream.Query из-за более высокой эффективности.
...
#### 2. Выполнить запрос и сопоставить его со списком динамических объектов без использования заголовка [[Попробовать]](https://dotnetfiddle.net/w5WD1J)
...
примечание: при одинаковых названиях столбцов используется последний правый
...
```
#### 4. Поддержка запросов LINQ Extension First/Take/Skip ...и т.д.
...
```
#### 9. Запрос Excel возвращает DataTable

Не рекомендуется, так как DataTable загрузит все данные в память и потеряется преимущество MiniExcel по низкому потреблению памяти.
...
#### 10. Укажите ячейку, с которой начинать чтение данных
...
Причина: Стандарт OpenXml размещает mergeCells внизу файла, что приводит к необходимости дважды проходить по sheetxml
...
поддержка заполнения переменной длины и ширины, многострочного и многоколончатого режима

![image](https://user-images.githubusercontent.com/12729184/117973820-6d2f1800-b35f-11eb-88d8-555063938108.png)

#### 12. Чтение большого файла с помощью кэширования на диске (Disk-Base Cache - SharedString)

Если размер SharedStrings превышает 5 МБ, MiniExcel по умолчанию использует кэширование на локальном диске, например, [10x100000.xlsx](https://github.com/MiniExcel/MiniExcel/files/8403819/NotDuplicateSharedStrings_10x100000.xlsx) (данные из миллиона строк), при отключённом кэшировании максимальное использование памяти составляет 195 МБ, но при включённом кэшировании требуется только 65 МБ. Обратите внимание, что эта оптимизация требует некоторых затрат на производительность, поэтому в этом случае время чтения увеличивается с 7,4 секунд до 27,2 секунд. Если вам это не нужно, вы можете отключить кэширование на диске с помощью следующего кода:
...
2. MiniExcel поддерживает параметр IEnumerable с отложенным выполнением. Если вы хотите использовать минимум памяти, пожалуйста, не вызывайте методы, такие как ToList

например: ToList или без использования памяти
...
#### 3.  IDataReader
- `Рекомендуется`, так как позволяет избежать загрузки всех данных в память
...
Экспорт нескольких листов с помощью DataReader (рекомендуется Dapper ExecuteReader)
...
- `Не рекомендуется`, так как вся информация будет загружена в память

- DataTable сначала использует Caption для имени столбца, затем использует columname
...
Начиная с v0.19.0  `OpenXmlConfiguration.AutoFilter` может включать/отключать автофильтр, значение по умолчанию — `true`, способ установки AutoFilter:
...
Начиная с версии 1.22.0, если тип значения — `byte[]`, система по умолчанию сохраняет путь к файлу в ячейке, а при импорте может преобразовать его в `byte[]`. Если вы не хотите использовать эту функцию, установите `OpenXmlConfiguration.EnableConvertByteArray` в `false`, это повысит эффективность системы.
...
Вы можете использовать @mergelimit для ограничения области вертикального объединения ячеек.
...
- Объявление аналогично шаблону Vue `{{имя переменной}}`, или отрисовке коллекции `{{имя коллекции.имя поля}}`
- Отрисовка коллекции поддерживает IEnumerable/DataTable/DapperRow
...
> Примечание 1: Используйте первый IEnumerable того же столбца в качестве основы для заполнения списка
...
Правила:
1. Поддерживаются DateTime, Double, Int с операторами ==, !=, >, >=, <, <=.
2. Поддерживаются String с операторами ==, !=.
3. Каждое выражение должно быть с новой строки.
4. Одиночный пробел должен быть до и после операторов.
5. Не должно быть новой строки внутри выражения.
...
##### 1. Пример
Префиксируйте вашу формулу знаком `$` и используйте `$enumrowstart` и `$enumrowend` для обозначения ссылок на начальную и конечную строки перечисления:
...
Начиная с V1.24.0, по умолчанию отсутствующие ключи параметров шаблона игнорируются и заменяются на пустую строку, параметр `IgnoreTemplateParameterMissing` может контролировать выбрасывать ли исключение.
...
### Атрибуты имени столбца Excel/индекса/игнорирования <a name="getstart4"></a>
...
Начиная с версии V0.21.0 поддерживается класс, содержащий метод формата `ToString(string content)`
...
Начиная с версии V1.26.0, мы можем динамически задавать атрибуты столбца
...
v1.28.0 поддерживает вставку данных из CSV с N строками после последней строки
...
- MiniExcel по умолчанию определяет, является ли файл xlsx или csv, на основе `расширения файла`, но это может быть неточно, пожалуйста, указывайте вручную.
- Для потока нельзя определить тип excel, пожалуйста, указывайте его вручную.
...
- По умолчанию возвращается тип `string`, и значение не будет преобразовано в числа или дату/время, если только тип не определён с помощью строгой типизации через дженерики.
...
```
С версии V1.30.1 добавлена поддержка пользовательского разделителя (спасибо @hyzx86)
...
- Если у вас есть индивидуальные требования к кодировке, пожалуйста, измените свойство StreamReaderFunc / StreamWriterFunc
...
- v0.17.0 поддерживает асинхронность (благодаря isdaniel ( SHIH,BING-SIOU)](https://github.com/isdaniel))
</translate-content>
```csharp
...
Убедитесь, что имена в Excel и свойства совпадают, система выполнит автоматическое сопоставление (без учета регистра)

![image](https://user-images.githubusercontent.com/12729184/116210595-9784b100-a775-11eb-936f-8e7a8b435961.png)

Начиная с V0.18.0 поддерживается Enum Description
...
Начиная с версии 1.22.0, вы можете задать пользовательский CultureInfo, по умолчанию используется системный `CultureInfo.InvariantCulture`.
...
Пожалуйста, добавляйте изображения до пакетной генерации данных строк, иначе система будет использовать много памяти при вызове AddPicture.
...
#### 1. SQLite и Dapper `Большой файл` Вставка SQL без переполнения памяти (OOM)
...
#### 2. ASP.NET Core 3.1 или MVC 5 Скачать/Загрузить Excel Xlsx API Демонстрация [Попробовать](https://raw.githubusercontent.com/mini-software/MiniExcel/master/tests/MiniExcel.Tests.AspNetCore)
...
#### 5. Динамическое многоязычие i18n и управление ролями и правами

Как в примере, создайте метод для обработки i18n и управления правами, и используйте `yield return` для возврата IEnumerable<Dictionary<string, object>> с целью достижения динамической обработки с низким потреблением памяти
</translate-content>
```csharp
void Main()
{
...
Нет, тест с изображением содержит 1 миллион строк*10 столбцов данных, максимальное использование памяти составляет <60 МБ, время выполнения — 13,65 секунд

![image](https://user-images.githubusercontent.com/12729184/117118518-70586000-adc3-11eb-9ce3-2ba76cf8b5e5.png)

#### В. Как Query использует целочисленные индексы?

Индекс по умолчанию для Query — это строковый ключ: A,B,C.... Если вы хотите использовать числовой индекс, создайте следующий метод для преобразования
...
Поскольку MiniExcel использует логику, схожую с JSON.NET, для динамического определения типа из значений с целью упрощения работы с API, тип невозможно определить без данных. Для подробностей смотрите [issue #133](https://github.com/mini-software/MiniExcel/issues/133).

![image](https://user-images.githubusercontent.com/12729184/122639771-546c0c00-d12e-11eb-800c-498db27889ca.png)

> Строго типизированные объекты и DataTable сгенерируют заголовки, но Dictionary по-прежнему дадут пустой Excel

#### В. Как остановить foreach при пустой строке?

MiniExcel можно использовать с `LINQ TakeWhile` для остановки итерации foreach.
...
Пожалуйста, используйте класс Stream для настройки логики создания файла, например:
</translate-content>
```C#
...
- xlsm поддерживается только для Query
