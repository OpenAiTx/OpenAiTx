...
<div align="center">
 Suas <a href="https://github.com/mini-software/MiniExcel">Stars</a> ou <a href="https://miniexcel.github.io">Doações</a> podem tornar o MiniExcel melhor
</div>
...
- Permite operações de dados em tempo real no nível de linha para melhor desempenho em grandes conjuntos de dados
- Suporta LINQ com execução diferida, permitindo paginação rápida e eficiente em memória, além de consultas complexas
- Leve, sem necessidade de Microsoft Office ou componentes COM+, e com DLL de tamanho inferior a 500KB
- API simples e intuitiva para ler/escrever/preencher excel

### Versão 2.0 preview

Estamos trabalhando em uma futura versão do MiniExcel, com uma nova API modular e focada,
pacotes nuget separados para funcionalidades Core e Csv, suporte completo para consultas transmitidas de forma assíncrona via `IAsyncEnumerable`,
...
Se fizer isso, não deixe de conferir também a [nova documentação](https://raw.githubusercontent.com/mini-software/MiniExcel/master/README-V2.md) e as [notas de atualização](https://raw.githubusercontent.com/mini-software/MiniExcel/master/V2-Upgrade-Notes.md).
...
- [Modelo de Excel](#getstart3)

- [Nome/Índice de Coluna Excel/Atributo Ignorar](#getstart4)
...
### Consulta/Importação do Excel  <a name="getstart1"></a>
...
Se o tamanho de SharedStrings exceder 5 MB, o MiniExcel usará por padrão o cache em disco local, por exemplo, [10x100000.xlsx](https://github.com/MiniExcel/MiniExcel/files/8403819/NotDuplicateSharedStrings_10x100000.xlsx) (um milhão de linhas de dados), ao desabilitar o cache em disco o uso máximo de memória é de 195MB, mas com o cache em disco habilitado são necessários apenas 65MB. Observe que essa otimização tem um custo de eficiência, então neste caso o tempo de leitura aumentará de 7,4 segundos para 27,2 segundos. Se você não precisar disso, pode desabilitar o cache em disco com o seguinte código:
...
2. O MiniExcel suporta execução adiada de parâmetro IEnumerable. Se você deseja usar menos memória, por favor, não chame métodos como ToList
...
```
![imagem](https://user-images.githubusercontent.com/31481586/241419455-3c0aec8a-4e5f-4d83-b7ec-6572124c165d.png)
...
MiniExcel.SaveAs(@"C:\temp\Book1.xlsx", dt, configuration: configuration);
```
![imagem](https://user-images.githubusercontent.com/31481586/241419441-c4f27e8f-3f87-46db-a10f-08665864c874.png)
...
### Preencher Dados em Modelo do Excel <a name="getstart3"></a>
...
![imagem](https://user-images.githubusercontent.com/12729184/114577091-5046ec80-9cae-11eb-924b-087c7becf8da.png)
...
![imagem](https://user-images.githubusercontent.com/12729184/114802504-64830a80-9dd0-11eb-8d56-8e8c401b3ace.png)

Resultado

![imagem](https://user-images.githubusercontent.com/12729184/114802419-43221e80-9dd0-11eb-9ffe-a2ce34fe7076.png)
...
3. Cada declaração deve estar em uma nova linha.
4. Deve ser adicionado um espaço simples antes e depois dos operadores.
5. Não deve haver nova linha dentro das declarações.
...
##### 1. Exemplo
Prefixe sua fórmula com `$` e use `$enumrowstart` e `$enumrowend` para marcar as referências ao início e ao fim das linhas enumeráveis:
...
| Média Alter. | `$=SUM(C{{$enumrowstart}}:C{{$enumrowend}}) / COUNT(C{{$enumrowstart}}:C{{$enumrowend}})` |
...
### Nome da Coluna do Excel/Índice/Ignorar Atributo <a name="getstart4"></a>
...
#### 2. Demonstração da API ASP.NET Core 3.1 ou MVC 5 Download/Upload Excel Xlsx [Experimente](https://raw.githubusercontent.com/mini-software/MiniExcel/master/tests/MiniExcel.Tests.AspNetCore)
...
#### P. Como preencher dados horizontalmente (da esquerda para a direita) com templates?

R. A renderização da coleção de templates do MiniExcel expande verticalmente (de cima para baixo). O preenchimento horizontal (da esquerda para a direita) ainda não é suportado (veja https://github.com/mini-software/MiniExcel/issues/619).

Se você só precisa do layout final, transponha seus dados para uma matriz e exporte usando `printHeader: false`:
...
```
Se você precisar usar um modelo para estilização, uma opção é utilizar marcadores escalares (por exemplo, `{{Name_1}}`, `{{Name_2}}` ...) e preencher um dicionário (requer um número máximo fixo de colunas).


#### P. Utilizar Count irá carregar todos os dados na memória?
...
#### P. Como o Query utiliza índices inteiros?

O índice padrão do Query é a chave string: A,B,C.... Se desejar alterar para índice numérico, por favor crie o seguinte método para converter
...
```
#### P. Nenhum título, Excel vazio é gerado quando o valor está vazio ao exportar para Excel

Como o MiniExcel utiliza uma lógica semelhante ao JSON.NET para obter dinamicamente o tipo dos valores a fim de simplificar as operações da API, o tipo não pode ser conhecido sem dados. Você pode conferir [issue #133](https://github.com/mini-software/MiniExcel/issues/133) para entender.
...
Agradecemos por fornecer um All product IDE gratuito para este projeto ([Licença](https://user-images.githubusercontent.com/12729184/123988233-6ab17c00-d9fa-11eb-8739-2a08c6a4a263.png))
