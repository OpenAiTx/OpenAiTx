...
ExcelファイルをPDFに変換する必要がある場合は、[MiniPdf](https://github.com/mini-software/MiniPdf)を使用できます。
...
システムはメモリを制御しませんが、保存速度を速くすることができます。
...
#### 7. バッチ画像追加 (MiniExcel.AddPicture)

画像は、行データをバッチ生成する前に追加してください。そうしないと、AddPicture呼び出し時にシステムが大量のメモリを消費します。
