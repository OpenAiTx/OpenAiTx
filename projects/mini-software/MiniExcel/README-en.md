...
MiniExcel is a simple and efficient .NET Excel processing tool that avoids OOM.

Currently, most popular frameworks need to load all the data into memory for operation, which causes memory consumption issues. MiniExcel attempts to use stream-based algorithms to reduce the original 1000 MB usage to just a few MB, thereby avoiding OOM (out of memory) situations.

![image](https://user-images.githubusercontent.com/12729184/113086657-ab8bd000-9214-11eb-9563-c970ac1ee35e.png)

### Features

- Low memory usage, avoids OOM (out of memory) and full GC
- Supports real-time processing of each row of data
- Supports LINQ deferred execution for low-consumption, fast paging, and other complex queries
- Lightweight, does not require Microsoft Office installation, no COM+, DLL size less than 400KB
- Simple API for reading/writing/filling Excel files
...
#### 2. Execute a query and map it to a list of dynamic objects without using a header [[Try it]](https://dotnetfiddle.net/w5WD1J)
...
note: same column name uses the last right one
...
#### 4. Query supports LINQ extensions First/Take/Skip etc.
...
Not recommended, because DataTable will load all data into memory and lose MiniExcel's low memory usage feature.
...
Reason: The OpenXml standard puts mergeCells at the bottom of the file, which requires iterating through the sheetxml twice
...
Supports variable length and width multi-row and column filling

![image](https://user-images.githubusercontent.com/12729184/117973820-6d2f1800-b35f-11eb-88d8-555063938108.png)

#### 12. Reading big file by disk-based cache (Disk-Base Cache - SharedString)

If the SharedStrings size exceeds 5 MB, MiniExcel by default will use local disk cache, e.g, [10x100000.xlsx](https://github.com/MiniExcel/MiniExcel/files/8403819/NotDuplicateSharedStrings_10x100000.xlsx) (one million rows data). When disk cache is disabled, the maximum memory usage is 195MB, but with disk cache, only 65MB is needed. Note, this optimization comes at some efficiency cost, so reading time increases from 7.4 seconds to 27.2 seconds. If you don't need it, you can disable disk cache with the following code:
...
You can use `SharedStringCacheSize` to change the sharedString file size threshold for disk caching
```csharp
...
2. MiniExcel supports IEnumerable deferred execution. If you want to use the least memory, please do not call methods such as ToList.
...
#### 3.  IDataReader
- `Recommended`, as it avoids loading all data into memory
```csharp
...
DataReader export multiple sheets (recommended by Dapper ExecuteReader)
...
- DataTable uses Caption for column name first, then uses column name
...
The code below will load all data into memory
...
```csharp
using (var stream = new MemoryStream()) // supports FileStream, MemoryStream, etc.
{
...
Since v0.19.0, `OpenXmlConfiguration.AutoFilter` can enable/disable AutoFilter, default value is `true`, and set AutoFilter this way:
...
Since 1.22.0, when the value type is `byte[]`, the system will save the file path in the cell by default, and when importing, it can be converted to `byte[]`. If you don't want to use this, you can set `OpenXmlConfiguration.EnableConvertByteArray` to `false`, which can improve system efficiency.
...
> Note1: Uses the first IEnumerable of the same column as the basis for filling the list
...
> Note: Supports multi-sheets and using the same variable
...
> NOTE: Using IEnumerable deferred execution instead of ToList can save maximum memory usage in MiniExcel
...
3. Each statement should be a new line.
4. Single space should be added before and after operators.
5. There shouldn't be a new line inside statements.
6. Cell should be in the exact format as below.
...
Since V1.24.0, by default, missing template parameter keys are ignored and replaced with empty strings. `IgnoreTemplateParameterMissing` can control whether to throw an exception.
...
public class ExcelAttributeDemo
{
    ...
    public string Test5 { get; } //without set will ignore
    public string Test6 { get; private set; } //non-public set will ignore
    [ExcelColumnIndex(3)] // starts with 0
...
Since V0.21.0, supports classes containing `ToString(string content)` method for formatting
...
Since 1.24.0, the system supports System.ComponentModel.DisplayNameAttribute = ExcelColumnName.excelColumnNameAttribute
...
Since V1.26.0, you can set the attributes of Column dynamically
```csharp
            var config = new OpenXmlConfiguration
            {
...
Since V1.31.4, you can set the attributes of Sheet dynamically. You can set sheet name and state (visibility).
```csharp
...
You can also use the new attribute ExcelSheetAttribute:
...
v1.28.0 supports CSV insert N rows data after the last row
...
v1.37.0 supports inserting a new sheet into an existing workbook
...
- MiniExcel will check whether it is xlsx or csv based on the `file extension` by default, but there may be inaccuracies, so please specify it manually.
- Stream type cannot be determined, so please specify it manually.
...
- By default, returns `string` type, and values will not be converted to numbers or datetime, unless the type is defined by strong typing generic.
...
Since V1.30.1 supports function for custom separator (thanks @hyzx86)
...
- v0.17.0 supports Async (thanks isdaniel ( SHIH,BING-SIOU)](https://github.com/isdaniel))
...
Make sure excel & property names are the same, the system will auto map (case insensitive)

![image](https://user-images.githubusercontent.com/12729184/116210595-9784b100-a775-11eb-936f-8e7a8b435961.png)

Since V0.18.0 supports Enum Description
...
Since 1.30.0 version supports excel Description to Enum, thanks @KaneLeung
...
Since 1.22.0, you can customize CultureInfo as below, system default is `CultureInfo.InvariantCulture`.
...
note: please don't call ToList/ToArray methods after Query, as it'll load all data into memory
...
#### Q: Excel header title not equal class property name, how to map?
...
A. Use `GetSheetNames` method with  Query  sheetName parameter.
...
A. Use `GetSheetInformations` method.
...
#### Q. Does using Count load all data into memory?

No, the test image has 1 million rows * 10 columns of data, maximum memory usage is <60MB, and it takes 13.65 seconds

![image](https://user-images.githubusercontent.com/12729184/117118518-70586000-adc3-11eb-9ce3-2ba76cf8b5e5.png)

#### Q. How does Query use integer indexes?

The default index of Query is the string Key: A,B,C.... If you want to switch to numeric index, please create the following method to convert
...
#### Q. Why is a title-less empty excel generated when the value is empty when exporting Excel?

Because MiniExcel uses logic similar to JSON.NET to dynamically get type from values to simplify API operations, type cannot be known without data. See [issue #133](https://github.com/mini-software/MiniExcel/issues/133) for understanding.

![image](https://user-images.githubusercontent.com/12729184/122639771-546c0c00-d12e-11eb-800c-498db27889ca.png)

> Strong type & DataTable will generate headers, but Dictionary are still empty Excel

#### Q. How to stop the foreach when a blank row is encountered?
...
Please use the Stream class to customize file creation logic, e.g.:
...
Or, since V1.25.0, SaveAs supports overwriteFile parameter to enable/disable overwriting an existing file
...
- Does not support xls and encrypted files now
