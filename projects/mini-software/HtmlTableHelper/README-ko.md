...
##### List/Array/Set/Enumerable Key/Value 타입이 아닌 예제
```C#
...
<!--
**그러나** 또 다른 Key/Value 타입은 `ToHtmlTableByDictionary` 사용 필요
```C#
var sourceData = new[] {
...
**테이블/TR/TD/TH 속성 커스텀 (Dynamic Type)**

```C#
var data = /*List/Array/Set/Enumrable..*/;
var html = data.ToHtmlTable( tableAttributes: new { @class = "SomeClass"} //이 타입은 dynamic으로, 모든 속성 지원
...
{
    [TableColumn(DisplayName = "Column1")] //MyProperty1 속성은 thead-td의 innertext로 "Column1"을 렌더링
    public string MyProperty1 { get; set; }
    [TableColumn(DisplayName = "Column2")] //MyProperty2 속성은 thead-td의 innertext로 "Column2"를 렌더링
...
public class ModelClassWithSkipAttr
{
    ...
    public string MyProperty1 { get; set; } //MyProperty1은 html 렌더링에서 제외됨
    public string MyProperty2 { get; set; }
}
```
...
내부 HTML 인코딩 설정 가능(특별한 이유가 없다면 비권장, XSS 공격 위험)
```C#
