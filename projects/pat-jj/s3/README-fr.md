...
# s3 - Formation Efficace d’Agent de Recherche par RL
***Vous n'avez pas besoin d'autant de données pour entraîner un agent de recherche***
...
`s3` est un cadre simple mais puissant pour entraîner des agents de recherche dans la génération augmentée par récupération (RAG). Il apprend aux modèles de langage à rechercher plus efficacement — sans modifier le générateur lui-même. En se concentrant uniquement sur le composant de recherche, `s3` atteint de solides performances sur les tâches de questions-réponses avec seulement une fraction des données utilisées par les méthodes précédentes. Il est modulaire, efficace et conçu pour fonctionner sans accroc avec tout LLM en boîte noire.
...
- [🔍 Exécuter Recherche/Récupération](https://github.com/pat-jj/s3?tab=readme-ov-file#-run-searchretrieval)
- [📈 Exécuter Évaluation](#-run-evaluation)

## 📦 Installation

**Environnement Chercheur & Générateur**
...
```
***Pré-calcul de l'initialisation naïve du RAG***
