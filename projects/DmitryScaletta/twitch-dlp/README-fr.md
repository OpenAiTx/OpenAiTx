...
Téléchargez n'importe quel VOD Twitch depuis le début pendant la diffusion en direct

## Fonctionnalités

- Téléchargez les VOD en direct depuis le début (`--live-from-start`)
- Téléchargez les VOD cachés en cours (ou s'ils ont été cachés pendant la diffusion)
- Téléchargez les VOD cachés terminés
  - Utilisez simplement les liens de [twitchtracker.com](https://twitchtracker.com), [streamscharts.com](https://streamscharts.com) ou [sullygnome.com](https://sullygnome.com) ([détails](https://github.com/DmitryScaletta/twitch-dlp/blob/master/DOWNLOAD_PRIVATE_VIDEOS.md))
- Téléchargez une partie spécifique de la vidéo (`--download-sections`)
- Téléchargez des clips (y compris les versions portrait)
- Désactivez automatiquement le mode silencieux des sections muettes si possible
- Reprenez le téléchargement d'une vidéo partiellement téléchargée (en cas de coupure réseau/électrique)
- Surveillez le statut de la chaîne. Si elle devient en direct, commencez le téléchargement (`--retry-streams DELAY`)
- Syntaxe similaire à `yt-dlp` (`youtube-dl`)

## Utilisation

Installez la dernière version de [Node.js](https://nodejs.org/) (v22 ou plus récente).
...
└─────────┴──────────────┴──────────────┴──────┴───────────────┴────────┘
```
Pour les flux en direct (streamlink)
...
- **ffmpeg**
- **streamlink** (si téléchargement par lien de chaîne sans `--live-from-start`)
