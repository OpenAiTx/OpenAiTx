...
在直播期间从头开始下载任何 Twitch VOD

## 功能

- 从头开始下载直播 VOD（`--live-from-start`）
- 下载正在进行的隐藏 VOD（或在直播期间被隐藏的）
- 下载已结束的隐藏 VOD
  - 只需使用 [twitchtracker.com](https://twitchtracker.com)、[streamscharts.com](https://streamscharts.com) 或 [sullygnome.com](https://sullygnome.com) 链接（[详情](https://github.com/DmitryScaletta/twitch-dlp/blob/master/DOWNLOAD_PRIVATE_VIDEOS.md)）
- 下载视频的特定部分（`--download-sections`）
- 下载剪辑（包括竖屏版本）
- 可能时自动取消静音静音部分
- 继续下载部分下载的视频（以防网络/断电中断）
- 监控频道状态。如果变为直播，开始下载（`--retry-streams DELAY`）
- 类似于 `yt-dlp`（`youtube-dl`）的语法

## 使用方法

安装最新的 [Node.js](https://nodejs.org/) 版本（v22 或更高）。
...
- **ffmpeg**
- **streamlink**（如果通过频道链接下载且不使用 `--live-from-start`）
