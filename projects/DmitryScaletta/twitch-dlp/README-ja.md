...
ライブ配信中のTwitch VODを開始からダウンロード

## 特徴

- ライブVODを開始からダウンロード（`--live-from-start`）
- 進行中の非公開VOD（または放送中に非公開になったもの）をダウンロード
- 終了した非公開VODをダウンロード
  - [twitchtracker.com](https://twitchtracker.com)、[streamscharts.com](https://streamscharts.com)、または [sullygnome.com](https://sullygnome.com) のリンクを使用（[詳細](https://github.com/DmitryScaletta/twitch-dlp/blob/master/DOWNLOAD_PRIVATE_VIDEOS.md)）
- 動画の特定部分をダウンロード（`--download-sections`）
- クリップをダウンロード（縦長バージョンも含む）
- 可能であればミュートされた部分を自動的にミュート解除
- 部分的にダウンロードされた動画の続きからダウンロード（ネットワークや電源障害時に便利）
- チャンネルの状態を監視。ライブになったらダウンロード開始（`--retry-streams DELAY`）
- `yt-dlp`（`youtube-dl`）に似た構文

## 使い方

最新の [Node.js](https://nodejs.org/) バージョン（v22以降）をインストールしてください。
...
- **ffmpeg**
- **streamlink**（`--live-from-start`なしでチャンネルリンクからダウンロードする場合）
