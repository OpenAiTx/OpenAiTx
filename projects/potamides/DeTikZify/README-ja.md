# DeTi*k*Zify<br><sub><sup>科学図表やスケッチのためのTi*k*Zグラフィックスプログラムの合成</sup></sub>
...
高品質な科学図を作成することは時間がかかり困難な場合が多いですが、アイデアを紙にスケッチすることは比較的簡単です。さらに、意味情報を保持する形式で保存されていない既存の図を再作成することも同様に複雑です。この問題に対処するために、私たちはスケッチや既存の図に基づいて意味を保持する[Ti*k*Z](https://github.com/pgf-tikz/pgf)グラフィックスプログラムとして科学図を自動合成する新しいマルチモーダル言語モデル[DeTi*k*Zify](https://github.com/potamides/DeTikZify)を紹介します。また、DeTi*k*Zifyが追加の訓練なしで出力を反復的に洗練できるMCTSベースの推論アルゴリズムも導入します。

https://github.com/potamides/DeTikZify/assets/53401822/203d2853-0b5c-4a2b-9d09-3ccb65880cd3

## ニュース
* **2025-06-30**: 最新モデル[DeTi*k*Zify<sub>v2.5</sub>
  (8b)](https://huggingface.co/nllg/detikzify-v2.5-8b)をリリースしました。これは[DeTi*k*Zify<sub>v2</sub>](https://huggingface.co/nllg/detikzify-v2-8b)に[自己フィードバックによる強化学習](https://huggingface.co/nllg/detikzify-v2.5-8b#reinforcement-learning-from-self-feedback) (RLSF)を適用したものです。私たちの[Hugging Face Space](https://huggingface.co/spaces/nllg/DeTikZify)で新しいデフォルトモデルに設定しています。詳細は[モデルカード](https://huggingface.co/nllg/detikzify-v2-8b-preview#model-card-for-detikzifyv2-8b)をご覧ください。
* **2025-06-26**: Ti*k*Zeroが[ICCV 2025](https://iccv.thecvf.com)の[ハイライト論文](https://iccv.thecvf.com/virtual/2025/poster/51)に採択されました！
* **2025-03-17**: [DeTi*k*Zify<sub>v2</sub>
  (8b)](https://huggingface.co/nllg/detikzify-v2-8b)に直接プラグインできる
  [Ti*k*Zero](https://huggingface.co/nllg/tikzero-adapter)アダプターと
  追加のエンドツーエンド微調整を加えた[Ti*k*Zero+](https://huggingface.co/nllg/tikzero-plus-10b)をリリースしました。詳細は[論文](https://arxiv.org/abs/2503.11509)と[使用例](#usage)をご覧ください。
* **2024-12-05**: これまでのバージョンを超える性能を持つ新しいDeTi*k*Zifyモデル
  [DeTi*k*Zify<sub>v2</sub> (8b)](https://huggingface.co/nllg/detikzify-v2-8b)をリリースしました。詳細は[モデルカード](https://huggingface.co/nllg/detikzify-v2-8b-preview#model-card-for-detikzifyv2-8b)をご覧ください。
* **2024-09-24**: DeTi*k*Zifyが[NeurIPS 2024](https://neurips.cc/Conferences/2024)の[スポットライト論文](https://neurips.cc/virtual/2024/poster/94474)に採択されました！

## インストール

> [!TIP]
> ご自身のハードウェアでのインストールや推論に問題がある場合は、私たちの[Hugging Face Space](https://huggingface.co/spaces/nllg/DeTikZify)を利用することを検討してください（スペースの再起動には最大30分かかる場合があります）。長い待ち行列が発生した場合は、有料のプライベートGPUランタイムで[複製](https://huggingface.co/spaces/nllg/DeTikZify?duplicate=true)するか、Dockerを使って[ローカルで実行](https://huggingface.co/spaces/nllg/DeTikZify?docker=true)することも可能です。また、[Google Colab](https://colab.research.google.com/drive/1hPWqucbPGTavNlYvOBvSNBAwdcPZKe8F)でのデモも試せますが、環境構築に時間がかかることがあり、無料枠では1bモデルの推論のみサポートされています。

DeTi*k*ZifyのPythonパッケージは[pip](https://pip.pypa.io/en/stable)を使って簡単にインストールできます：
...
```
`[legacy]` エクストラは、DeTi*k*Zify<sub>v1</sub> モデルを使用する予定がある場合にのみ必要です。DeTi*k*Zify<sub>v2</sub> のみを使用する場合は、これを削除できます。付属の [examples](examples) を実行することが目的の場合は、リポジトリをクローンして編集可能モードでインストールする方が簡単です。
...
pip install -e DeTikZify[examples]
 ```
さらに、DeTi*k*Zifyは完全な
[TeX Live 2023](https://www.tug.org/texlive)のインストール、
[ghostscript](https://www.ghostscript.com)、および
[poppler](https://poppler.freedesktop.org)が必要であり、
これらはパッケージマネージャーやその他の方法でインストールする必要があります。

## 使用法

> [!TIP]
> 対話的な使用や一般的な[使用のヒント](detikzify/webui#usage-tips)については、
> コマンドラインから直接起動できる[web UI](detikzify/webui)の利用を推奨します（すべてのオプション一覧は`--help`を使用）：
> ```sh
> python -m detikzify.webui --light
> ```

必要な依存関係がすべてインストールされていれば、コンパイル、レンダリング、Ti*k*Zグラフィックの保存、
およびMCTSベースの推論など、DeTi*k*Zifyの全機能はプログラミングインターフェースを通じて利用可能です：
<details open><summary>DeTi<i>k</i>Zifyの例</summary>
...
[Ti*k*Zero](https://huggingface.co/nllg/tikzero-adapter) アダプターおよび
[Ti*k*Zero+](https://huggingface.co/nllg/tikzero-plus-10b) を通じて、
テキストを条件としたグラフィックスプログラムの合成も可能です（詳細は
[論文](https://arxiv.org/abs/2503.11509) を参照）。これは現在、
プログラミングインターフェースを通じてのみサポートされています：

<details open><summary>Ti<i>k</i>Zero+ の例</summary>
...
<details><summary>Ti<i>k</i>Zeroの例</summary>
...
より複雑な例、例えば評価やトレーニング用の例は
[examples](examples) フォルダにあります。

## モデル重みとデータセット
我々はすべての DeTi*k*Zify モデルとデータセットを [Hugging Face
Hub](https://huggingface.co/collections/nllg/detikzify-664460c521aa7c2880095a8b)
にアップロードしています
(Ti*k*Zero モデルは
[こちら](https://huggingface.co/collections/nllg/tikzero-67d1952fab69f5bd172de1fe)で入手可能です)。
ただし、[DaTi*k*Z<sub>v2</sub>](https://huggingface.co/datasets/nllg/datikz-v2)
および [DaTi*k*Z<sub>v3</sub>](https://huggingface.co/datasets/nllg/datikz-v3)
データセットの公開リリースに際しては、[arXiv](https://arxiv.org) に由来する
Ti*k*Z 図のかなりの部分を削除しなければなりませんでした。
これは [arXiv 非独占ライセンス](https://arxiv.org/licenses/nonexclusive-distrib/1.0/license.html)
が再配布を許可していないためです。
しかしながら、我々は [データセット作成スクリプト](https://github.com/potamides/DaTikZ)
を公開しており、誰でも DaTi*k*Z の完全版を再作成することを推奨しています。

## 引用
DeTi*k*Zify および Ti*k*Zero があなたの研究や応用に役立った場合は、
以下のように引用していただけると幸いです。
...
## 謝辞
DeTi*k*Zifyモデルアーキテクチャの実装は、
[LLaVA](https://github.com/haotian-liu/LLaVA)および
[AutomaTikZ](https://github.com/potamides/AutomaTikZ)（v1）、および[Idefics
3](https://huggingface.co/HuggingFaceM4/Idefics3-8B-Llama3)（v2）に基づいています。私たちのMCTS
実装は
[VerMCTS](https://github.com/namin/llm-verified-with-monte-carlo-tree-search)に基づいています。
Ti*k*Zeroアーキテクチャは
[Flamingo](https://deepmind.google/discover/blog/tackling-multiple-tasks-with-a-single-visual-language-model/)
および[LLaMA
3.2-Vision](https://ai.meta.com/blog/llama-3-2-connect-2024-vision-edge-mobile-devices)から着想を得ています。
