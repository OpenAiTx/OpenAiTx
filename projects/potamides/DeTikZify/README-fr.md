# DeTi*k*Zify<br><sub><sup>Synthèse de programmes graphiques pour figures scientifiques et croquis avec Ti*k*Z</sup></sub>
[![OpenReview](https://img.shields.io/badge/View%20on%20OpenReview-8C1B13?labelColor=gray&logo=data:image/png;base64,)](https://openreview.net/forum?id=bcVLFQCOjc)
[![arXiv](https://img.shields.io/badge/View%20on%20arXiv-B31B1B?logo=arxiv&labelColor=gray)](https://arxiv.org/abs/2405.15306)
[![Hugging Face](https://img.shields.io/badge/View%20on%20Hugging%20Face-blue?labelColor=gray&logo=data:image/png;base64,)](https://huggingface.co/collections/nllg/detikzify-664460c521aa7c2880095a8b)
[![Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/drive/1hPWqucbPGTavNlYvOBvSNBAwdcPZKe8F)

Créer des figures scientifiques de haute qualité peut être long et complexe,
même si esquisser des idées sur papier est relativement facile. De plus,
reproduire des figures existantes qui ne sont pas stockées dans des formats
préservant l'information sémantique est tout aussi complexe. Pour résoudre ce
problème, nous présentons [DeTi*k*Zify](https://github.com/potamides/DeTikZify),
un nouveau modèle de langage multimodal qui synthétise automatiquement des
figures scientifiques sous forme de programmes graphiques [Ti*k*Z](https://github.com/pgf-tikz/pgf)
préservant la sémantique, à partir de croquis et de figures existantes. Nous
introduisons également un algorithme d'inférence basé sur MCTS qui permet à
DeTi*k*Zify de raffiner itérativement ses sorties sans nécessiter d'entraînement
supplémentaire.

https://github.com/potamides/DeTikZify/assets/53401822/203d2853-0b5c-4a2b-9d09-3ccb65880cd3

## Actualités
* **2025-06-30** : Nous publions [DeTi*k*Zify<sub>v2.5</sub>
  (8b)](https://huggingface.co/nllg/detikzify-v2.5-8b), notre dernier modèle,
  qui s'appuie sur
  [DeTi*k*Zify<sub>v2</sub>](https://huggingface.co/nllg/detikzify-v2-8b) en
  utilisant l'[apprentissage par renforcement à partir d'auto-retours](https://huggingface.co/nllg/detikzify-v2.5-8b#reinforcement-learning-from-self-feedback)
  (RLSF). Nous en faisons le modèle par défaut dans notre [Espace Hugging Face](https://huggingface.co/spaces/nllg/DeTikZify).
  Consultez la [fiche du modèle](https://huggingface.co/nllg/detikzify-v2-8b-preview#model-card-for-detikzifyv2-8b)
  pour plus d'informations.
* **2025-06-26** : Ti*k*Zero a été accepté comme [article phare](https://iccv.thecvf.com/virtual/2025/poster/51)
  à [ICCV 2025](https://iccv.thecvf.com) !
* **2025-03-17** : Nous publions les adaptateurs
  [Ti*k*Zero](https://huggingface.co/nllg/tikzero-adapter) qui se branchent
  directement dans [DeTi*k*Zify<sub>v2</sub>
  (8b)](https://huggingface.co/nllg/detikzify-v2-8b) et permettent la
  conditionnalité textuelle en zero-shot, ainsi que
  [Ti*k*Zero+](https://huggingface.co/nllg/tikzero-plus-10b) avec un
  affinement bout-à-bout supplémentaire. Pour plus d'informations, consultez
  notre [article](https://arxiv.org/abs/2503.11509) et les exemples d'utilisation
  [ci-dessous](#usage).
* **2024-12-05** : Nous publions [DeTi*k*Zify<sub>v2</sub>
  (8b)](https://huggingface.co/nllg/detikzify-v2-8b), notre nouveau modèle
  DeTi*k*Zify qui surpasse toutes les versions précédentes lors de nos
  évaluations. Consultez la [fiche du modèle](https://huggingface.co/nllg/detikzify-v2-8b-preview#model-card-for-detikzifyv2-8b)
  pour plus d'informations.
* **2024-09-24** : DeTi*k*Zify a été accepté à [NeurIPS 2024](https://neurips.cc/Conferences/2024)
  comme [article en vedette](https://neurips.cc/virtual/2024/poster/94474) !

## Installation

> [!TIP]
> Si vous rencontrez des difficultés lors de l'installation ou de l'inférence sur
> votre propre matériel, pensez à visiter notre [Espace Hugging Face](https://huggingface.co/spaces/nllg/DeTikZify)
> (notez que le redémarrage de l'espace peut prendre jusqu'à 30 minutes). En cas
> de longues files d'attente, vous avez la possibilité de
> [dupliquer](https://huggingface.co/spaces/nllg/DeTikZify?duplicate=true) cet espace
> avec un runtime GPU privé payant ou de [l'exécuter localement](https://huggingface.co/spaces/nllg/DeTikZify?docker=true)
> avec Docker. Vous pouvez également essayer notre démo sur [Google Colab](https://colab.research.google.com/drive/1hPWqucbPGTavNlYvOBvSNBAwdcPZKe8F).
> Cependant, la mise en place de l'environnement peut prendre un certain temps,
> et le niveau gratuit ne supporte l'inférence que pour les modèles 1b.

Le paquet Python de DeTi*k*Zify peut être facilement installé via
[pip](https://pip.pypa.io/en/stable) :





```sh
pip install 'detikzify[legacy] @ git+https://github.com/potamides/DeTikZify'
```
L’extra `[legacy]` est uniquement nécessaire si vous prévoyez d’utiliser les
modèles DeTi*k*Zify<sub>v1</sub>. Si vous prévoyez d’utiliser uniquement
DeTi*k*Zify<sub>v2</sub>, vous pouvez le supprimer. Si votre objectif est d’exécuter les
[exemples](examples) inclus, il est plus simple de cloner le dépôt et de l’installer en
mode modifiable comme ceci :
 ```sh
git clone https://github.com/potamides/DeTikZify
pip install -e DeTikZify[examples]
 ```
De plus, DeTi*k*Zify nécessite une installation complète
de [TeX Live 2023](https://www.tug.org/texlive),
[ghostscript](https://www.ghostscript.com), et
[poppler](https://poppler.freedesktop.org) que vous devez installer via
votre gestionnaire de paquets ou par d’autres moyens.

## Utilisation

> [!TIP]
> Pour une utilisation interactive et des [conseils d’utilisation](detikzify/webui#usage-tips),
> nous recommandons de consulter notre [interface web](detikzify/webui), qui peut être lancée
> directement depuis la ligne de commande (utilisez `--help` pour la liste de toutes les options) :
> ```sh
> python -m detikzify.webui --light
> ```

Si toutes les dépendances requises sont installées, l’ensemble des fonctionnalités de DeTi*k*Zify
telles que la compilation, le rendu, la sauvegarde des graphiques Ti*k*Z,
et l’inférence basée sur MCTS peuvent être accessibles via son interface de programmation :
<details open><summary>Exemple DeTi<i>k</i>Zify</summary>

```python
from operator import itemgetter

from detikzify.model import load
from detikzify.infer import DetikzifyPipeline

image = "https://w.wiki/A7Cc"
pipeline = DetikzifyPipeline(*load(
    model_name_or_path="nllg/detikzify-v2.5-8b",
    device_map="auto",
    torch_dtype="bfloat16",
))

# generate a single TikZ program
fig = pipeline.sample(image=image)

# if it compiles, rasterize it and show it
if fig.is_rasterizable:
    fig.rasterize().show()

# run MCTS for 10 minutes and generate multiple TikZ programs
figs = set()
for score, fig in pipeline.simulate(image=image, timeout=600):
    figs.add((score, fig))

# save the best TikZ program
best = sorted(figs, key=itemgetter(0))[-1][1]
best.save("fig.tex")
```
</details>

Grâce aux adaptateurs [Ti*k*Zero](https://huggingface.co/nllg/tikzero-adapter) et
[Ti*k*Zero+](https://huggingface.co/nllg/tikzero-plus-10b), il est également possible
de synthétiser des programmes graphiques conditionnés par du texte (cf. notre
[article](https://arxiv.org/abs/2503.11509) pour
plus de détails). Notez que cela est actuellement uniquement pris en charge via l'interface
de programmation :

<details open><summary>Exemple Ti<i>k</i>Zero+</summary>

```python
from detikzify.model import load
from detikzify.infer import DetikzifyPipeline

caption = "A multi-layer perceptron with two hidden layers."
pipeline = DetikzifyPipeline(*load(
    model_name_or_path="nllg/tikzero-plus-10b",
    device_map="auto",
    torch_dtype="bfloat16",
))

# generate a single TikZ program
fig = pipeline.sample(text=caption)

# if it compiles, rasterize it and show it
if fig.is_rasterizable:
    fig.rasterize().show()
```
</details>
<details><summary>Exemple Ti<i>k</i>Zero</summary>

```python
from detikzify.model import load, load_adapter
from detikzify.infer import DetikzifyPipeline

caption = "A multi-layer perceptron with two hidden layers."
pipeline = DetikzifyPipeline(
    *load_adapter(
        *load(
            model_name_or_path="nllg/detikzify-v2-8b",
            device_map="auto",
            torch_dtype="bfloat16",
        ),
        adapter_name_or_path="nllg/tikzero-adapter",
    )
)

# generate a single TikZ program
fig = pipeline.sample(text=caption)

# if it compiles, rasterize it and show it
if fig.is_rasterizable:
    fig.rasterize().show()
```
</details>

Des exemples plus complexes, par exemple pour l'évaluation et l'entraînement, peuvent être trouvés
dans le dossier [examples](examples).

## Poids des Modèles & Jeux de Données
Nous téléversons tous nos modèles et jeux de données DeTi*k*Zify sur le [Hugging Face
Hub](https://huggingface.co/collections/nllg/detikzify-664460c521aa7c2880095a8b)
(les modèles Ti*k*Zero sont disponibles
[ici](https://huggingface.co/collections/nllg/tikzero-67d1952fab69f5bd172de1fe)).
Cependant, veuillez noter que pour la publication publique des
jeux de données [DaTi*k*Z<sub>v2</sub>](https://huggingface.co/datasets/nllg/datikz-v2)
et [DaTi*k*Z<sub>v3</sub>](https://huggingface.co/datasets/nllg/datikz-v3),
nous avons dû retirer une grande partie des dessins Ti*k*Z
provenant de [arXiv](https://arxiv.org), car la [licence non exclusive d’arXiv](https://arxiv.org/licenses/nonexclusive-distrib/1.0/license.html) ne
permet pas la redistribution. Nous publions cependant nos [scripts de création de jeux de données](https://github.com/potamides/DaTikZ) et encourageons chacun à recréer
la version complète de DaTi*k*Z par eux-mêmes.

## Citation
Si DeTi*k*Zify et Ti*k*Zero ont été bénéfiques pour votre recherche ou
vos applications, nous vous prions de bien vouloir les reconnaître en les citant comme
suit :



```bibtex
@inproceedings{belouadi2024detikzify,
    title={{DeTikZify}: Synthesizing Graphics Programs for Scientific Figures and Sketches with {TikZ}},
    author={Jonas Belouadi and Simone Paolo Ponzetto and Steffen Eger},
    booktitle={The Thirty-eighth Annual Conference on Neural Information Processing Systems},
    year={2024},
    url={https://openreview.net/forum?id=bcVLFQCOjc}
}
```

```bibtex
@misc{belouadi2025tikzero,
    title={{TikZero}: Zero-Shot Text-Guided Graphics Program Synthesis},
    author={Jonas Belouadi and Eddy Ilg and Margret Keuper and Hideki Tanaka and Masao Utiyama and Raj Dabre and Steffen Eger and Simone Paolo Ponzetto},
    year={2025},
    eprint={2503.11509},
    archivePrefix={arXiv},
    primaryClass={cs.CL},
    url={https://arxiv.org/abs/2503.11509},
}
```
## Remerciements  
L'implémentation de l'architecture du modèle DeTi*k*Zify est basée sur  
[LLaVA](https://github.com/haotian-liu/LLaVA) et  
[AutomaTikZ](https://github.com/potamides/AutomaTikZ) (v1), et [Idefics  
3](https://huggingface.co/HuggingFaceM4/Idefics3-8B-Llama3) (v2). Notre implémentation MCTS  
est basée sur  
[VerMCTS](https://github.com/namin/llm-verified-with-monte-carlo-tree-search).  
L'architecture Ti*k*Zero s'inspire de  
[Flamingo](https://deepmind.google/discover/blog/tackling-multiple-tasks-with-a-single-visual-language-model/)  
et [LLaMA  
3.2-Vision](https://ai.meta.com/blog/llama-3-2-connect-2024-vision-edge-mobile-devices).



---

Tranlated By [Open Ai Tx](https://github.com/OpenAiTx/OpenAiTx) | Last indexed: 2025-08-06

---