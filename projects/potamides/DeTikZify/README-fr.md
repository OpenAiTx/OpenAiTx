# DeTi*k*Zify<br><sub><sup>Synthétiser des programmes graphiques pour figures scientifiques et croquis avec Ti*k*Z</sup></sub>
...
Créer des figures scientifiques de haute qualité peut être chronophage et difficile,  
même si esquisser des idées sur papier est relativement facile. De plus,  
recréer des figures existantes qui ne sont pas stockées dans des formats préservant les informations sémantiques  
est tout aussi complexe. Pour relever ce défi, nous présentons  
[DeTi*k*Zify](https://github.com/potamides/DeTikZify), un nouveau modèle de langage multimodal  
qui synthétise automatiquement des figures scientifiques sous forme de  
programmes graphiques [Ti*k*Z](https://github.com/pgf-tikz/pgf)  
préservant la sémantique, à partir de croquis et de figures existantes. Nous introduisons également un  
algorithme d'inférence basé sur MCTS qui permet à DeTi*k*Zify d'affiner itérativement  
ses résultats sans nécessiter d'entraînement supplémentaire.
...
* **2025-06-30** : Nous publions [DeTi*k*Zify<sub>v2.5</sub>  
  (8b)](https://huggingface.co/nllg/detikzify-v2.5-8b), notre dernier modèle, qui  
  s'appuie sur  
  [DeTi*k*Zify<sub>v2</sub>](https://huggingface.co/nllg/detikzify-v2-8b) utilisant  
  [l'apprentissage par renforcement à partir d'autocritique](https://huggingface.co/nllg/detikzify-v2.5-8b#reinforcement-learning-from-self-feedback)  
  (RLSF). Nous en faisons le modèle par défaut dans notre [Hugging Face  
  Space](https://huggingface.co/spaces/nllg/DeTikZify). Consultez la [fiche modèle](https://huggingface.co/nllg/detikzify-v2-8b-preview#model-card-for-detikzifyv2-8b)  
  pour plus d'informations.
* **2025-06-26** : Ti*k*Zero a été accepté comme [article  
  phare](https://iccv.thecvf.com/virtual/2025/poster/51) à [ICCV  
  2025](https://iccv.thecvf.com) !
* **2025-03-17** : Nous publions  
  [Ti*k*Zero](https://huggingface.co/nllg/tikzero-adapter), des adaptateurs qui se branchent  
  directement sur [DeTi*k*Zify<sub>v2</sub>  
  (8b)](https://huggingface.co/nllg/detikzify-v2-8b) et permettent une conditionnement textuel zéro-shot, ainsi que  
  [Ti*k*Zero+](https://huggingface.co/nllg/tikzero-plus-10b) avec un affinage de bout en bout supplémentaire.  
  Pour plus d'informations, consultez notre  
  [article](https://arxiv.org/abs/2503.11509) et les exemples d'utilisation [ci-dessous](#usage).
* **2024-12-05** : Nous publions [DeTi*k*Zify<sub>v2</sub>  
  (8b)](https://huggingface.co/nllg/detikzify-v2-8b), notre nouveau modèle DeTi*k*Zify  
  qui surpasse toutes les versions précédentes lors de notre évaluation. Consultez la [fiche modèle](https://huggingface.co/nllg/detikzify-v2-8b-preview#model-card-for-detikzifyv2-8b)  
  pour plus d'informations.
* **2024-09-24** : DeTi*k*Zify a été accepté à [NeurIPS  
  2024](https://neurips.cc/Conferences/2024) comme [article  
  phare](https://neurips.cc/virtual/2024/poster/94474) !

## Installation

> [!TIP]
> Si vous rencontrez des difficultés avec l'installation ou l'inférence sur votre propre  
> matériel, pensez à visiter notre [Hugging Face  
> Space](https://huggingface.co/spaces/nllg/DeTikZify) (notez que  
> le redémarrage de l'espace peut prendre jusqu'à 30 minutes). En cas de longues  
> files d'attente, vous avez la possibilité de  
> [dupliquer](https://huggingface.co/spaces/nllg/DeTikZify?duplicate=true) cet espace  
> avec un runtime GPU privé payant ou de [le faire  
> tourner localement](https://huggingface.co/spaces/nllg/DeTikZify?docker=true) avec  
> Docker. De plus, vous pouvez essayer notre démo sur [Google  
> Colab](https://colab.research.google.com/drive/1hPWqucbPGTavNlYvOBvSNBAwdcPZKe8F).  
> Cependant, la configuration de l'environnement peut prendre un certain temps, et  
> le niveau gratuit ne supporte l'inférence que pour les modèles 1b.

Le paquet Python de DeTi*k*Zify peut être facilement installé avec  
...
```
L’extra `[legacy]` est uniquement nécessaire si vous prévoyez d’utiliser les modèles
DeTi*k*Zify<sub>v1</sub>. Si vous prévoyez uniquement d’utiliser
...
> directement depuis la ligne de commande (utilisez `--help` pour une liste de toutes les options) :
> ```sh
...
telles que la compilation, le rendu, la sauvegarde des graphiques Ti*k*Z, et
l’inférence basée sur MCTS sont accessibles via son interface de programmation :
...
les détails). Notez que cela est actuellement uniquement pris en charge via l'interface
de programmation :
...
## Poids du modèle & Jeux de données
...
nous avons dû retirer une portion considérable des dessins Ti*k*Z
provenant de [arXiv](https://arxiv.org), car la [licence non exclusive
d'arXiv](https://arxiv.org/licenses/nonexclusive-distrib/1.0/license.html) ne
permet pas la redistribution. Nous publions cependant nos [scripts de création
de jeu de données](https://github.com/potamides/DaTikZ) et encourageons chacun à recréer
la version complète de DaTi*k*Z par eux-mêmes.

## Citation
Si DeTi*k*Zify et Ti*k*Zero ont été bénéfiques pour vos recherches ou
applications, nous vous prions de bien vouloir les citer comme suit :
...
[AutomaTikZ](https://github.com/potamides/AutomaTikZ) (v1), ainsi que [Idefics
