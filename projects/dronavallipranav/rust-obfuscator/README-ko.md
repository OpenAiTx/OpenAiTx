...
`rust-obfuscator`는 절차적 매크로를 삽입하거나 (선택적으로) 소스 코드 내에 직접 난독화를 제공하여 Rust 소스 코드를 자동으로 난독화하는 도구 세트입니다. 보다 세밀한 난독화를 위해 절차적 매크로 라이브러리 [cryptify](https://crates.io/crates/cryptify)도 제공됩니다.
...
- **문자열 암호화**: 컴파일 시 로컬 변수에 할당된 문자열 리터럴을 자동으로 암호화합니다.
    - 포맷된 문자열에도 사용할 수 있지만, 현재는 수동 배치가 필요합니다.
    ```rs
