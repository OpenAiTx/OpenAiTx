...
- موقع شخصي تم تطويره باستخدام `Astro`
- يستخدم `algolia` للبحث 🔍
- يستخدم `giscus` للتعليقات والمناقشة 🌈
- يستخدم `mailerlite` لإرسال الاشتراكات البريدية 📮
- هذا المشروع مستضاف على `netlify`، ويعتمد على `crawler` لجلب المستندات وتخزين مخطط البحث المطلوب لـ `algolia`
