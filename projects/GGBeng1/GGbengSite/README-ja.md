...
- `Astro` を使用して開発された個人サイト
- `algolia` を利用した検索機能 🔍
- `giscus` を使ったコメント・ディスカッション 🌈
- `mailerlite` を利用したメール購読配信 📮
- 本プロジェクトは `netlify` でホスティングされており、`crawler` を活用してクローリングでドキュメントを収集し、`algolia` 用の検索 `schema` に保存
