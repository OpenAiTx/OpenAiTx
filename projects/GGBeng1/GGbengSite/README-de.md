...
- Persönliche Webseite entwickelt mit `Astro`
- Suche mit `algolia` implementiert 🔍
- Kommentare und Diskussionen mit `giscus` 🌈
- E-Mail-Abonnements und Versand mit `mailerlite` 📮
- Dieses Projekt wird auf `netlify` gehostet und verwendet einen `crawler`, um Dokumente zu durchsuchen und das für `algolia` benötigte Such-`Schema` zu speichern.
