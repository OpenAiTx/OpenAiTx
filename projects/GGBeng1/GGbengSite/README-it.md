...
- Ricerca effettuata con `algolia` 🔍
- Commenti e discussioni gestiti con `giscus` 🌈
- Invio e gestione delle iscrizioni email tramite `mailerlite` 📮
- Questo progetto è ospitato su `netlify`, e utilizza `crawler` per indicizzare e memorizzare i documenti necessari allo schema di ricerca di `algolia`
