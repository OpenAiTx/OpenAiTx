...
- Personal website developed with `Astro`
- Uses `algolia` for search 🔍
- Uses `giscus` for comments and discussions 🌈
- Uses `mailerlite` for email subscription and sending 📮
- This project is hosted on `netlify`, and relies on a `crawler` to scrape documents and store the search `schema` required by `algolia`
