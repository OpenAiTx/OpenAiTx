...
- A personal website developed with `Astro`
- Uses `algolia` for search 🔍
- Uses `giscus` for comments and discussions 🌈
- Uses `mailerlite` for email subscription and delivery 📮
- This project is hosted on `netlify`, relying on a `crawler` to crawl documents and store the search `schema` required by `algolia`
