...
- Osobista strona internetowa stworzona przy użyciu `Astro`
- Wyszukiwanie realizowane za pomocą `algolia` 🔍
- Komentarze i dyskusje obsługiwane przez `giscus` 🌈
- Subskrypcje i wysyłka maili za pomocą `mailerlite` 📮
- Projekt hostowany na `netlify`, dokumentacja pobierana przez `crawler` i zapisywana w `algolia` zgodnie z wymaganym schematem wyszukiwania
