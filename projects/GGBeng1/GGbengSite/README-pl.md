...
- Strona osobista opracowana przy użyciu `Astro`
- Wyszukiwanie realizowane przez `algolia` 🔍
- Komentarze i dyskusje poprzez `giscus` 🌈
- Subskrypcja mailowa i wysyłka przez `mailerlite` 📮
- Projekt hostowany na `netlify`, dokumenty do wyszukiwania w `algolia` pobierane przez `crawler` według wymaganego schematu wyszukiwania
