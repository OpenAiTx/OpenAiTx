...
- Комментарии и обсуждения реализованы через `giscus` 🌈
- Рассылка и подписка на почту с помощью `mailerlite` 📮
- Проект размещён на `netlify`, а с помощью `crawler` происходит сканирование и сохранение документации в нужной для поиска `algolia` схеме
