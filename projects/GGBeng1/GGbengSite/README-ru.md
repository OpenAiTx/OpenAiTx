...
- Персональный сайт, разработанный с помощью `Astro`
- Поиск реализован с использованием `algolia` 🔍
- Для комментариев и обсуждений используется `giscus` 🌈
- Для рассылки и подписки на почту используется `mailerlite` 📮
- Проект размещён на `netlify`, с использованием `crawler` для сканирования и хранения документов в поисковой `schema` для `algolia`
