...
- Kişisel web sitesi `Astro` ile geliştirildi
- Arama için `algolia` kullanıldı 🔍
- Yorum ve tartışma için `giscus` kullanıldı 🌈
- E-posta abonelik gönderimi için `mailerlite` kullanıldı 📮
- Bu proje `netlify` üzerinde barındırılmakta, dokümanların taranıp `algolia` için gerekli arama `schema`larının depolanmasında `crawler` kullanılmakta
