...
- Yorum ve tartışmalar için `giscus` kullanılır 🌈
- E-posta aboneliği ve gönderimi için `mailerlite` kullanılır 📮
- Bu proje `netlify` üzerinde barındırılmakta olup, dökümanların taranıp `algolia` için gerekli arama `schema`sının oluşturulmasında `crawler` kullanılmaktadır
