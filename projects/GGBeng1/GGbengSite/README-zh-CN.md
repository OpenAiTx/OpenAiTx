...
- Comments and discussions enabled via `giscus` 🌈
- Newsletter subscriptions managed and sent using `mailerlite` 📮
- Project hosted on `netlify`, using `crawler` to fetch documents and store the search `schema` required by `algolia`
