...
- استفاده از `algolia` برای جستجو 🔍
- استفاده از `giscus` برای ارسال پیام و بحث 🌈
- استفاده از `mailerlite` برای ارسال اشتراک ایمیل 📮
- این پروژه در `netlify` میزبانی شده است و با استفاده از `crawler` اسناد را برای ذخیره `schema` جستجوی مورد نیاز `algolia` جمع‌آوری می‌کند
