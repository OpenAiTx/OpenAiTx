...
With dimensionality reduction you can project high-dimensional data to a lower dimensionality while keeping method-specific properties of the data.
...
DruidJS internally uses the Matrix class for storing data. You can use it by creating a `druid.Matrix` object, for example with the function `from`, as follows:
...
If `rows == columns`, then `matrix` would be an identity matrix.
A shortcut for an identity matrix is:
