...
To bypass status error catching you can set the `ignoreResponseError` option:
...
`ofetch` automatically retries the request if an error occurs and if the response status code is included in the `retryStatusCodes` list:
...
You can specify the number of retries and the delay between them using the `retry` and `retryDelay` options, and also provide a custom array of codes using the `retryStatusCodes` option.
...
By using the `baseURL` option, `ofetch` prepends it for trailing/leading slashes and query search params for baseURL using [ufo](https://github.com/unjs/ufo):
...
By using the `query` option (or `params` as an alias), `ofetch` adds query search params to the URL while preserving the query in the request itself using [ufo](https://github.com/unjs/ufo):
...
It is possible to provide async interceptors to hook into lifecycle events of the `ofetch` call.

You may want to use `ofetch.create` to set shared interceptors.
...
`onResponse` will be called after the `fetch` call and body parsing.
...
`onResponseError` works the same as `onResponse` but will be called when fetch occurs but `response.ok` is not `true`.
...
If necessary, it is also possible to pass an array of functions that will be called sequentially.
...
By using the `headers` option, `ofetch` adds extra headers in addition to the request's default headers:
...
If you need to access the raw response (for headers, etc), you can use `ofetch.raw`:
...
As a shortcut, you can use `ofetch.native` which provides the native `fetch` API
