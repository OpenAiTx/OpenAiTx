...
# Due to the holiday season and spending time with families, we will not be available to work on any additional requests until the end of January. During this time it will not be possible to raise new issues. #
...
There are a couple of small quirks that are out of my control as it's down to the launcher's behavior, but they are easy to work around and won’t cause too much of an issue.
...
This happens because the app developer changed the app's activity, so the icon is still mapped to an activity that no longer exists within the application. I've mentioned this to the projectivy dev, so hopefully it will be something that can be fixed in the launcher.
