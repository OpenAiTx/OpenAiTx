...
NoneBot DeepSeek 플러그인입니다. DeepSeek 모델을 연동하여 지능형 대화 및 질의응답 기능을 제공합니다.

Wiki: 👉 [바로가기](https://github.com/KomoriDev/nonebot-plugin-deepseek/wiki)

> [!IMPORTANT]
> **프로젝트를 즐겨찾기** 하시면, GitHub에서 모든 릴리스 알림을 실시간으로 받아보실 수 있습니다~⭐️
...
아래에 언급된 방법 중 **하나만** 선택하여 사용하시면 됩니다.

> [!TIP]
> Markdown을 이미지로 변환하는 기능을 사용하려면 `nonebot-plugin-deepseek[image]`를 설치해야 합니다.

<details open>
<summary>[추천] nb-cli로 설치하기</summary>
Bot의 루트 디렉터리에서 명령줄을 열고, 아래 명령어를 입력하면 설치됩니다.
...
프로젝트의 설정 파일에 아래 표의 설정을 추가하세요.

> [!note]
> `api_key`는 [DeepSeek 오픈 플랫폼](https://platform.deepseek.com/)에서 발급받으세요  
> `enable_models`는 [`CustomModel`](https://github.com/KomoriDev/nonebot-plugin-deepseek/blob/ee9f0b0f0568eedb3eb87423e6c1bf271787ab76/nonebot_plugin_deepseek/config.py#L34) 구조의 딕셔너리이며, 로컬 모델을 연결할 필요가 없다면 수정할 필요가 없습니다  
> 로컬 모델을 연결하려면 👉 [Wiki - 로컬 모델 연결](https://github.com/KomoriDev/nonebot-plugin-deepseek/wiki/본地模型) 참고  

|              설정 항목              | 필수  |                             기본값                             |                                                                          설명                                                                          |
| :------------------------------: | :---: | :------------------------------------------------------------: | :----------------------------------------------------------------------------------------------------------------------------------------------------: |
|        deepseek__api_key         |  예   |                               없음                              |                                                                        API Key                                                                         |
|     deepseek__enable_models      |  아니오 | [{ "name": "deepseek-v4-flash" }, { "name": "deepseek-v4-pro" }] | 활성화된 모델 [설정 설명](https://github.com/KomoriDev/nonebot-plugin-deepseek/wiki/%E9%85%8D%E7%BD%AE#enable_models-%E9%85%8D%E7%BD%AE%E8%AF%B4%E6%98%8E) |
|         deepseek__prompt         |  아니오 |                               없음                              |                                                                        모델 프리셋                                                                      |
|         deepseek__stream         |  아니오 |                             False                              |                                                                    스트리밍 전송 활성화 여부                                                           |
|        deepseek__timeout         |  아니오 |             {"api_request": 100, "user_input": 60}             |                                                                        타임아웃 설정                                                                   |
|       deepseek__md_to_pic        |  아니오 |                             False                              |                                                                Markdown을 이미지로 변환 여부                                                           |
|  deepseek__enable_send_thinking  |  아니오 |                             False                              |                                                                     사고 체인 전송 여부                                                               |

## 🎉 사용법

> [!note]
> `COMMAND_START` 및 위의 설정 항목을 확인하세요. 여기서는 기본적으로 `/`를 사용합니다.
