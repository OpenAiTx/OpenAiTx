...
An object representing the `limit`, `methods`, `statusCodes`, `afterStatusCodes`, and `maxRetryAfter` fields for the maximum retry count, allowed methods, allowed status codes, status codes allowed to use the [`Retry-After`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Retry-After) time, and the maximum [`Retry-After`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Retry-After) time.
...
If `maxRetryAfter` is set to `undefined`, it will use `options.timeout`. If the [`Retry-After`](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Retry-After) header is greater than `maxRetryAfter`, it will use `maxRetryAfter`.
...
Timeout in milliseconds for getting a response, including any retries. Cannot be greater than 2147483647.
