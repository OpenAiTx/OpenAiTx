...
跨平台 Zig TUI 函式庫，靈感來自 [Ratatui](https://github.com/ratatui/ratatui)。
...
- **高效渲染** — 以儲存格為基礎的差異比對，只重繪有變更的部分
- **元件** — 區塊、段落、清單、指標、表格
- **15 種內建主題** — Nord、Dracula、Gruvbox、Catppuccin、Tokyo Night...
- **Kitty 圖形協定** — 顯示圖片（並有 Unicode 備援）
- **無隱藏分配** — 明確的記憶體管理

## 系統需求

- Zig 0.15.0 以上
- Windows 10 以上／Linux／macOS
...
**可用主題：** `default`、`nord`、`dracula`、`monokai`、`gruvbox_dark`、`gruvbox_light`、`solarized_dark`、`solarized_light`、`tokyo_night`、`catppuccin_mocha`、`catppuccin_latte`、`one_dark`、`cyberpunk`、`matrix`、`high_contrast`
...
在支援 [Kitty 圖形協議](https://sw.kovidgoyal.net/kitty/graphics-protocol/) 的終端機顯示圖片。會自動回退到 Unicode 區塊。
...
**支援：** Kitty、WezTerm、foot、Konsole（部分支援）  
**備用：** Windows Terminal、iTerm2、Terminal.app

## 平台支援

| 平台        | 終端機                    | 備註               |
|-------------|--------------------------|--------------------|
| Windows 10+ | Windows Terminal、WezTerm | 原生 Console API   |
| Linux       | 任何 ANSI 相容終端機      | POSIX termios      |
