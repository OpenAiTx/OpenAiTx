...
# Claude Code 서브에이전트 모음집

[Claude Code](https://docs.anthropic.com/en/docs/claude-code)를 위한 전문 AI 서브에이전트의 종합 컬렉션으로, 도메인별 전문성을 통해 개발 워크플로우를 향상시키도록 설계되었습니다.

## 개요

이 저장소에는 Claude Code의 기능을 확장하는 57개의 전문화된 서브에이전트가 포함되어 있습니다. 각 서브에이전트는 특정 도메인의 전문가로서, 상황에 따라 자동으로 호출되거나 필요시 명시적으로 호출될 수 있습니다. 모든 에이전트는 과제의 복잡도에 따라 최적의 성능과 비용 효율성을 위해 특정 Claude 모델로 구성되어 있습니다.
...
- **[ui-ux-designer](https://raw.githubusercontent.com/wshobson/agents/main/ui-ux-designer.md)** - 인터페이스 디자인, 와이어프레임, 디자인 시스템 생성
...
- **[golang-pro](https://raw.githubusercontent.com/wshobson/agents/main/golang-pro.md)** - 고루틴, 채널, 인터페이스를 활용한 Go 코드 작성
- **[rust-pro](https://raw.githubusercontent.com/wshobson/agents/main/rust-pro.md)** - 소유권 패턴, 라이프타임, 트레이트 구현을 활용한 Rust 코드 작성
- **[c-pro](https://raw.githubusercontent.com/wshobson/agents/main/c-pro.md)** - 효율적인 메모리 관리와 시스템 콜을 사용한 C 코드 작성
...
- **[php-pro](https://raw.githubusercontent.com/wshobson/agents/main/php-pro.md)** - 최신 기능과 성능 최적화를 갖춘 PHP 코드 작성
- **[java-pro](https://raw.githubusercontent.com/wshobson/agents/main/java-pro.md)** - 스트림, 동시성, JVM 최적화를 갖춘 최신 자바 마스터
...
- **[csharp-pro](https://raw.githubusercontent.com/wshobson/agents/main/csharp-pro.md)** - 고급 기능과 .NET 최적화를 갖춘 최신 C# 코드 작성
- **[unity-developer](https://raw.githubusercontent.com/wshobson/agents/main/unity-developer.md)** - 최적화된 스크립트와 성능 튜닝을 활용한 Unity 게임 개발
...
- **[incident-responder](https://raw.githubusercontent.com/wshobson/agents/main/incident-responder.md)** - 프로덕션 사고를 신속하고 정확하게 처리
...
- **[dx-optimizer](https://raw.githubusercontent.com/wshobson/agents/main/dx-optimizer.md)** - 개발자 경험 전문가로서 툴링, 셋업, 워크플로우 개선

### 품질 및 보안
- **[code-reviewer](https://raw.githubusercontent.com/wshobson/agents/main/code-reviewer.md)** - 심층 구성 보안 및 프로덕션 신뢰성을 중점으로 한 전문 코드 리뷰
- **[security-auditor](https://raw.githubusercontent.com/wshobson/agents/main/security-auditor.md)** - 취약점 코드 검토 및 OWASP 준수 보장
- **[test-automator](https://raw.githubusercontent.com/wshobson/agents/main/test-automator.md)** - 단위, 통합, e2e 테스트로 포괄적인 테스트 스위트 생성
...
- **[search-specialist](https://raw.githubusercontent.com/wshobson/agents/main/search-specialist.md)** - 고급 검색 기법과 종합을 활용한 웹 연구 전문가

### 데이터 & AI
- **[data-scientist](https://raw.githubusercontent.com/wshobson/agents/main/data-scientist.md)** - SQL 쿼리, BigQuery 작업, 데이터 인사이트 전문가
...
- **[prompt-engineer](https://raw.githubusercontent.com/wshobson/agents/main/prompt-engineer.md)** - LLM 및 AI 시스템용 프롬프트 최적화

### 특화 도메인
- **[api-documenter](https://raw.githubusercontent.com/wshobson/agents/main/api-documenter.md)** - OpenAPI/Swagger 명세 작성 및 개발자 문서화
- **[payment-integration](https://raw.githubusercontent.com/wshobson/agents/main/payment-integration.md)** - Stripe, PayPal, 결제 프로세서 통합
- **[quant-analyst](https://raw.githubusercontent.com/wshobson/agents/main/quant-analyst.md)** - 금융 모델 구축, 트레이딩 전략 백테스트, 시장 데이터 분석
- **[risk-manager](https://raw.githubusercontent.com/wshobson/agents/main/risk-manager.md)** - 포트폴리오 리스크, R-멀티플, 포지션 한도 모니터링
- **[legacy-modernizer](https://raw.githubusercontent.com/wshobson/agents/main/legacy-modernizer.md)** - 레거시 코드베이스 리팩터링 및 점진적 현대화 구현
- **[context-manager](https://raw.githubusercontent.com/wshobson/agents/main/context-manager.md)** - 여러 에이전트 및 장기 실행 작업의 컨텍스트 관리

### 문서화
- **[docs-architect](https://raw.githubusercontent.com/wshobson/agents/main/docs-architect.md)** - 기존 코드베이스에서 포괄적인 기술 문서 생성
- **[reference-builder](https://raw.githubusercontent.com/wshobson/agents/main/reference-builder.md)** - 기술 레퍼런스 및 API 문서의 철저한 작성
- **[tutorial-engineer](https://raw.githubusercontent.com/wshobson/agents/main/tutorial-engineer.md)** - 코드 기반 단계별 튜토리얼 및 교육 콘텐츠 제작

### 비즈니스 & 마케팅
- **[business-analyst](https://raw.githubusercontent.com/wshobson/agents/main/business-analyst.md)** - 지표 분석, 보고서 생성, KPI 추적
...
- **[legal-advisor](https://raw.githubusercontent.com/wshobson/agents/main/legal-advisor.md)** - 개인정보 처리방침, 서비스 이용 약관, 고지사항 및 법률 문서 작성

## 모델 할당

57개의 서브에이전트 모두 작업 복잡도에 따라 특정 Claude 모델로 구성되어 있습니다:

### 🚀 Haiku (빠르고 비용 효율적) - 9개 에이전트
**모델:** `haiku`
- `data-scientist` - SQL 쿼리 및 데이터 분석
- `api-documenter` - OpenAPI/Swagger 문서화
- `reference-builder` - 포괄적인 기술 참조 및 API 문서화
...
- `legal-advisor` - 개인정보 보호정책 및 준수 문서

### ⚡ Sonnet (균형 잡힌 성능) - 35개 에이전트
**모델:** `sonnet`

**개발 & 언어:**
- `python-pro` - 고급 기능의 파이썬 개발
- `javascript-pro` - 최신 JavaScript 및 Node.js
- `typescript-pro` - 타입 시스템을 갖춘 고급 TypeScript
...
- `java-pro` - 스트림과 동시성을 활용한 최신 자바
- `elixir-pro` - OTP 패턴 및 Phoenix를 활용한 엘릭서
- `csharp-pro` - .NET 프레임워크 및 패턴을 적용한 최신 C#
...
- `frontend-developer` - 리액트 컴포넌트 및 UI
...
- `terraform-specialist` - 코드 기반 인프라스트럭처
...
- `docs-architect` - 코드베이스로부터 포괄적 기술 문서 작성
- `tutorial-engineer` - 단계별 튜토리얼 및 교육 콘텐츠

## 설치

이 서브에이전트들은 `~/.claude/agents/` 디렉터리에 배치하면 자동으로 사용 가능합니다.
...
- **rust-pro**: Rust 개발, 메모리 안전, 시스템 프로그래밍
- **c-pro**: C 프로그래밍, 임베디드 시스템, 성능 최적화 코드
- **javascript-pro**: 최신 JavaScript, 비동기 패턴, Node.js/브라우저 코드
...
- **incident-responder**: 즉각적인 대응이 필요한 중대한 장애
- **database-optimizer**: 쿼리 성능, 인덱싱 전략
- **database-admin**: 백업 전략, 복제, 사용자 관리, 재해 복구
- **terraform-specialist**: 코드형 인프라, Terraform 모듈, 상태 관리
...
- **reference-builder**: 방대한 API 참조, 구성 가이드, 파라미터 문서화
- **tutorial-engineer**: 단계별 튜토리얼, 학습 경로, 교육 콘텐츠

### 💼 비즈니스 & 전략
- **business-analyst**: KPI, 수익 모델, 성장 예측, 투자자 지표
- **risk-manager**: 포트폴리오 리스크, 헤지 전략, R-배수, 포지션 사이징
...
- **legal-advisor** - 개인정보 처리방침, 서비스 이용약관, 면책조항, 법적 고지 초안 작성

## 모범 사례

### 🎯 작업 위임
1. **Claude Code가 자동으로 위임하도록 하세요** - 메인 에이전트가 맥락을 분석해 최적의 에이전트를 선택
2. **요구사항을 구체적으로 명시하세요** - 제약조건, 기술 스택, 품질 요구사항 포함
3. **에이전트 전문성을 신뢰하세요** - 각 에이전트는 해당 도메인에 최적화됨

### 🔄 다중 에이전트 워크플로우
4. **상위 수준 요청부터 시작하세요** - 에이전트가 복잡한 다단계 작업을 조정하도록
5. **에이전트 간 맥락을 제공하세요** - 에이전트가 필요한 배경 정보를 갖추도록
6. **통합 지점 리뷰** - 서로 다른 에이전트의 결과물이 어떻게 결합되는지 확인

### 🎛️ 명시적 제어
7. **특정 필요에 명시적 호출 사용** - 특정 전문가의 관점을 원할 때
8. **여러 에이전트를 전략적으로 조합** - 서로의 작업을 상호 검증 가능
9. **특정 리뷰 패턴 요청** - "security-auditor가 backend-architect의 API 설계를 리뷰하도록"

### 📈 최적화
10. **에이전트 효율성 모니터링** - 어떤 에이전트가 내 사용 사례에 최적인지 학습
11. **복잡한 작업 반복 개선** - 에이전트 피드백을 활용해 요구사항을 다듬기
12. **에이전트의 강점 활용** - 작업 복잡도를 에이전트의 역량에 맞게 조정

## 기여 방법

새로운 서브에이전트를 추가하려면:
1. 위의 형식을 따라 새로운 `.md` 파일을 만드세요
2. 소문자와 하이픈(-)으로 이름을 작성하세요
3. 서브에이전트를 사용해야 하는 상황에 대해 명확하게 설명하세요
...
**에이전트가 자동으로 호출되지 않음:**
- 요청이 명확하게 도메인을 나타내는지 확인하세요 (예: "성능 문제" → performance-engineer)
- 작업 유형을 구체적으로 명시하세요 (예: "코드 리뷰" → code-reviewer)

**예상치 못한 에이전트 선택:**
- 기술 스택과 요구사항에 대한 추가 정보를 제공하세요
- 특정 에이전트가 필요할 경우 명시적으로 호출하세요

**여러 에이전트가 상충되는 조언을 제공함:**
- 이는 정상입니다 - 각 전문가는 우선순위가 다를 수 있습니다
- 명확하게 요청하세요: "security-auditor와 performance-engineer의 권고 사항을 조정해 주세요"
...
1. 각 에이전트의 파일에서 설명을 확인하세요
2. 요청에 더 구체적인 언어를 사용해 보세요
3. 특정 에이전트를 테스트하려면 명시적으로 호출하세요
4. 프로젝트와 목표에 대한 더 많은 정보를 제공하세요
