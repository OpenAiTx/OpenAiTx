...
Storm options are functions that can be passed when constructing your Storm instance. You can pass any number of options.
...
To store data in BoltDB, Storm marshals it in JSON by default. If you wish to change this behavior, you can pass a codec that implements [`codec.MarshalUnmarshaler`](https://godoc.org/github.com/asdine/storm/codec#MarshalUnmarshaler) via the [`storm.Codec`](https://godoc.org/github.com/asdine/storm#Codec) option:
...
These can be used by importing the relevant package and using that codec to configure Storm. The example below shows all variants (without proper error handling):
...
**Tip**: Adding Storm tags to generated Protobuf files can be tricky. A good solution is to use [this tool](https://github.com/favadi/protoc-go-inject-tag) to inject the tags during compilation.
...
Storm takes advantage of BoltDB's nested buckets feature by using `storm.Node`.
...
A transaction can also be passed to Storm
