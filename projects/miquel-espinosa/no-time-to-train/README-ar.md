...
python run_lightening.py test --config $CONFIG  \
                              ...
                              --trainer.devices $GPUS
```
إذا كنت ترغب في رؤية نتائج الاستدلال عبر الإنترنت (أثناء حسابها)، أضف الوسيط:
...
```
سيتم الآن حفظ الصور في `results_analysis/few_shot_classes/`. تعرض الصورة على اليسار الحقيقة الأرضية، بينما تعرض الصورة على اليمين الكائنات المجزأة التي تم العثور عليها بواسطة طريقتنا الخالية من التدريب.
...
#### 2.1 تصور بنك الذاكرة بعد المعالجة
...
python run_lightening.py test --config $YAML_PATH \
    ...
    --trainer.devices 1
```
يتم تخزين تصورات PCA و K-means لصور بنك الذاكرة في `results_analysis/memory_vis/my_custom_dataset`.
...
<details>
<summary><b>تصوير ميزات DINOv2 PCA:</b></summary>
...
<details>
<summary><b>تصوير ميزة SAM2 AMG:</b></summary>
...
<a href="https://www.star-history.com/?repos=miquel-espinosa%2Fno-time-to-train&type=date&legend=top-left">
 <picture>
   ...
   <img alt="مخطط تاريخ النجوم" src="https://api.star-history.com/image?repos=miquel-espinosa/no-time-to-train&type=date&legend=top-left" />
 </picture>
</a>
...
إذا استخدمت هذا العمل، يرجى ذكرنا:
