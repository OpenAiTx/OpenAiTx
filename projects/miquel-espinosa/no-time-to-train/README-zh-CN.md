...
- [📊 推理代码：在Few-shot COCO中复现30-shot SOTA结果](#-inference-code)
  - [0. 创建参考集](#0-create-reference-set)
  - [1. 用参考填充内存](#1-fill-memory-with-references)
  - [2. 后处理内存库](#2-post-process-memory-bank)
  - [3. 在目标图像上推理](#3-inference-on-target-images)
  - [结果](#results)
- [🔍 引用](#-citation)


## 🎯 亮点
- 💡 **零训练**：无需微调，无需提示工程——只需一张参考图片。  
- 🖼️ **基于参考**：仅需少量样本即可分割新目标。  
- 🔥 **SOTA性能**：在COCO、PASCAL VOC和跨域FSOD上优于以往零训练方法。
...
> 图像分割模型的性能历来受限于大规模标注数据的高昂获取成本。Segment Anything Model（SAM）通过可提示、语义无关的分割范式缓解了这一原始问题，但在处理新图片时仍然需要人工视觉提示或复杂的、依赖领域的提示生成规则。为减轻这一新负担，我们的工作研究了在仅提供少量参考图片时的目标分割任务。我们的关键见解是利用基础模型学习到的强语义先验，在参考图像和目标图像之间识别对应区域。我们发现，这种对应关系能够实现下游任务中实例级分割掩码的自动生成，并通过一个多阶段、零训练的方法实现我们的想法，包括（1）内存库构建；（2）表征聚合以及（3）语义感知特征匹配。实验结果表明，我们在分割指标上取得了显著提升，在COCO FSOD（36.8% nAP）、PASCAL VOC Few-Shot（71.2% nAP50）上达到最新水平，并在Cross-Domain FSOD基准上（22.4% nAP）优于现有零训练方法。
