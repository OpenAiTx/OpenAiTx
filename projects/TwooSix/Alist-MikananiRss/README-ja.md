...
<p align="center">
  <a href="https://mikanani.me/">蜜柑計画</a>やその他のアニメ番組関連のRSSフィードから自動で最新話を取得し、Alist経由で対応するクラウドストレージにオフラインダウンロードします
</p>  
<p align="center">
  さらにChatGPTによるリソース名解析を組み合わせ、Embyで認識可能な形式にリネームします。
</p>
</p>  
...
## 機能
- アニメの更新を自動取得し、対応するクラウドストレージにダウンロード
- PushPlus、Telegramなどのチャネルを通じて更新通知を送信
- embyが認識可能な形式に自動リネーム、さらに自動解析結果のカスタムリマッピングもサポートし、リネーム結果の精度向上

## 準備作業
1. [Alist](https://github.com/alist-org/alist)プロジェクトドキュメントを参照してAlistをデプロイしてください（バージョンは>=3.42.0必要）、Aria2/qBittorrentのオフラインダウンロードも構築
2. 自分でMikan Projectのアカウントを登録し、アニメを購読、購読リンクを取得

付記：他のRSS購読元にも一定の適応を行っており、理論上ほとんどの購読元（アニメ関連）をサポートしています。未対応RSSについてもissue提出歓迎です。

## 使用方法
Dockerやソースコード実行など、その他の実行方法は[使用ドキュメント](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B)をご参照ください。

pipでインストールして実行
1. Pythonのバージョンが3.11以上であることを確認してください
2. pipでインストール: `pip install alist-mikananirss`
3. ディレクトリ内に`config.yaml`設定ファイルを新規作成し、以下のように設定ファイルを記入（完全な機能例と詳細は[設定説明](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E9%85%8D%E7%BD%AE%E8%AF%B4%E6%98%8E)参照）
   ```yaml
   common:
     interval_time: 300
