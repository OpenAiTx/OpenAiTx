...
<p align="center">
  Automatyczne pobieranie aktualizacji anime z <a href="https://mikanani.me/">Mikan Project</a> lub innych RSS powiązanych z anime oraz ich offline'owe pobieranie na odpowiedni dysk sieciowy przez Alist
</p>  
<p align="center">
  W połączeniu z analizą nazw zasobów przez ChatGPT, pliki są automatycznie zmieniane na format rozpoznawalny przez Emby.
</p>
</p>  

--- 

[Dokumentacja użytkownika](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B)
## Funkcje
- Automatyczne pobieranie aktualizacji anime i zapisywanie ich na odpowiednim dysku sieciowym
- Wysyłanie powiadomień o aktualizacjach przez PushPlus, Telegram i inne kanały
- Automatyczne zmienianie nazw plików na format rozpoznawalny przez emby, z możliwością własnego mapowania wyników, co zwiększa dokładność nazw

## Przygotowanie 
1. Samodzielnie wdroż Alist według dokumentacji projektu [Alist](https://github.com/alist-org/alist) (wymagana wersja >=3.42.0) oraz skonfiguruj Aria2/qBittorrent do pobierania offline
2. Zarejestruj konto na stronie Mikan Project, subskrybuj anime i uzyskaj link do subskrypcji

Uwaga: Skrypt został częściowo dostosowany także do innych źródeł RSS, teoretycznie obsługuje większość źródeł subskrypcji (związanych z anime), jeśli jakieś RSS nie są obsługiwane, zgłoś issue

## Jak używać
Więcej metod uruchamiania jak Docker, uruchamianie ze źródeł itp. opisano w [dokumentacji użytkownika](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B) 

Instalacja i uruchomienie przez pip
1. Upewnij się, że używasz Pythona w wersji 3.11 lub wyższej
2. Zainstaluj pakiet przez pip: `pip install alist-mikananirss`
3. Utwórz w katalogu plik `config.yaml` i wypełnij go zgodnie z poniższym przykładem (pełny opis funkcji i konfiguracji znajdziesz w [instrukcji konfiguracji](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E9%85%8D%E7%BD%AE%E8%AF%B4%E6%98%8E))
   ```yaml
   common:
     interval_time: 300
   ...
   ```
4. Uruchom kod: `python -m alist_mikananirss --config /path/to/config.yaml`  
5. Ciesz się


## Prezentacja efektu zmiany nazw
<div align=center>
