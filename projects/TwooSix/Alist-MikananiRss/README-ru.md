...
<p align="center">
  Автоматически получает обновления аниме из <a href="https://mikanani.me/">Mikan Project</a> или других связанных с аниме RSS-лент и скачивает их через Alist на соответствующий облачный диск в оффлайн-режиме
</p>  
<p align="center">
  В сочетании с использованием ChatGPT анализирует названия ресурсов и переименовывает их в формат, совместимый с Emby.
</p>
</p>  

--- 

[Документация по использованию](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B)
## Функции
- Автоматически получает обновления аниме и загружает их на соответствующий облачный диск
- Отправляет уведомления об обновлениях через PushPlus, Telegram и другие каналы
- Автоматически переименовывает файлы в формат, распознаваемый emby, с поддержкой пользовательского переназначения автоматически разобранных результатов для более точного переименования

## Подготовка 
1. Пожалуйста, самостоятельно разверните Alist согласно документации проекта [Alist](https://github.com/alist-org/alist) (требуется версия >=3.42.0) и настройте Aria2/qBittorrent для оффлайн-загрузки
2. Зарегистрируйте аккаунт Mikan Project, подпишитесь на аниме и получите ссылку на подписку

Примечание: Поддержка других RSS-источников также реализована в определённой степени, теоретически поддерживается большинство источников (связанных с аниме), для неподдерживаемых RSS приветствуется создание issue

## Как использовать
Подробнее о запуске через Docker, исходный код и другие способы см. в [документации по использованию](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E5%BF%AB%E9%80%9F%E5%BC%80%E5%A7%8B) 

Установка и запуск через pip
1. Убедитесь, что у вас установлен python версии 3.11 или выше
2. Установите через pip: `pip install alist-mikananirss`
3. В каталоге создайте новый файл конфигурации `config.yaml` и заполните его следующим образом (подробное объяснение всех функций см. в [описании конфигурации](https://github.com/TwooSix/Alist-MikananiRss/wiki/%E9%85%8D%E7%BD%AE%E8%AF%B4%E6%98%8E))
   ```yaml
   common:
     interval_time: 300
   ...
   ```
4. Запустите код: `python -m alist_mikananirss --config /path/to/config.yaml`  
5. Наслаждайтесь


## Демонстрация эффекта переименования
<div align=center>
