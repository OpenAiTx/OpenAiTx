...
ライティングコンペに提出されたサンプルストーリーは[こちら](https://blog.reedsy.com/short-story/wo9iuy/#comments)でご覧いただけます

AIモデルを用いて創造的なストーリーや一般的な文章を生成・評価・進化させる反復型競争を通じた、現代的な進化的ライティングシステムです。モダンな設計原則に従ったクリーンでモジュラーなPythonアーキテクチャで構築されています。
## ✨ 特徴

- 🧬 **進化的アルゴリズム**: AI駆動のトーナメントでストーリーが進化
- 🎯 **二重の執筆モード**: 創造的な物語作成と一般的な文章最適化
- 🏆 **ELOトーナメントシステム**: ペア比較による高度なランキング
...
システムは、関心の分離を明確にした最新のPython設計原則に従っています：
...
- **`EvolutionPipeline`**: 完全な進化サイクルを管理するメインオーケストレーター  
- **`InitialStoryGenerator`**: プロンプトから最初の世代の物語を作成  
- **`NextBatchGenerator`**: 上位のパフォーマーを進化させ、改良版を生成  
- **`TournamentRunner`**: 物語ランキングのためのELOトーナメントを管理  
- **`EloRankingSystem`**: 高度なランキングアルゴリズムを実装  

## 📖 動作の仕組み

システムは三段階の進化サイクルに従います：

### 1. **初期バッチの生成**  
- あなたのプロンプトから複数の物語を作成  
- 設定可能なAIモデル（GPT、Claude、Llamaなど）を使用  
- 初期ELO評価を割り当て  
...
この反復プロセスにより、時間とともにより良い文章が進化します。  
...
- **`prompt.txt`** - あなたの執筆プロンプト／テーマ  
- **`rubric.txt`** - 評価基準のルーブリック  
- **`config.json`** - システム設定


### APIキーの設定

システムは `config.json` を通じて複数のAIプロバイダーをサポートします。設定はモデルをプロバイダーにマッピングし、APIキーを含む環境変数を指定します：
...
使用するプロバイダーのキーのみを設定すれば十分です。システムは設定に基づいてモデルのリクエストを正しいプロバイダーに自動的にルーティングします。
...
上級ユーザー向けに、`src/generators/`ディレクトリ内のファイルを編集して生成プロンプト、著者スタイル、ミッションセットをカスタマイズできます：
...
#### 一般的な文章カスタマイズ (`src/generators/generate_response_general.py`)

**学術的な焦点分野** - 特定の執筆分野を対象とする：
```python
academic_focuses = [
...
### ジャッジ基準のカスタマイズ（`src/generators/judge_response.py`）

ジャッジ基準や評価プロンプトを変更して、文章の特定の品質要素に焦点を当てることができます：
...
1. **元ファイルのバックアップ**：変更を加える前に必ず元のジェネレータファイルのコピーを保管してください  
2. **段階的にテスト**：小さな変更を行い、数回のイテレーションでテストしてから完全な進化サイクルを実行してください  
3. **結果の監視**：ウェブインターフェースを使ってカスタマイズが物語の質を向上させているかを検証しましょう  
4. **組み合わせる**：異なるミッションセットや作家スタイルを組み合わせて独自の生成戦略を作成しましょう  
5. **バージョン管理**：gitでカスタマイズを管理し、必要に応じて元に戻せるようにしましょう  

このモジュラー方式により、特定の執筆目標やドメインに合わせて進化プロセスを微調整できます。

## 🌐 ウェブインターフェース

組み込みのウェブインターフェースで進化結果を検証しましょう：
