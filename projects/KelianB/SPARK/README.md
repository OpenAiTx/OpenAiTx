...
- Create the environment using [setup.sh](https://raw.githubusercontent.com/KelianB/SPARK/main/./setup.sh).
- Run [TrackerAdaptation/setup_submodules.sh](https://raw.githubusercontent.com/KelianB/SPARK/main/./TrackerAdaptation/setup_submodules.sh). This may take a few minutes.
...
We advise starting from the provided example config and modifying `input_dir`, `train_dir` and `output_dir`. For a list of all parameters, please refer to [arguments.py](https://raw.githubusercontent.com/KelianB/SPARK/main/./MultiFLARE/arguments.py) or the output of `python train.py --help`. Parameters can be passed either in the config file or as command line arguments.
