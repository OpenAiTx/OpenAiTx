...
I have included the file explorer Nautilus as a dependency in order to have access to the share menu and to have the file sync status indicator (like on Windows or Ubuntu).\
I have also included a GNOME shell extension as a dependency in order to see the tray icon (works natively with KDE).\
I have built a second package for users who don't use GNOME. It does not contain the two dependencies mentioned above (they can be installed later if you want).
...
4. Optional, if you want to change the version:
   ...
   2. Edit the first two lines of `synology-drive.spec` or `synology-drive-noextra.spec`, depending on whether you are running GNOME or another desktop environment. 
5. For GNOME:
...
7. Clean build root : `rm -r ~/rpmbuild`
**Attention**_**: This solution uses the FEX emulator. If you are using the QEMU emulator it will probalbly break because binfmt_misc allows only one emulator to register for x86_64 binaries.
