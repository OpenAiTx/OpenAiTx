...
By continuously plotting all payloads for each CAN id, spotting trends that correspond to a specific action can become significantly easier to identify. 
...
The GUI can be launched by running one of the commands below from a terminal.
```sh 
can-explorer
``` 
