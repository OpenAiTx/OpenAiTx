...
You will need simple program / script to calibrate each finger, available here :
...
If you need to build a standalone Left hand, you can keep the same IDs for servos location, and select if it's a right or left hand in the software.

BUT, if you need to build both right and left hands to plug them on a robot, you will have to attribute different IDs for right and left hands. You can't have same ID for different servos on the same serial bus...
...
This design has not yet been tested for long and complex prehensile tasks. Before being able to grasp objects safely (that means without damaging servos or mechanical parts), kind of smart software needs to be built.
SCS0009 servos have smart capabilities such as:
...
- Study possibility to have 4 different finger lengths, or add a 5th finger
...
- Study possibility to add compliancy by replacing rigid links with springs
...
- [Augustin Crampette](https://fr.linkedin.com/in/augustin-crampette) & [Matthieu Lapeyre](https://www.linkedin.com/in/matthieulapeyre/) for open discussions and mechanical advice
