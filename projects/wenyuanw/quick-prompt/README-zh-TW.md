...
一款強大的瀏覽器擴充套件，專注於提示詞管理與快速輸入。協助使用者建立、管理和組織提示詞庫，並可在任何網頁輸入框中快速插入預設的 Prompt 內容，大幅提升工作效率。

> 由於瀏覽器擴充套件僅能用於網頁端，因此我們開源了一個功能相同的 [Raycast 插件](https://github.com/wenyuanw/quick-prompt-raycast)，相容瀏覽器擴充套件資料格式，支援以相同 JSON 資料無縫遷移。

## ✨ 功能特性

- 📚 **提示詞管理**：便捷地建立、編輯和管理你的提示詞庫
- 🚀 **快速輸入**：在任何網頁輸入框中通過 `/p` 快速觸發提示詞選擇器
- ⌨️ 支援設定快捷鍵開啟提示詞選擇器&儲存選中文本為提示詞
- 📑 支援右鍵選單直接儲存選中文本為提示詞
- 🎯 支援自訂提示詞，包括分類、標題、內容、標籤和變數
- 🧑‍💻 **變數支援**：提示詞中可以使用 `{{變數名}}` 格式的變數，使用時可填寫具體值
- 💾 **資料備份**：支援提示詞庫的匯出和匯入，方便跨裝置遷移與備份
- 🔗 **雲端同步**：支援將提示詞庫與 Notion 資料庫或 Gitee/GitHub Gist 同步
- 🔍 支援搜尋和篩選提示詞功能
- 🌙 自動適應系統的明暗主題
...
3. **選擇提示詞**：從彈出的選擇器中點擊所需提示詞，將自動插入當前輸入框
4. **快速儲存提示詞**：選取任意文字後，使用 `Ctrl+Shift+S`（Windows/Linux）或 `Command+Shift+S`（macOS）快速儲存為提示詞
5. **右鍵選單儲存**：選取任意文字後，右鍵點擊並選擇「儲存該提示詞」，將選取內容儲存為提示詞
6. **匯出提示詞庫**：在管理頁面點擊「匯出」按鈕，將所有提示詞匯出為 JSON 檔案並儲存到本機
7. **匯入提示詞庫**：在管理頁面點擊「匯入」按鈕，從本機 JSON 檔案中匯入提示詞（支援合併或覆蓋現有提示詞）

## 📸 介面預覽

Quick Prompt 提供直觀友善的使用者介面，讓你輕鬆管理和使用提示詞。
...
現已上架 Chrome 應用程式商店！[點此下載安裝](https://chromewebstore.google.com/detail/quick-prompt/hnjamiaoicaepbkhdoknhhcedjdocpkd)
