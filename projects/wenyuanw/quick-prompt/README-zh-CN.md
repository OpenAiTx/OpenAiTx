...
一款强大的浏览器扩展，专注于提示词管理与快速输入。帮助用户创建、管理和组织提示词库，并能在任意网页输入框中快速插入预设的 Prompt 内容，提高工作效率。

## ✨ 功能特性

- 📚 **Prompt词管理**：便捷地创建、编辑和管理你的Prompt词库
- 🚀 **快速输入**：在任何网页输入框中通过 `/p` 快速触发Prompt词选择器
- ⌨️ 支持配置快捷键打开Prompt词选择器&保存选中文本为Prompt词
- 📑 支持右键菜单直接保存选中文本为Prompt词
- 🎯 支持自定义Prompt词，包括分类、标题、内容、附件、标签和变量
- 🧑‍💻 **变量支持**：Prompt词中可以使用 `{{变量名}}` 格式的变量，使用时可以填写具体的值
- 💾 **数据备份**：支持Prompt词库的导出和导入，方便跨设备迁移和备份
- 🔗 **云端同步**：Prompt词库支持 WebDAV 同步、Notion数据库、Gitee/GitHub Gist同步
- 🔍 支持Prompt词搜索和过滤功能
- 🌙 自动适应系统的明暗主题

## 🚀 使用方法

1. **快速触发**：在任何网页的文本输入框中输入 `/p` 以触发Prompt词选择器
2. **快捷键打开选择器**：使用 `Ctrl+Shift+P`（Windows/Linux）或 `Command+Shift+P`（macOS）打开Prompt词选择器
3. **选择Prompt词**：从弹出的选择器中点击所需的Prompt词，它将自动插入到当前输入框中
4. **快速保存Prompt词**：选中任意文本后，使用 `Ctrl+Shift+S`（Windows/Linux）或 `Command+Shift+S`（macOS）快速保存为Prompt词
5. **右键菜单保存**：选中任意文本后，右键点击并选择“保存该Prompt词”，将选中内容保存为Prompt词
6. **导出Prompt词库**：在管理页面点击“导出”按钮，将所有Prompt词导出为JSON文件保存到本地
7. **导入Prompt词库**：在管理页面点击“导入”按钮，从本地JSON文件中导入Prompt词（支持合并或覆盖现有Prompt词）

## 📸 界面预览

Quick Prompt 提供直观友好的用户界面，让你轻松管理和使用Prompt词。

### Prompt词选择器

![Prompt词选择器](https://github.com/user-attachments/assets/41b9897c-d701-4ff0-97f7-2f1754f570a8)

![Prompt词选择器](https://github.com/user-attachments/assets/22d9d30c-b4c3-4e34-a0a0-8ef51e2cb942)

使用 `/p` 快捷指令或快捷键可以在任何输入框中快速调出Prompt词选择器，让你方便地挑选并插入需要的Prompt词。

### Prompt词管理页面

![Prompt词管理](https://github.com/user-attachments/assets/371ae51e-1cee-4a66-a2a5-cca017396872)

在管理页面中，你可以创建新的Prompt词、编辑现有Prompt词、添加标签以及进行分类管理。界面简洁明了，操作便捷。
