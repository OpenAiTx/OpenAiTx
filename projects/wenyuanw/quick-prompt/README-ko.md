...
- ⌨️ 단축키 설정을 통해 프롬프트 선택기 열기 및 선택한 텍스트를 프롬프트로 저장 지원
- 📑 마우스 오른쪽 버튼 메뉴에서 선택한 텍스트를 바로 프롬프트로 저장 지원
- 🎯 프롬프트 사용자 정의 지원: 분류, 제목, 내용, 첨부파일, 태그, 변수 포함
- 🧑‍💻 **변수 지원**: 프롬프트에 `{{변수명}}` 형식의 변수를 사용할 수 있으며, 사용 시 값을 입력 가능
- 💾 **데이터 백업**: 프롬프트 라이브러리 내보내기 및 가져오기 지원, 기기 간 이전 및 백업 용이
- 🔗 **클라우드 동기화**: 프롬프트 라이브러리 WebDAV, Notion 데이터베이스, Gitee/GitHub Gist 동기화 지원
- 🔍 프롬프트 검색 및 필터링 기능 지원
- 🌙 시스템의 다크/라이트 테마 자동 적용

## 🚀 사용 방법

1. **빠른 호출**: 모든 웹페이지의 텍스트 입력란에 `/p` 입력으로 프롬프트 선택기 호출
2. **단축키로 선택기 열기**: `Ctrl+Shift+P`(Windows/Linux) 또는 `Command+Shift+P`(macOS)로 프롬프트 선택기 열기
3. **프롬프트 선택**: 팝업된 선택기에서 원하는 프롬프트 클릭 시 현재 입력란에 자동 삽입
4. **프롬프트 빠른 저장**: 텍스트 선택 후 `Ctrl+Shift+S`(Windows/Linux) 또는 `Command+Shift+S`(macOS)로 빠르게 프롬프트로 저장
5. **마우스 오른쪽 버튼 메뉴 저장**: 텍스트 선택 후 우클릭, "해당 프롬프트 저장" 선택으로 내용 저장
6. **프롬프트 라이브러리 내보내기**: 관리 페이지에서 "내보내기" 버튼 클릭 시 전체 프롬프트를 JSON 파일로 로컬 저장
7. **프롬프트 라이브러리 가져오기**: 관리 페이지에서 "가져오기" 버튼 클릭, 로컬 JSON 파일에서 프롬프트 가져오기(병합 또는 기존 프롬프트 덮어쓰기 지원)

## 📸 인터페이스 미리보기

Quick Prompt는 직관적이고 친숙한 사용자 인터페이스를 제공하여 프롬프트 관리 및 사용이 쉽습니다.
...
`/p` 단축 명령이나 단축키로 어떤 입력란에서도 프롬프트 선택기를 빠르게 불러와 필요 프롬프트를 간편하게 선택, 삽입할 수 있습니다.

### 프롬프트 관리 페이지

![프롬프트 관리](https://github.com/user-attachments/assets/371ae51e-1cee-4a66-a2a5-cca017396872)

관리 페이지에서 새 프롬프트 생성, 기존 프롬프트 편집, 태그 추가, 분류 관리를 할 수 있습니다. 인터페이스는 간결하고 조작이 편리합니다.

### 마우스 오른쪽 버튼 메뉴 저장

![우클릭 메뉴 저장](https://github.com/user-attachments/assets/17fc3bfd-3fa4-4b0b-ae1a-5cfd0b62be2e)

웹페이지에서 원하는 텍스트를 선택한 후, 우클릭만으로 해당 내용을 빠르게 프롬프트로 저장하여 업무 효율을 높일 수 있습니다.
...
1. 확장 프로그램 아이콘을 클릭한 후 "프롬프트 관리" 버튼을 클릭하세요.
...
<p>
  <a href="https://chromewebstore.google.com/detail/quick-prompt/hnjamiaoicaepbkhdoknhhcedjdocpkd">
    <img src="https://img.shields.io/badge/Chrome-%EC%95%B1%EC%8A%A4%ED%86%A0%EC%96%B4-4285F4?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Chrome 앱스토어">
  </a>
  <a href="https://addons.mozilla.org/firefox/addon/quick-prompt/">
    <img src="https://img.shields.io/badge/Firefox-%EC%B6%94%EA%B0%80%ED%99%94-%EC%BB%A4%EB%84%A4-FF7139?style=for-the-badge&logo=firefoxbrowser&logoColor=white" alt="Firefox 추가 기능 스토어">
  </a>
</p>
...
2. 최신 버전의 빌드된 확장 프로그램 패키지를 다운로드하세요.
3. 다운로드한 파일을 압축 해제하세요.
4. 아래 설명에 따라 빌드된 확장 프로그램을 설치하세요
