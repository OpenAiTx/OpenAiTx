...
El `fittencode.nvim` refactorizado está listo.

## ✨ Características
- 🚀 Autocompletado rápido gracias a Fitten Code
- ⚡️ E/S asíncrona para mejor rendimiento
- ✏️ Autocompletado en línea mejorado
  - ✨ Autocompletado en línea
  - ✂️ Edición de autocompletado
- 📂 Autocompletado de proyectos
  - 🧩 C/C++
- 🔄 Aceptar/Revocar por rango
  - 📝 Carácter
  - 📖 Palabra
  - 📜 Línea
  - 📋 Todo
- 🀄️ Autocompletado chino mejorado con soporte para segmentación de caracteres
- 🔌 Fuentes para `blink.cmp` y servidor LSP nativo de Neovim
...
Instale el plugin con su gestor de paquetes preferido:

Por ejemplo con `lazy.nvim`:
...
> Esta versión no soporta la funcionalidad de chat.
...
| login3rd               | Inicio de sesión tercero|
| logout                 | Cerrar sesión          |
| ask_question           | Hacer pregunta         |
| user_guide             | Guía del usuario       |
| enable_completions     | Habilitar completaciones|
| disable_completions    | Deshabilitar completaciones|
| onlyenable_completions | Habilitar completaciones para archivos específicos|
| onlydisable_completions| Deshabilitar completaciones para archivos específicos|

## ✏️ API

| Función               | Descripción                               |
|-----------------------|-------------------------------------------|
| `has_completions()`   | Verificar si hay sugerencias de completado disponibles |
| `accept(scope)`       | Aceptar la sugerencia de completado actual|
| `revoke()`            | Deshacer la última operación de completado|
| `completion_cancel()` | Cancelar la sugerencia de completado actual|
| `get_status()`        | Obtener el estado actual del sistema de completado|
