...
```
Cela fonctionne également avec ```sqlmodel```


### Exemples

Notez que l'objet session fourni par ``db.session`` est basé sur ``ContextVar`` de Python3.7+. Cela signifie que
chaque session est liée au contexte de requête individuel dans lequel elle a été créée.
