...
```
También funciona con ```sqlmodel```


### Ejemplos

Tenga en cuenta que el objeto session proporcionado por ``db.session`` se basa en ``ContextVar`` de Python3.7+. Esto significa que
cada sesión está vinculada al contexto de solicitud individual en el que fue creada.
