...
## Workflows
Please find the workflows in [workflows](workflows) folder, [this v1.1 workflow](https://raw.githubusercontent.com/ToTheBeginning/ComfyUI-DreamO/main/workflows/dreamo_comfyui_v1.1.json) is the latest version.
...
- DreamORefEncode
  - This node encodes the reference image into a latent representation based on the selected task type. Three task types are available: ip, id, and style.
    - ip: will remove the background of the reference image
    - id: will align & crop the face from the reference image, similar to PuLID
    - style: will keep the background of the reference image. You still need to trigger meta prompt to activate the style transfer task
- ApplyDreamO
