...
- Tự động tạo thuộc tính SRI cho các thẻ `<script>` và `<link>` trong tệp HTML
- Hỗ trợ các thuật toán băm SHA-256, SHA-384, SHA-512 (có thể cấu hình)
- Tự động thêm thuộc tính `crossorigin="anonymous"` để đảm bảo SRI hoạt động bình thường
- Chỉ hoạt động trong môi trường sản xuất, tự động bỏ qua trong môi trường phát triển
