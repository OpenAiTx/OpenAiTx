...
zyfun — это бесплатный, минималистичный и универсальный кроссплатформенный медиа-менеджер, который позволяет в одном месте управлять всеми типами ресурсов T1-T4, с встроенным многопоточным плеером для свободного переключения. Эксклюзивная функция «клавиша босса» — мгновенное скрытие приложения для незаметной работы. Откройте мир через просмотр фильмов!

<details>
<summary>Показать скриншоты программы</summary>

|                    Видео                    |                    Трансляция             |
| :-----------------------------------------: | :---------------------------------------: |
|  ![Видео](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-film.png)   | ![Трансляция](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-live.png) |
|                  **История**                |                 **Воспроизведение**       |
| ![История](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-history.png) | ![Воспроизведение](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-play.png) |

</details>

## 🌴 Важные замечания

- Публикация только на этом Github, не поддавайтесь на мошенничество; администраторам запрещено рекламировать и продвигать это ПО.
- Настоятельно призываем к легальному просмотру, данное ПО служит только в качестве проигрывателя и не занимается хранением или распространением ресурсов.
- Использовать только для личного обучения и обмена опытом, удалите в течение 24 часов, не используйте в коммерческих целях.
- Проект основан на [ZY-Player](https://github.com/Hunlongyu/ZY-Player), иконка создана @fourbeauty.
...
- **MacOS**:
  - Доступны установочные пакеты для архитектур arm64 (Apple Silicon)/x64 (Intel)
  - Пути для базы данных/файлов/плагинов/логов: `~/Library/Application Support/zyfun/`
  - Путь к бинарным файлам: `~/.zy/bin/`
- **Linux**:
  - Доступны image/deb-пакеты для архитектур arm64/x64
  - Пути для базы данных/файлов/плагинов/логов: `~/.config/zyfun/`
  - Путь к бинарным файлам: `~/.zy/bin/`
- **Windows**:
  - Доступны установочные пакеты для архитектур arm64/x64
  - Пути для базы данных/файлов/плагинов/логов: `%USERPROFILE%\AppData\Roaming\zyfun\`
  - Путь к бинарным файлам: `%USERPROFILE%\.zy\bin\`
  <!-- - **OpenHarmony**:
  - Доступны установочные пакеты для архитектур arm64/x64
  - Пути для базы данных/файлов/плагинов/логов: `/data/storage/zyfun/base/files/log`
  - Путь к бинарным файлам: `~/.zy/bin/`-->

## 🛠️ Установка и скачивание

> **Последний стабильный релиз**: Перейдите на [страницу релизов GitHub](https://github.com/Hiram-Wong/ZyPlayer/releases), чтобы скачать

> **Последний девелоперский билд**: Перейдите на [страницу GitHub Actions](https://github.com/Hiram-Wong/ZyPlayer/actions/workflows/nightly-build.yml), требуется вход в аккаунт Github
...
### Ошибка запуска Appimage в Linux

> Запуск от имени root без --no-sandbox не поддерживается
...
> Выход из процесса GPU из-за ошибок при инициализации

```bash
xhost +
```

### Не удалось установить deb-пакет в Linux
...
> Пакет libxss1 не установлен

```bash
sudo apt-get install libxss1
```

### Ошибка установки rpm-пакета в Linux

> Проверка зависимостей не пройдена: требуется libXScrnSaver
...
> Проверка зависимостей не выполнена: требуется xdg-utils
...
> Пожалуйста, строго следуйте структуре данных при импорте, чтобы избежать ненужных ошибок (например, несоответствие типов данных в полях)

<details>
<summary>Развернуть для просмотра описания интерфейса</summary>
...
- **Вклад в код**: Разработка новых функций или оптимизация существующего кода <[Документация по разработке](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/Develop.md)>
- **Исправление ошибок**: Отправка исправлений для любых обнаруженных вами ошибок <[Стандарты для коммитов](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/Conventional_Commits.md)>
- **Управление вопросами**: Помощь в управлении проблемами на GitHub
- **Дизайн продукта**: Участие в обсуждениях по дизайну
- **Написание документации**: Улучшение руководств пользователя и инструкций
...
<a href="https://www.star-history.com/#Hiram-Wong/ZyPlayer&Date">
 <picture>
   ...
   <img alt="График истории звезд" src="https://api.star-history.com/svg?repos=Hiram-Wong/ZyPlayer&type=Date" />
 </picture>
</a>
