...
zyfun یک مدیریت‌کننده رایگان، ساده و قدرتمند چندرسانه‌ای چندسکویی است که به صورت یک‌جا منابع همه نوع T1-T4 را مدیریت می‌کند و دارای پخش‌کننده چندهسته‌ای داخلی با امکان تعویض آسان است. کلید اختصاصی مخفی‌سازی، با یک کلیک بدون نگرانی پنهان شوید. با تماشا، جهان را کشف کنید!

<details>
<summary>برای مشاهده تصاویر نرم‌افزار کلیک کنید</summary>

|                            فیلم (صفحه اصلی)                            |                            فیلم (جستجو)                            |
| :--------------------------------------------------------------: | :--------------------------------------------------------------: |
| ![فیلم-صفحه‌اصلی](https://s2.loli.net/2024/07/20/T8diRvA1q2QaJmP.png) | ![فیلم-جستجو](https://s2.loli.net/2024/07/20/utwCUsVG6c7ShRJ.png) |
|                            فیلم (پخش)                            |                            آنالیز (صفحه اصلی)                            |
| ![فیلم-پخش](https://s2.loli.net/2024/07/20/DNFBu3LphASROVy.png)  | ![آنالیز-صفحه‌اصلی](https://s2.loli.net/2024/07/20/JZANVebW4EHOU7I.png) |
|                            پخش زنده (صفحه اصلی)                            |                            پخش زنده (پخش)                            |
| ![پخش‌زنده-صفحه‌اصلی](https://s2.loli.net/2024/07/20/ucxSw3nR2EahG59.png) | ![پخش‌زنده-پخش](https://s2.loli.net/2024/07/20/jYEFc6NOwtdWJBV.png) |
|                             تاریخچه                             |                               تنظیمات                               |
| ![تاریخچه](https://s2.loli.net/2024/07/20/DiYsx4fbcH3KU6T.png)  |   ![تنظیمات](https://s2.loli.net/2024/07/20/QeaDxZ2IYCFoVki.png)    |

</details>

## 🌴 نکات مهم

- فقط در گیت‌هاب منتشر شده است، مراقب کلاهبرداری باشید؛ لطفاً مدیران نرم‌افزار را از تبلیغ و جذب کاربر منع کنید.
- به شدت توصیه می‌شود به صورت قانونی فیلم تماشا کنید، این نرم‌افزار فقط ابزار پخش است و هیچ نقشی در ذخیره یا توزیع منابع ندارد.
- صرفاً برای یادگیری و تبادل شخصی است، لطفاً ظرف ۲۴ ساعت به طور خودکار حذف کنید و از استفاده تجاری خودداری نمایید.
- منبع اولیه از [ZY-Player](https://github.com/Hunlongyu/ZY-Player) بوده و آیکون توسط @fourbeauty طراحی شده است.
- لطفاً قبل از استفاده، حتماً توافق‌نامه کاربری را به دقت مطالعه و قبول کنید تا از رعایت مقررات اطمینان حاصل شود.

## 💻 راهنمای پلتفرم‌های مختلف

- **MacOS**:
  - بسته نصبی برای معماری arm64 (چیپ اپل)/x64 (چیپ اینتل) ارائه شده است
  - مسیر بانک اطلاعاتی/فایل/افزونه/لاگ: `~/Library/Application Support/zyfun/`
  - مسیر فایل اجرایی: `~/.zy/bin/`
- **Linux**:
  - بسته نصبی-image/deb برای معماری arm64/x64 ارائه شده است
  - مسیر بانک اطلاعاتی/فایل/افزونه/لاگ: `~/.config/zyfun/`
  - مسیر فایل اجرایی: `~/.zy/bin/`
- **Windows**:
  - بسته نصبی برای معماری arm64/x64 ارائه شده است
  - مسیر بانک اطلاعاتی/فایل/افزونه/لاگ: `%USERPROFILE%\AppData\Roaming\zyfun\`
  - مسیر فایل اجرایی: `%USERPROFILE%\.zy\bin\`
  <!-- - **OpenHarmony**:
  - بسته نصبی برای معماری arm64/x64 ارائه شده است
  - مسیر بانک اطلاعاتی/فایل/افزونه/لاگ: `/data/storage/zyfun/base/files/log`
  - مسیر فایل اجرایی: `~/.zy/bin/`-->

## 🛠️ دانلود و نصب

> **دانلود آخرین نسخه منتشرشده**: از [صفحه انتشار GitHub](https://github.com/Hiram-Wong/ZyPlayer/releases) دریافت نمایید

> **دانلود آخرین نسخه توسعه**: از [صفحه GitHub Actions](https://github.com/Hiram-Wong/ZyPlayer/actions/workflows/nightly-build.yml) دریافت نمایید، نیاز به ورود به حساب گیت‌هاب دارد

### بعد از نصب MacOS با پیام «فایل خراب است» مواجه می‌شوید
...
> اجرای به عنوان root بدون --no-sandbox پشتیبانی نمی‌شود
...
> خروج از فرآیند GPU به دلیل خطاها در هنگام اولیه‌سازی

```bash
xhost +
```

### نصب deb در لینوکس با شکست مواجه شد
...
> لطفاً داده‌ها را دقیقاً مطابق با ساختار داده وارد کنید تا از خطاهای غیر ضروری (مانند عدم تطابق نوع داده‌ها) جلوگیری شود.

<details>
<summary>برای مشاهده توضیحات رابط کاربری کلیک کنید</summary>
...
در اینجا چند راه وجود دارد که می‌توانید مشارکت کنید:

- **مشارکت در کد**: توسعه ویژگی‌های جدید یا بهینه‌سازی کد فعلی <[مستندات توسعه](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/Develop.md)>
- **رفع اشکال**: ارسال اصلاحات برای هرگونه باگی که پیدا می‌کنید <[قالب ارسال](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/Conventional_Commits.md)>
- **مدیریت مسائل**: کمک به مدیریت مشکلات GitHub
- **طراحی محصول**: شرکت در بحث‌های طراحی
- **نویسندگی مستندات**: بهبود راهنماها و مستندات کاربری
- **مشارکت اجتماعی**: شرکت در بحث‌ها و کمک به کاربران
...
<a href="https://www.star-history.com/#Hiram-Wong/ZyPlayer&Date">
 <picture>
   ...
   <img alt="نمودار تاریخچه Star" src="https://api.star-history.com/svg?repos=Hiram-Wong/ZyPlayer&type=Date" />
 </picture>
</a>

## 💰 حامیان مالی

- [NodeSupport](https://github.com/NodeSeekDev/NodeSupport) این پروژه را حمایت کرده است
...
[sponsor-shield]: https://img.shields.io/badge/حمایت-FF6699.svg?logo=githubsponsors&logoColor=white
[sponsor-link]: https://ko-fi.com/hiramwong
[telegram-shield]: https://img.shields.io/badge/پیام‌رسان-Telegram-0088CC?logo=telegram
