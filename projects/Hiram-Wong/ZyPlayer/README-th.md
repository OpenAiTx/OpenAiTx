...
zyfun เป็นแอปพลิเคชันฟรี เรียบง่าย และครบเครื่องสำหรับจัดการสื่อข้ามแพลตฟอร์มแบบออลอินวัน รองรับการจัดการทรัพยากรทุกประเภท T1-T4 พร้อมเครื่องเล่นมัลติคอร์สลับเปลี่ยนได้ตามใจ มีฟีเจอร์ปุ่มลับเฉพาะสำหรับซ่อนหน้าจอได้ทันที ดูหนังค้นพบโลก!

<details>
<summary>คลิกเพื่อดูภาพหน้าจอซอฟต์แวร์</summary>

|                    ภาพยนตร์                     |                   ไลฟ์สด                   |
| :-----------------------------------------: | :--------------------------------------: |
|  ![ภาพยนตร์](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-film.png)   | ![ไลฟ์สด](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-live.png) |
|                  **ประวัติ**                   |                 **เล่น**                 |
| ![ประวัติ](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-history.png) | ![เล่น](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/assets/images/app-play.png) |

</details>

## 🌴 ข้อควรระวัง

- เผยแพร่เฉพาะบน Github นี้เท่านั้น กรุณาอย่าหลงเชื่อ; ขอให้ผู้ดูแลระบบทุกท่านงดเผยแพร่หรือชักจูงมายังซอฟต์แวร์นี้
- สนับสนุนการรับชมอย่างถูกกฎหมายอย่างเคร่งครัด ซอฟต์แวร์นี้เป็นเพียงเครื่องมือเล่นไฟล์ ไม่เกี่ยวข้องกับการจัดเก็บหรือแจกจ่ายทรัพยากร
- ใช้เพื่อการศึกษาแลกเปลี่ยนส่วนบุคคลเท่านั้น กรุณาถอดถอนภายใน 24 ชั่วโมง ห้ามใช้ในเชิงพาณิชย์
- ต้นกำเนิดมาจาก [ZY-Player](https://github.com/Hunlongyu/ZY-Player) ไอคอนออกแบบโดย @fourbeauty
- กรุณาอ่านและยอมรับข้อตกลงผู้ใช้โดยละเอียดก่อนเริ่มใช้งาน เพื่อให้มั่นใจว่าปฏิบัติตามข้อกำหนด

## 💻 คำอธิบายข้ามแพลตฟอร์ม

- **MacOS**:
  - มีแพ็กเกจติดตั้งสำหรับสถาปัตยกรรม arm64(ชิป Apple)/x64(ชิป Intel)
  - ฐานข้อมูล/ไฟล์/ปลั๊กอิน/บันทึก เส้นทาง: `~/Library/Application Support/zyfun/`
  - ไบนารี เส้นทาง: `~/.zy/bin/`
- **Linux**:
  - มี image/deb แพ็กเกจติดตั้งสำหรับสถาปัตยกรรม arm64/x64
  - ฐานข้อมูล/ไฟล์/ปลั๊กอิน/บันทึก เส้นทาง: `~/.config/zyfun/`
  - ไบนารี เส้นทาง: `~/.zy/bin/`
- **Windows**:
  - มีแพ็กเกจติดตั้งสำหรับสถาปัตยกรรม arm64/x64
  - ฐานข้อมูล/ไฟล์/ปลั๊กอิน/บันทึก เส้นทาง: `%USERPROFILE%\AppData\Roaming\zyfun\`
  - ไบนารี เส้นทาง: `%USERPROFILE%\.zy\bin\`
  <!-- - **OpenHarmony**:
  - มีแพ็กเกจติดตั้งสำหรับสถาปัตยกรรม arm64/x64
  - ฐานข้อมูล/ไฟล์/ปลั๊กอิน/บันทึก เส้นทาง: `/data/storage/zyfun/base/files/log`
  - ไบนารี เส้นทาง: `~/.zy/bin/`-->

## 🛠️ การดาวน์โหลดและติดตั้ง

> **ดาวน์โหลดเวอร์ชันล่าสุดที่เผยแพร่**: เข้าไปที่ [หน้าปล่อยเวอร์ชัน GitHub Releases](https://github.com/Hiram-Wong/ZyPlayer/releases) เพื่อรับไฟล์

> **ดาวน์โหลดเวอร์ชันล่าสุดสำหรับนักพัฒนา**: เข้าไปที่ [หน้าทำงาน GitHub Actions](https://github.com/Hiram-Wong/ZyPlayer/actions/workflows/nightly-build.yml) เพื่อรับไฟล์ ต้องเข้าสู่ระบบบัญชี Github

### MacOS เปิดใช้งานหลังติดตั้งแจ้งว่า "ไฟล์เสียหาย"
...
### การใช้งาน Linux Appimage ล้มเหลว

> การรันในโหมด root โดยไม่ใช้ --no-sandbox ไม่ได้รับการสนับสนุน

```bash
./zyfun.AppImage --no-sandbox
```

> dlopen()：เกิดข้อผิดพลาดขณะโหลด libfuse.so.2

```bash
sudo apt-get install libfuse2
```

> กำลังออกจากกระบวนการ GPU เนื่องจากเกิดข้อผิดพลาดระหว่างการเริ่มต้น
...
> ยังไม่ได้ติดตั้งแพ็กเกจ libxss1

```bash
sudo apt-get install libxss1
```

### การติดตั้งแพ็คเกจ rpm บน Linux ล้มเหลว

> ตรวจสอบการพึ่งพาล้มเหลว ต้องการ libXScrnSaver
...
> ตรวจสอบการพึ่งพาล้มเหลว ต้องการ xdg-utils
...
> กรุณานำเข้าข้อมูลตามโครงสร้างข้อมูลอย่างเคร่งครัด เพื่อหลีกเลี่ยงข้อผิดพลาดที่ไม่จำเป็น (เช่น ประเภทฟิลด์ข้อมูลไม่ตรงกัน)

<details>
<summary>คลิกเพื่อดูรายละเอียด API</summary>
...
- **ร่วมพัฒนาโค้ด**: สร้างฟีเจอร์ใหม่หรือปรับปรุงโค้ดเดิม <[เอกสารการพัฒนา](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/Develop.md)>
- **แก้ไขข้อผิดพลาด**: ส่งข้อเสนอแก้ไขข้อผิดพลาดที่คุณพบ <[มาตรฐานการส่งงาน](https://raw.githubusercontent.com/Hiram-Wong/ZyPlayer/main/docs/Conventional_Commits.md)>
- **ดูแลปัญหา**: ช่วยจัดการปัญหาใน GitHub
- **ออกแบบผลิตภัณฑ์**: เข้าร่วมการอภิปรายด้านการออกแบบ
- **เขียนเอกสาร**: ปรับปรุงคู่มือผู้ใช้และคำแนะนำ
- **เข้าร่วมชุมชน**: ร่วมสนทนาและช่วยเหลือผู้ใช้

[![][github-contributors-shield]][github-contributors-link]

## 🌟 ประวัติการดาวน์โหลด Star
...
<a href="https://www.star-history.com/#Hiram-Wong/ZyPlayer&Date">
 <picture>
   ...
   <img alt="กราฟประวัติ Star" src="https://api.star-history.com/svg?repos=Hiram-Wong/ZyPlayer&type=Date" />
 </picture>
</a>

## 💰 ผู้สนับสนุนหลัก

- [NodeSupport](https://github.com/NodeSeekDev/NodeSupport) สนับสนุนโปรเจ็กต์นี้
