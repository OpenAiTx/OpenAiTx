...
Ajan projemizle en iyi deneyimi elde etmek için ince ayar yapılmış modelimizi **şiddetle tavsiye ediyoruz**. Deneylerimize göre, **mevcut açık kaynaklı modelleri önemli ölçüde geride bırakmaktadır**.

| Format | HuggingFace | ModelScope |
|--------|-------------|------------|
| GGUF (Kantitatif) | [Forceless/DeepPresenter-9B-GGUF](https://huggingface.co/Forceless/DeepPresenter-9B-GGUF) | [forceless/DeepPresenter-9B-GGUF](https://modelscope.cn/models/forceless/DeepPresenter-9B-GGUF) |
| Tam Ağırlıklar | [Forceless/DeepPresenter-9B](https://huggingface.co/Forceless/DeepPresenter-9B) | [forceless/DeepPresenter-9B](https://modelscope.cn/models/forceless/DeepPresenter-9B) |

## 📅 Haberler

- **[2026/04]** 🎉 [DeepPresenter](https://arxiv.org/abs/2602.22839) **ACL 2026**'da kabul edildi!
- **[2026/03]** 🤗 İnce ayar yapılmış modeller ve görev setini [Hugging Face](https://huggingface.co/collections/ICIP/deeppresenter) üzerinde yayınladık.
- **[2026/01]** 🆕 Serbest ve şablon üretimi artık PPTX dışa aktarımı ve çevrimdışı modu destekliyor. Bağlam taşmasını önlemek için bağlam yönetimi eklendi.
- **[2025/12]** 🔥 **DeepPresenter** kod tabanı büyük yükseltmelerle yayınlandı — Derin Araştırma Entegrasyonu, Serbest Biçimli Görsel Tasarım, Otonom Varlık Oluşturma, Metinden Görsele Üretim ve 20+ araçlı sandık ortamı ile Ajan Ortamı.
...
- **[2025/08]** 🎉 [PPTAgent](https://arxiv.org/abs/2501.03936) **EMNLP 2025**'te kabul edildi!
- **[2025/05]** ⭐ GitHub'da **1.000 yıldız** sayısına ulaşıldı!
- **[2025/01]** 🔓 PPTAgent kod tabanı açık kaynak olarak yayınlandı.

## Kullanım 📖

> [!ÖNEMLİ]
> Windows desteklenmemektedir. Eğer Windows kullanıyorsanız, lütfen WSL kullanın.
>
> Bağımlılıkların ve ortamın doğru şekilde yapılandırıldığını doğrulamak için CLI ve minimum görev ile başlamanızı şiddetle tavsiye ederiz.

### Yapılandırma

CLI kullanıyorsanız, `pptagent onboard` bu yapılandırmaları interaktif olarak oluşturmanıza ve güncellemenize yardımcı olabilir. Docker Compose veya kaynaktan derleme kullanıyorsanız, bunları manuel olarak hazırlamalısınız:
...
#### Kaliteyi Artıran Opsiyonel Servisler
...
- **Tavily**: web arama kalitesini artırır. [tavily.com](https://www.tavily.com/) adresinden bir API anahtarı edinin, ardından `TAVILY_API_KEY` değerini [`deeppresenter/mcp.json`](https://raw.githubusercontent.com/icip-cas/PPTAgent/main/deeppresenter/mcp.json) dosyasına ekleyin.
- **MinerU**: PDF ayrıştırma kalitesini artırır. [mineru.net](https://mineru.net/apiManage/docs) üzerinden bir API anahtarı edinebilir ve `MINERU_API_KEY` değerini [`deeppresenter/mcp.json`](https://raw.githubusercontent.com/icip-cas/PPTAgent/main/deeppresenter/mcp.json) dosyasına ekleyebilir ya da MinerU’yu yerel olarak dağıtıp bunun yerine `MINERU_API_URL` değerini ayarlayabilirsiniz.
- **Metinden-görüntüye model**: görsel oluşturma kalitesini artırır. [`deeppresenter/config.yaml`](https://raw.githubusercontent.com/icip-cas/PPTAgent/main/deeppresenter/config.yaml) içindeki `t2i_model` değerini yapılandırın.


Tamamen çevrimdışı bir kurulum istiyorsanız, MinerU’yu yerel olarak dağıtın ve ağ bağlantılı araçların (web arama gibi) yüklenmesini önlemek için `deeppresenter/config.yaml` dosyasında `offline_mode: true` ayarını yapın.

Daha fazla yapılandırılabilir değişkeni [constants.py](https://raw.githubusercontent.com/icip-cas/PPTAgent/main/deeppresenter/utils/constants.py) dosyasında bulabilirsiniz.
...
| `pptagent generate` | Sunum oluştur                                    |
...
DeepPresenter iki çalışma zamanı imajı yayımlar:

| Yerel imaj adı | Amaç | Docker Hub | 1ms.run aynası |
| --- | --- | --- | --- |
| `deeppresenter-host` | Web UI ve orkestrasyon çalışma zamanı için ana servis | [`forceless/deeppresenter-host`](https://hub.docker.com/r/forceless/deeppresenter-host) | [`docker.1ms.run/forceless/deeppresenter-host`](https://1ms.run/r/forceless/deeppresenter-host) |
| `deeppresenter-sandbox` | İzole araç çalıştırması için çalışma zamanı tarafından kullanılan sandbox imajı | [`forceless/deeppresenter-sandbox`](https://hub.docker.com/r/forceless/deeppresenter-sandbox) | [`docker.1ms.run/forceless/deeppresenter-sandbox`](https://1ms.run/r/forceless/deeppresenter-sandbox) |
...
### 3. Sunucu Dağıtımı: Docker Compose
...
[![Yıldız Geçmişi Grafiği](https://api.star-history.com/svg?repos=icip-cas/PPTAgent&type=Date)](https://star-history.com/#icip-cas/PPTAgent&Date)

## Atıf 🙏

Eğer bu projeyi faydalı bulduysanız, lütfen aşağıdakini kaynak olarak kullanın:
```bibtex
