...
niestandardowa lista słów do fuzzingu `fuzzing_list.txt`
```
cat urls.txt | sed 's|\(.*\)/[^/]*$|\1|' | cut -d"/" -f4,5,6,7,8,9,10,11 | tr "/" "\n" | sed '/^$/d' | anew fuzzing_list.txt
```

niestandardowa lista słów DNS `dns-wordlist.txt`
```
cat alltargets.txt | sed 's/\.[^.]*$//' | tr "." "\n" | egrep -v '^[0-9]*$' | anew dns-wordlist.txt
```

skanuj te adresy URL pod kątem błędnej konfiguracji nuclei `urls-for-nuclei.txt`
```
...
niestandardowa lista parametrów `params.txt`
```
cat urls.txt | grep "\.php?" | uro | grep "?" | cut -f2 -d"?" | cut -f1 -d"=" | sed '/^\s*$/d'| anew params.txt
```

niestandardowa lista słów do fuzzingu `onelistforall.txt`
```
...
## payloady
- do top 50 => `*-small.txt`
- do top 500 => `*-medium.txt`
- wszystkie payloady bez limitu => `*-large.txt`, jeśli więcej niż 50MB to `*-large-1.txt`, `*-large-2.txt`

## technologie
- wszystkie technologie bez limitu => `techname/techname.txt`, jeśli więcej niż 50MB to `techname/techname-1.txt`, `techname/techname-2.txt`

## nuclei-technologies
Użycie: [nuclei-wordlist-generator.go](https://github.com/rix4uni/WordList/blob/main/wordlist-generator-tools/nuclei-wordlist-generator.go)
