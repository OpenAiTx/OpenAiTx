...
lista personalizada de fuzzing `fuzzing_list.txt`
```
...
escaneie essas URLs para núcleos de má configuração `urls-for-nuclei.txt`
```
...
lista personalizada de parâmetros `params.txt`
```
cat urls.txt | grep "\.php?" | uro | grep "?" | cut -f2 -d"?" | cut -f1 -d"=" | sed '/^\s*$/d'| anew params.txt
```

lista personalizada de fuzzing `onelistforall.txt`
```
...
## payloads
- até top 50 => `*-small.txt`
- até top 500 => `*-medium.txt`
- todos os payloads sem limite => `*-large.txt`, se for maior que 50mb então `*-large-1.txt`, `*-large-2.txt`

## tecnologias
- todas as tecnologias sem limite => `techname/techname.txt`, se for maior que 50mb então `techname/techname-1.txt`, `techname/techname-2.txt`

## nuclei-technologies
Utilizando: [nuclei-wordlist-generator.go](https://github.com/rix4uni/WordList/blob/main/wordlist-generator-tools/nuclei-wordlist-generator.go)
