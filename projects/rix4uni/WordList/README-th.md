...
รายการคำสำหรับ fuzzing แบบกำหนดเอง `fuzzing_list.txt`
```
cat urls.txt | sed 's|\(.*\)/[^/]*$|\1|' | cut -d"/" -f4,5,6,7,8,9,10,11 | tr "/" "\n" | sed '/^$/d' | anew fuzzing_list.txt
```

รายการคำ DNS แบบกำหนดเอง `dns-wordlist.txt`
```
cat alltargets.txt | sed 's/\.[^.]*$//' | tr "." "\n" | egrep -v '^[0-9]*$' | anew dns-wordlist.txt
```

สแกน URL เหล่านี้สำหรับ nuclei misconfiguration `urls-for-nuclei.txt`
```
...
รายการคำสำหรับพารามิเตอร์แบบกำหนดเอง `params.txt`
```
cat urls.txt | grep "\.php?" | uro | grep "?" | cut -f2 -d"?" | cut -f1 -d"=" | sed '/^\s*$/d'| anew params.txt
```

รายการคำสำหรับ fuzzing แบบกำหนดเอง `onelistforall.txt`
```
...
## payloads
- สูงสุด 50 รายการแรก => `*-small.txt`
- สูงสุด 500 รายการแรก => `*-medium.txt`
- รายการ payload ทั้งหมดแบบไม่จำกัด => `*-large.txt` , หากมีขนาดมากกว่า 50mb ให้ใช้ `*-large-1.txt`, `*-large-2.txt`

## technologies
- เทคโนโลยีทั้งหมดแบบไม่จำกัด => `techname/techname.txt` , หากมีขนาดมากกว่า 50mb ให้ใช้ `techname/techname-1.txt`, `techname/techname-2.txt`

## nuclei-technologies
ใช้: [nuclei-wordlist-generator.go](https://github.com/rix4uni/WordList/blob/main/wordlist-generator-tools/nuclei-wordlist-generator.go)
