...
кастомный список слов для фаззинга `fuzzing_list.txt`
```
cat urls.txt | sed 's|\(.*\)/[^/]*$|\1|' | cut -d"/" -f4,5,6,7,8,9,10,11 | tr "/" "\n" | sed '/^$/d' | anew fuzzing_list.txt
```

кастомный DNS-список слов `dns-wordlist.txt`
```
cat alltargets.txt | sed 's/\.[^.]*$//' | tr "." "\n" | egrep -v '^[0-9]*$' | anew dns-wordlist.txt
```

сканировать эти URL для nuclei misconfiguration `urls-for-nuclei.txt`
```
...
кастомный список параметров `params.txt`
```
cat urls.txt | grep "\.php?" | uro | grep "?" | cut -f2 -d"?" | cut -f1 -d"=" | sed '/^\s*$/d'| anew params.txt
```

кастомный список слов для фаззинга `onelistforall.txt`
```
...
## payloads
- до топ 50 => `*-small.txt`
- до топ 500 => `*-medium.txt`
- все payload-ы без ограничения => `*-large.txt`, если больше 50MB, то `*-large-1.txt`, `*-large-2.txt`

## технологии
- все технологии без ограничения => `techname/techname.txt`, если больше 50MB, то `techname/techname-1.txt`, `techname/techname-2.txt`

## nuclei-technologies
Используется: [nuclei-wordlist-generator.go](https://github.com/rix4uni/WordList/blob/main/wordlist-generator-tools/nuclei-wordlist-generator.go)
