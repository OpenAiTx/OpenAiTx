...
liste de mots personnalisée pour le fuzzing `fuzzing_list.txt`
```
cat urls.txt | sed 's|\(.*\)/[^/]*$|\1|' | cut -d"/" -f4,5,6,7,8,9,10,11 | tr "/" "\n" | sed '/^$/d' | anew fuzzing_list.txt
```

liste de mots DNS personnalisée `dns-wordlist.txt`
```
cat alltargets.txt | sed 's/\.[^.]*$//' | tr "." "\n" | egrep -v '^[0-9]*$' | anew dns-wordlist.txt
```

scanner ces urls pour la mauvaise configuration nuclei `urls-for-nuclei.txt`
```
...
liste personnalisée de paramètres `params.txt`
```
cat urls.txt | grep "\.php?" | uro | grep "?" | cut -f2 -d"?" | cut -f1 -d"=" | sed '/^\s*$/d'| anew params.txt
```

liste de mots personnalisée pour le fuzzing `onelistforall.txt`
```
...
## payloads
- jusqu’au top 50 => `*-small.txt`
- jusqu’au top 500 => `*-medium.txt`
- tous les payloads sans limite => `*-large.txt` , si plus de 50mb alors `*-large-1.txt`, `*-large-2.txt`

## technologies
- toutes les technologies sans limite => `techname/techname.txt` , si plus de 50mb alors `techname/techname-1.txt`, `techname/techname-2.txt`

## nuclei-technologies
Utilisation de : [nuclei-wordlist-generator.go](https://github.com/rix4uni/WordList/blob/main/wordlist-generator-tools/nuclei-wordlist-generator.go)
