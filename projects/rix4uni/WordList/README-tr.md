...
özel fuzzing kelime listesi `fuzzing_list.txt`
```
cat urls.txt | sed 's|\(.*\)/[^/]*$|\1|' | cut -d"/" -f4,5,6,7,8,9,10,11 | tr "/" "\n" | sed '/^$/d' | anew fuzzing_list.txt
```

özel dns kelime listesi `dns-wordlist.txt`
```
cat alltargets.txt | sed 's/\.[^.]*$//' | tr "." "\n" | egrep -v '^[0-9]*$' | anew dns-wordlist.txt
```

nuclei yanlış yapılandırmaları için bu url'leri tara `urls-for-nuclei.txt`
```
...
özel parametre kelime listesi `params.txt`
```
cat urls.txt | grep "\.php?" | uro | grep "?" | cut -f2 -d"?" | cut -f1 -d"=" | sed '/^\s*$/d'| anew params.txt
```

özel fuzzing kelime listesi `onelistforall.txt`
```
...
## payloadlar
- en fazla ilk 50 => `*-small.txt`
- en fazla ilk 500 => `*-medium.txt`
- sınırsız tüm payloadlar => `*-large.txt` , eğer 50mb'den büyükse `*-large-1.txt`, `*-large-2.txt`

## teknolojiler
- sınırsız tüm teknolojiler => `techname/techname.txt` , eğer 50mb'den büyükse `techname/techname-1.txt`, `techname/techname-2.txt`

## nuclei-teknolojileri
Kullanılan: [nuclei-wordlist-generator.go](https://github.com/rix4uni/WordList/blob/main/wordlist-generator-tools/nuclei-wordlist-generator.go)
