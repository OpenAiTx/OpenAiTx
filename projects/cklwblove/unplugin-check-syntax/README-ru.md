...
Универсальный плагин для проверки синтаксиса, поддерживающий различные сборщики (Webpack, Vite, Rollup, esbuild, Rspack и др.), предназначен для проверки совместимости синтаксиса ECMAScript в файлах JavaScript и HTML. Этот плагин основан на [rsbuild-plugin-check-syntax](https://github.com/rspack-contrib/rsbuild-plugin-check-syntax).

## Особенности

- 🔧 **Поддержка нескольких сборщиков**: поддержка Webpack, Vite, Rollup, esbuild, Rspack и др.
- 📝 **Различные типы файлов**: поддержка JavaScript-файлов и встроенных скриптов в HTML
- 🎯 **Точное определение местоположения**: поддержка sourcemap, можно отследить исходное расположение файла
- ⚙️ **Гибкая настройка**: поддержка исключения файлов, ручного включения, фильтрации по типу ошибок и др.
- 📁 **Интеллектуальный поиск файлов**: поддержка glob-масок для ручного добавления дополнительных JS/HTML файлов
- 🌐 **Интеграция с Browserslist**: поддержка автоматического преобразования конфигурации browserslist в версию ECMAScript
...
Если вы переходите с `@rsbuild/plugin-check-syntax`:
...
1. **Проверка после сборки**: Плагин проверяет выходные файлы после завершения сборки
2. **Синтаксический разбор**: Использует парсер Acorn для проверки синтаксиса JavaScript
3. **Проверка совместимости**: Проверяет совместимость синтаксиса в соответствии с настроенной версией ECMAScript
4. **Отчет об ошибках**: Предоставляет подробную информацию об ошибках, включая расположение в файле и фрагмент исходного кода
5. **Поддержка sourcemap**: Если присутствует sourcemap, осуществляется обратная трассировка до исходного файла
