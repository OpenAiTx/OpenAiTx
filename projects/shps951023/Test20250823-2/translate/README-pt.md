<!-- traduzido por https://openaitx.com por favor não remova esta observação, o sistema precisa disso para verificar o status do projeto -->
<div align="right">
  <details>
...
Existe uma função de desligamento ao usar Microsoft.AspNet.Server.Kestrel? O ASP.NET Core (anteriormente ASP.NET vNext) claramente possui uma sequência de inicialização, mas não há menção de sequência de desligamento e como lidar com o encerramento limpo.
...
No meu arquivo `Startup`, estou registrando o serviço OData, construindo os modelos e adicionando componentes de rota. Considere que a função `ConfigureCommonServices` está sendo chamada a partir do `Startup`:
...
O controlador OData é definido assim:
...
```
Embora `odata/v1/$metadata` mostre `UpdateRead` e `DeleteBulk` registrados como Actions, quando eu acesso a url POST `.../odata/v1/NotificationEntry/NotificationService.Api.UpdateRead` ou para `DeleteBulk` ele retorna 404.

Isso foi implementado anteriormente para .net6 e após a atualização para .net8 eu adicionei muitas mudanças para lidar com o roteamento OData, para o qual adicionei o código.

Tentei habilitar a opção `RoutingAttributes`, mas não ajudou.
