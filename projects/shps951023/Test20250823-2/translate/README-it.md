<!-- tradotto da https://openaitx.com per favore non rimuovere questa nota, il sistema ne ha bisogno per controllare lo stato del progetto -->
<div align="right">
  <details>
...
Esiste una funzione di spegnimento quando si utilizza Microsoft.AspNet.Server.Kestrel? ASP.NET Core (precedentemente ASP.NET vNext) ha chiaramente una sequenza di avvio, ma non viene menzionata alcuna sequenza di spegnimento né come gestire una chiusura pulita.
...
Nel mio file `Startup`, sto registrando il servizio OData, costruendo i modelli e aggiungendo i componenti di routing. Supponi che la funzione `ConfigureCommonServices` venga chiamata da `Startup`:
...
Il controller OData è definito in questo modo:
...
Anche se `odata/v1/$metadata` mostra `UpdateRead` e `DeleteBulk` registrate come Azioni, quando provo a chiamare l'URL POST `.../odata/v1/NotificationEntry/NotificationService.Api.UpdateRead` o per `DeleteBulk` restituisce 404.

Questo era stato implementato precedentemente per .net6 e dopo l'aggiornamento a .net8 ho apportato molte modifiche per gestire il routing OData per il quale ho aggiunto il codice.

Ho provato ad abilitare l'opzione `RoutingAttributes` ma non ha aiutato.
