<!-- diterjemahkan oleh https://openaitx.com mohon jangan hapus keterangan ini, sistem membutuhkan untuk memeriksa status proyek -->
<div align="right">
  <details>
...
Apakah terdapat fungsi shutdown saat menggunakan Microsoft.AspNet.Server.Kestrel? ASP.NET Core (sebelumnya ASP.NET vNext) jelas memiliki urutan Startup, namun tidak ada penyebutan tentang urutan shutdown dan bagaimana menangani penutupan secara bersih.
...
Dalam file `Startup` saya, saya mendaftarkan layanan OData, membangun model, dan menambahkan komponen rute. Asumsikan bahwa fungsi `ConfigureCommonServices` dipanggil dari `Startup`:
...
Kontroler OData didefinisikan seperti berikut:
...
```
Meskipun `odata/v1/$metadata` menampilkan `UpdateRead` dan `DeleteBulk` terdaftar sebagai Actions, ketika saya mengakses url POST `.../odata/v1/NotificationEntry/NotificationService.Api.UpdateRead` atau untuk `DeleteBulk` malah menghasilkan error 404.

Ini sebelumnya diimplementasikan untuk .net6 dan setelah upgrade ke .net8 saya menambahkan banyak perubahan untuk menangani routing OData yang mana saya telah menambahkan kodenya.

Saya mencoba mengaktifkan opsi `RoutingAttributes` tetapi tidak membantu.
