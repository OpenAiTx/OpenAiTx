...
Microsoft.AspNet.Server.Kestrel을 사용할 때 종료(shutdown) 기능이 있나요? ASP.NET Core(이전 명칭: ASP.NET vNext)에는 분명히 Startup 시퀀스가 있지만, 종료 시퀀스 및 정상적인 종료 처리 방법에 대한 언급은 없습니다.
...
내 `Startup` 파일에서 OData 서비스를 등록하고, 모델을 빌드하며, 라우트 컴포넌트를 추가하고 있습니다. `ConfigureCommonServices` 함수가 `Startup`에서 호출된다고 가정하세요:
...
비록 `odata/v1/$metadata`에서 `UpdateRead`와 `DeleteBulk`가 Action으로 등록된 것으로 표시되지만, POST url `.../odata/v1/NotificationEntry/NotificationService.Api.UpdateRead` 또는 `DeleteBulk`에 요청하면 404 오류가 발생합니다.

이 기능은 이전에 .net6에서 구현되었으며 .net8로 업그레이드한 후 OData 라우팅을 처리하기 위해 많은 코드를 추가하였습니다.

`RoutingAttributes` 옵션을 활성화해보았지만 도움이 되지 않았습니다.
