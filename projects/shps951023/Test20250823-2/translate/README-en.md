...
In my `Startup` file, I'm registering OData service, building the models and adding route components. Assume that the `ConfigureCommonServices` function is being called from `Startup`:
...
This was previously implemented for .net6 and after upgrading to .net8 I added a lot of changes to handle OData routing for which I have added the code.

I tried enabling the `RoutingAttributes` option but it didn't help.
