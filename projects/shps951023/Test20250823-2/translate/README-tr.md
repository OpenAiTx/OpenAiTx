...
Microsoft.AspNet.Server.Kestrel kullanırken bir kapatma fonksiyonu var mı? ASP.NET Core (eski adıyla ASP.NET vNext) açıkça bir Başlatma dizisine sahip, ancak kapatma dizisi ve düzgün kapanışın nasıl ele alınacağı ile ilgili bir bilgi yok.
...
`Startup` dosyamda OData servisini kaydediyor, modelleri oluşturuyor ve rota bileşenlerini ekliyorum. `ConfigureCommonServices` fonksiyonunun `Startup` dosyasından çağrıldığını varsayabilirsiniz:
...
OData denetleyicisi şu şekilde tanımlanır:
...
Her ne kadar `odata/v1/$metadata` içinde `UpdateRead` ve `DeleteBulk` Action olarak kayıtlı görünse de, POST url'sine `.../odata/v1/NotificationEntry/NotificationService.Api.UpdateRead` veya `DeleteBulk` için istek gönderdiğimde 404 hatası alıyorum.

Bu daha önce .net6 için uygulanmıştı ve .net8'e yükselttikten sonra OData yönlendirmesini yönetmek için bir sürü değişiklik ekledim ve ilgili kodu ekledim.

`RoutingAttributes` seçeneğini etkinleştirmeyi denedim ama işe yaramadı.
