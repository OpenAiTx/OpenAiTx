...
Moreover, observe that the ``aspect_ratio`` parameter is defined as the height of the plot divided by its width. Even though aspect ratios are more commonly defined as width/height, this choice results in having the width and the height of the figure proportional to ``width_ratio`` and ``aspect_ratio`` respectively. 
